...
public class ReceiptDetailModel {
    ...
    public ReceiptDetailModel loadInstanceguest(AppView app, String id) throws BasicException {
        ...
        List<ReceiptGuest> li = new StaticSentence(app.getSession(), "SELECT RECEIPT,RDATE,CNAME,CKEY,WNAME,BAMOUNT,TAX,TOTAL,PAYMENT,NUM,RUSER,ROLE,GUESTNAME FROM " +
                "(SELECT R.ID AS RECEIPT,R.DATENEW AS RDATE,C.NAME AS CNAME,C.SEARCHKEY AS CKEY,G.NAME AS WNAME,G.RATE  AS BAMOUNT,0 AS TAX,P.TOTAL AS TOTAL,P.PAYMENT AS PAYMENT ,GL.NUM AS NUM,R.RUSER AS RUSER,RO.NAME AS ROLE,GL.NAMES AS GUESTNAME  FROM RECEIPTS R,GUESTCAT G,CUSTOMERS C,PAYMENTS P ,GUESTLOG GL,BILL B,PEOPLE PE,ROLES RO WHERE  P.CUSTOMER=C.ID AND R.ID=? AND P.RECEIPT=R.ID AND G.ID=R.DESC_ AND C.ID=GL.MEMNO AND G.ID=GL.GUESTCAT AND R.RUSER = PE.NAME AND PE.ROLE=RO.ID " +
                "UNION ALL " +
                " SELECT R.ID AS RECEIPT,R.DATENEW AS RDATE,C.NAME AS CNAME,C.SEARCHKEY AS CKEY,G.NAME AS WNAME,G.RATE  AS BAMOUNT,0 AS TAX,P.TOTAL AS TOTAL,P.PAYMENT AS PAYMENT ,GL.NUM AS NUM,R.RUSER AS RUSER,RO.NAME AS ROLE,GL.NAMES AS GUESTNAME FROM RECEIPTS_ARV R,GUESTCAT G,CUSTOMERS C,PAYMENTS_ARV P ,GUESTLOG GL,BILL B,PEOPLE PE,ROLES RO  WHERE  P.CUSTOMER=C.ID AND R.ID=? AND P.RECEIPT=R.ID AND G.ID=R.DESC_ AND C.ID=GL.MEMNO AND G.ID=GL.GUESTCAT AND GL.RECEIPTNO=? AND R.RUSER = PE.NAME AND PE.ROLE=RO.ID)" +
                "AS RECEIPT  GROUP BY RECEIPT",
                new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.STRING, Datas.STRING}), new SerializerReadClass(ReceiptGuest.class)).list(new Object[]{id, id, id});
        if (li == null) {
...
//    }
    public ReceiptDetailModel loadInstancemember(AppView app, String id) throws BasicException {
        ReceiptDetailModel rm = new ReceiptDetailModel();
        List<ReceiptMember> li = new StaticSentence(app.getSession(), "SELECT R.ID AS RECEIPTS ,R.DATENEW AS DATENEW,C.NAME AS CNAME,C.SEARCHKEY AS CKEY,F.NAME AS FACILITY,A.AMOUNT AS AMOUNT ,P.TOTAL AS TOTAL,A.NARRATION AS NARRATION,P.PAYMENT AS PAYMENTMODE,A.CREATEDBY AS CREATEDBY,A.COUNTER AS COUNTER,RO.NAME AS ROLE,A.PAYMENTREF AS PAYREF FROM " +
                "CUSTOMERS C,PEOPLE PO,ROLES RO,PAYMENTS P,RECEIPTS R,ACCOUNTJOURNAL A JOIN FACILITY F ON A.TRANSREF=F.ID  WHERE R.ID =? AND P.RECEIPT = R.ID AND  RO.ID = PO.ROLE AND PO.NAME = A.CREATEDBY AND P.CUSTOMER=C.ID AND A.PAYMENTREF LIKE '%' ? ' # %'",
        ...
        if (li == null) {
            ...
            ReceiptMember r = (ReceiptMember) new StaticSentence(app.getSession(), "SELECT NULL AS RECEIPTS ,NULL AS DATENEW,NULL  AS CNAME,NULL  AS CKEY,NULL  AS FACILITY,A.AMOUNT  AS AMOUNT,P.TOTAL AS TOTAL,NULL  AS NARRATION,P.PAYMENT AS PAYMENTMODE,NULL  AS CREATEDBY,NULL  AS COUNTER,NULL  AS ROLE,NULL  AS PAYREF FROM " +
                    "CUSTOMERS C,PEOPLE PO,ROLES RO,PAYMENTS P,RECEIPTS R,ACCOUNTJOURNAL A JOIN FACILITY F ON A.TRANSREF=F.ID  WHERE R.ID =? AND P.RECEIPT = R.ID AND  RO.ID = PO.ROLE AND PO.NAME = A.CREATEDBY AND P.CUSTOMER=C.ID AND A.PAYMENTREF LIKE '%' ? ' # %'", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.STRING}), new SerializerReadClass(ReceiptMember.class)).find(new Object[]{id, id});
            if (r == null) {
