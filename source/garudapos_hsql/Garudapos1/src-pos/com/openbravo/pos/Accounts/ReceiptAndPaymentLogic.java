  ...
  // private Map<String ,Account> mapdetail;
   ...
   private final static String[] HEADERS = {"Account","Amount"};
    ...
    public void getOpeningAndClosingBalance() throws BasicException{
      OBacclist=new PreparedSentence(m_App.getSession(), "SELECT AM.NAME,AM.ID,AM.SEARCHKEY,AM.PARENT,AM.LEVEL,AM.OPERANDS,COALESCE(SUM(AJ.DEBIT+AJ.CURDEBIT),0.0),COALESCE(SUM(AJ.CREDIT+AJ.CURCREDIT),0.0) FROM ACCOUNTMASTER Am JOIN AJPERIODTOTALS AJ ON AM.ID=AJ.ACCOUNTID WHERE (AM.PARENT='1.1.1.2' OR AM.parent='1.1.2') AND AJ.EDATE<? GROUP BY AM.NAME,AM.ID,AM.SEARCHKEY,AM.PARENT,AM.LEVEL,AM.OPERANDS ORDER BY AM.SEARCHKEY"
              , new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP}),new SerializerReadClass(Account.class) ).list(new Object[]{sdateincr});
      CLacclist=new PreparedSentence(m_App.getSession(), "SELECT AM.NAME,AM.ID,AM.SEARCHKEY,AM.PARENT,AM.LEVEL,AM.OPERANDS,COALESCE(SUM(AJ.DEBIT+AJ.CURDEBIT),0.0),COALESCE(SUM(AJ.CREDIT+AJ.CURCREDIT),0.0) FROM ACCOUNTMASTER Am LEFT JOIN AJPERIODTOTALS AJ ON AM.ID=AJ.ACCOUNTID  WHERE (AM.PARENT='1.1.1.2' OR AM.parent='1.1.2') AND AJ.EDATE<?   GROUP BY AM.NAME,AM.ID,AM.SEARCHKEY,AM.PARENT,AM.LEVEL,AM.OPERANDS ORDER BY AM.SEARCHKEY"
    ...
    public void getCreditAndDebitList() throws BasicException, SQLException{
        ...
        while(scal.before(fscal) && scal.before(ecal)){
            ...
            if(flag){//if table is present
                ...
                String sent=    "insert into rptable1 (name,accid,amount,type)  select   am.name,am.id,sum(a.amount),a.transtype "+
                   " from  "+name+" a join "+name+" a1 on  a.tid=a1.tid and a1.active=true  and a.id != a1.id  join accountmaster am on am.id=a1.accountid where a.tid in (select a.tid from accountjournal a  join accountmaster am on a.accountid=am.id and  (am.parent='1.1.1.2' or am.parent='1.1.2') where  a.transref!='Contra' and a.transref !='Cash handover' and a.transref !='Cheque handover') and a.active=true and am.parent != '1.1.2' and am.parent !='1.1.1.2' and a.transref != '1' group by am.id,am.name,a.transtype  ";
                if(barsent==null)
                   barsent=" SELECT SUM(AMOUNT) as amt FROM "+name+" where transref='1' and tid in  (select a.tid from accountjournal a  join accountmaster am on a.accountid=am.id and  (am.parent='1.1.1.2' or am.parent='1.1.2') where  a.transref!='Contra' and a.transref !='Cash handover' and a.transref !='Cheque handover') and active=true  ";
                else
                    barsent+=" union all SELECT SUM(AMOUNT) as amt FROM "+name+" where transref='1' and tid in  (select a.tid from accountjournal a  join accountmaster am on a.accountid=am.id and  (am.parent='1.1.1.2' or am.parent='1.1.2') where  a.transref!='Contra' and a.transref !='Cash handover' and a.transref !='Cheque handover') and active=true ";
                sentList.add(sent);
        ...
        if(!ecal.after(fscal)){//report end date is before or equals financial year start date
            ...
            //report end date is between financial year start date and financial years end date
             String sent=    "insert into rptable1 (name,accid,amount,type)  select   am.name,am.id,sum(a.amount),a.transtype "+
                   " from  accountjournal a   join accountmaster am on am.id=a.accountid  where   a.tid in (select a.tid from accountjournal a  join accountmaster am on a.accountid=am.id and  (am.parent='1.1.1.2' or am.parent='1.1.2') where a.date>=? and a.date< ? and a.transref!='Contra' and a.transref !='Cash handover' and a.transref !='Cheque handover') and a.active=true and am.parent != '1.1.2' and am.parent !='1.1.1.2' and a.transref != '1' group by am.id,am.name,a.transtype  ";
          ...
          // values.add(d);
           ...
           while(fecal.before(ecal) || fecal.equals(ecal)){
            ...
            boolean flag=list.contains(name);
            if(flag){
                 String sent1=    "insert into rptable1 (name,accid,amount,type)  select   am.name,am.id,sum(a.amount),a.transtype "+
                   " from  "+name+" a   join accountmaster am on am.id=a.accountid  where a.tid in (select a.tid from accountjournal a  join accountmaster am on a.accountid=am.id and  (am.parent='1.1.1.2' or am.parent='1.1.2') where  a.transref!='Contra' and a.transref !='Cash handover' and a.transref !='Cheque handover') and a.active=true and am.parent != '1.1.2' and am.parent !='1.1.1.2' and a.transref != '1' group by am.id,am.name,a.transtype  ";
            
                 sentList.add(sent1);
                 sentListarg.add(new Object[]{"acc"});
                 if(barsent==null)
                    barsent=" SELECT SUM(AMOUNT) as amt FROM accountjournal where  transref='1' and tid in  (select a.tid from accountjournal a  join accountmaster am on a.accountid=am.id and  (am.parent='1.1.1.2' or am.parent='1.1.2') where  a.transref!='Contra' and a.transref !='Cash handover' and a.transref !='Cheque handover') and active=true ";
                else
                    barsent+=" union all SELECT SUM(AMOUNT) as amt FROM accountjournal   where transref='1' and tid in  (select a.tid from accountjournal a  join accountmaster am on a.accountid=am.id and  (am.parent='1.1.1.2' or am.parent='1.1.2') where  a.transref!='Contra' and a.transref !='Cash handover' and a.transref !='Cheque handover') and active=true ";
            ...
            Date d=new Date();
                 ...
                 while(!ecal.before(fecal)){
                      String name="AJ_"+fecal.getDisplayName(Calendar.MONTH,Calendar.SHORT, Locale.UK).toUpperCase()+fecal.get(Calendar.YEAR);
                       boolean flag=list.contains(name);
                  if(flag){
                       sent=    "insert into rptable1 (name,accid,amount,type) select   am.name,am.id,sum(a.amount),a.transtype "+
                   " from  "+name+" a   join accountmaster am on am.id=a.accountid  where  a.tid in (select a.tid from accountjournal a  join accountmaster am on a.accountid=am.id and  (am.parent='1.1.1.2' or am.parent='1.1.2') where  a.transref!='Contra' and a.transref !='Cash handover' and a.transref !='Cheque handover') and a.active=true and am.parent != '1.1.2' and am.parent !='1.1.1.2' and a.transref != '1' group by am.id,am.name,a.transtype  ";
                        sentList.add(sent);
                        sentListarg.add(new Object[]{"acc"});
                        if(barsent==null)
        ...
        acclist=new PreparedSentence(m_App.getSession(), "SELECT AM.ID,AM.NAME,AM.SEARCHKEY,AM.PARENT FROM ACCOUNTMASTER AM WHERE AM.PARENT='1.1.1.2' OR AM.PARENT='1.2.1'"
      ...
      //  for(AccountMaster accm:acclist){
        ...
        Object[] value=(Object[])new PreparedSentence(m_App.getSession(),  "select sum(amt) from ( "+ barsent +" )" , new SerializerWriteBasic(datas)
                ,new SerializerReadBasic(new Datas[]{Datas.DOUBLE}) ).find(obj1);
         if(value!=null && value[0]!=null){
           new PreparedSentence(m_App.getSession(), "INSERT INTO RPTABLE1 (NAME,ACCID,AMOUNT,TYPE) VALUES (?,?,?,?)"
       ...
       // }
        ...
        //select a.name,sum(r.amount),sum(r1.amount) from rptable1 r right outer join accountmaster a on a.id=r.accid and r.type='D' join rptable1 r1 on r1.accid=a.id and r1.type='C'  group by a.name
        dcAcclist=new PreparedSentence(m_App.getSession(), "SELECT AM.NAME,AM.ID,AM.SEARCHKEY,AM.PARENT,AM.LEVEL,AM.OPERANDS,COALESCE(SUM(r.AMOUNT),0.0),COALESCE(SUM(r1.AMOUNT),0.0) from rptable1 r right outer join accountmaster am on am.id=r.accid and r.type='D' left outer join rptable1 r1 on r1.accid=am.id and r1.type='C' GROUP BY AM.NAME,AM.ID,AM.SEARCHKEY,AM.PARENT,AM.LEVEL,AM.OPERANDS ORDER BY AM.SEARCHKEY"
