   ...
   // private Account CIAccount;
    ...
    private double calculateStock(Date date,boolean status) throws BasicException{
        ...
        List<Object[]> stkDiaryList=new PreparedSentence(m_App.getSession(), " select pid,pdtname,cat,sum(ob) as ob,sum(pur) as pur,sum(res_) as rec_,sum(sales) as sales,sum(out_) as out_,sum(break) as break,sum(insp) as insp,lparent,pcat from ( "+
                                                 " select p.id as pid,p.name as pdtname,c.category as cat,sum(s.units1) as ob,0.0 as pur,0.0 as res_,0.0  as sales,0.0 as out_,0.0 as break,0.0 as insp,l.parent as lparent,c.category as pcat from products p join stockdiary s on p.id=s.product1 and s.reason1=1 and rno='Opening Balance' join pdt_prcat c on p.id=c.id join locations l on c.category=l.id   where datenew < ? group by p.name,pid,cat,lparent,pcat "+
                                                 " union all select p.id as pid,p.name as pdtname,c.category as cat,0.0 as ob,sum(s1.units1) as pur ,0.0 as res_,0.0 as sales,0.0 as out_,0.0 as break,0.0 as insp,l.parent as lparent,c.category as pcat from products p join stockdiary s1 on p.id=s1.product1 and s1.reason1=1 and rno!='Opening Balance' join pdt_prcat c on p.id=c.id  join locations l on c.category=l.id   where datenew < ?  group by p.name,pid,cat,lparent,pcat  "+
                                                 " union all select p.id as pid,p.name as pdtname,c.category as cat,0.0 as ob,0.0 as pur,sum(s1.units1) as res_,0.0 as sales,0.0 as out_,0.0 as break,0.0 as insp,l.parent as lparent,c.category as pcat from products p join stockdiary s1 on p.id=s1.product1 and s1.reason1=4 join pdt_prcat c on p.id=c.id join locations l on c.category=l.id  where datenew < ?  group by p.name,pid,cat,lparent,pcat  "+
                                                 " union all select p.id as pid,p.name as pdtname,c.category as cat,0.0 as ob,0.0 as pur,0.0 as res_,sum(s2.units1) as sales,0.0 as out_,0.0 as break,0.0 as insp,l.parent as lparent,c.category as pcat from products p join stockdiary  s2 on p.id=s2.product1 and s2.reason1=-1 join pdt_prcat c on p.id=c.id join locations l on c.category=l.id  where datenew < ?  group by p.name,pid,cat,lparent,pcat  "+
                                                 " union all select p.id as pid,p.name as pdtname,c.category as cat,0.0 as ob,0.0 as pur,0.0 as res_,0.0 as sales,sum(s2.units ) as out_,0.0 as break,0.0 as insp,l.parent as lparent,c.category as pcat from products p join stockdiary  s2 on p.id=s2.product and s2.reason=-4 join pdt_prcat c on p.id=c.id join locations l on c.category=l.id where datenew < ?  group by p.name,pid,cat,lparent,pcat  "+
                                                 " union all select p.id as pid,p.name as pdtname,c.category as cat,0.0 as ob,0.0 as pur,0.0 as res_,0.0 as sales,0.0 as out_,sum((s2.units1 * -1)) as break,0.0 as insp,l.parent as lparent,c.category as pcat from products p join stockdiary  s2 on p.id=s2.product1 and s2.reason1=-3 join pdt_prcat c on p.id=c.id join locations l on c.category=l.id where datenew < ?  group by p.name,pid,cat,lparent,pcat  "+
                                                 " union all select p.id as pid,p.name as pdtname,c.category as cat,0.0 as ob,0.0 as pur,0.0 as res_,0.0 as sales,0.0 as out_,0.0 as break,sum(s2.units1 ) as insp,l.parent as lparent,c.category as pcat from products p join stockdiary  s2 on p.id=s2.product1 and s2.reason1=5 join pdt_prcat c on p.id=c.id join locations l on c.category=l.id where datenew < ?  group by p.name,p.id,cat,lparent,pcat ) as stk group by pdtname,pid,cat,lparent,pcat order by lparent "
    ...
    private List<Account> CalculateFromTrailBalance(Date start,Date end) throws BasicException{
      ...
      List<Account> list=new PreparedSentence(m_App.getSession()
                                   , "SELECT AM.NAME,AM.ID,AM.SEARCHKEY,AM.PARENT,AM.LEVEL,AM.OPERANDS,COALESCE(SUM(AJ.DEBIT+AJ.CURDEBIT),0.0) AS DEBIT,COALESCE(SUM(AJ.CREDIT+AJ.CURCREDIT),0.0) AS CREDIT FROM ACCOUNTMASTER AM LEFT OUTER JOIN AJPERIODTOTALS AJ ON AM.ID=AJ.ACCOUNTID  AND AJ.EDATE>=? AND AJ.EDATE<=? "
                                   +"  GROUP BY AM.NAME,AM.ID,AM.SEARCHKEY,AM.PARENT,AM.LEVEL,AM.OPERANDS ORDER BY AM.SEARCHKEY"
                                   , new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP,Datas.TIMESTAMP})
