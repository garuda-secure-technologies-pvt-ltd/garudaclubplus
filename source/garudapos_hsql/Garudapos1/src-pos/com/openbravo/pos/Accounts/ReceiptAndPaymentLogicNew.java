  ...
  // private Map<String ,Account> mapdetail;
   ...
   private final static String[] HEADERS = {"Account","Amount"};
    ...
    public void getOpeningAndClosingBalance() throws BasicException{
      OBacclist=new PreparedSentence(m_App.getSession(), "SELECT AM.NAME,AM.ID,AM.SEARCHKEY,AM.PARENT,AM.LEVEL,AM.OPERANDS,SUM(AJ.DEBIT+AJ.CURDEBIT),SUM(AJ.CREDIT+AJ.CURCREDIT) FROM ACCOUNTMASTER Am JOIN AJPERIODTOTALS AJ ON AM.ID=AJ.ACCOUNTID WHERE (AM.PARENT='1.1.1.2' OR AM.parent='1.1.2') AND AJ.EDATE<=? GROUP BY AM.NAME,AM.ID,AM.SEARCHKEY,AM.PARENT,AM.LEVEL,AM.OPERANDS ORDER BY AM.SEARCHKEY"
              , new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP}),new SerializerReadClass(Account.class) ).list(new Object[]{sdate});
      CLacclist=new PreparedSentence(m_App.getSession(), "SELECT AM.NAME,AM.ID,AM.SEARCHKEY,AM.PARENT,AM.LEVEL,AM.OPERANDS,SUM(AJ.DEBIT+AJ.CURDEBIT),SUM(AJ.CREDIT+AJ.CURCREDIT) FROM ACCOUNTMASTER Am LEFT JOIN AJPERIODTOTALS AJ ON AM.ID=AJ.ACCOUNTID WHERE (AM.PARENT='1.1.1.2' OR AM.parent='1.1.2') AND AJ.EDATE<=?  GROUP BY AM.NAME,AM.ID,AM.SEARCHKEY,AM.PARENT,AM.LEVEL,AM.OPERANDS ORDER BY AM.SEARCHKEY"
    ...
    public void getCreditAndDebitList() throws BasicException{
      dcAcclist=new PreparedSentence(m_App.getSession(), "SELECT AM.NAME,AM.ID,AM.SEARCHKEY,AM.PARENT,AM.LEVEL,AM.OPERANDS,SUM(AJ.DEBIT+AJ.CURDEBIT),SUM(AJ.CREDIT+AJ.CURCREDIT) FROM ACCOUNTMASTER Am JOIN AJPERIODTOTALS AJ ON AM.ID=AJ.ACCOUNTID WHERE   AJ.EDATE>? AND AJ.EDATE<=? AND (AM.SIGN='C' OR AM.SIGN='D' OR AM.SIGN='E' OR AM.SUMMARY=FALSE) GROUP BY AM.NAME,AM.ID,AM.SEARCHKEY,AM.PARENT,AM.LEVEL,AM.OPERANDS ORDER BY AM.SEARCHKEY"
