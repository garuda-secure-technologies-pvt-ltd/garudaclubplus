...
public class AccEntryConfTableModel {
     ...
     public static AccEntryConfTableModel loadInstance(AppView app) throws BasicException{
         ...
         if(chdetail==null){
              ...
              mc.entrylist=chdetail;
         List<AccEntryConfTableModel.RejectedLine> rejectedList = new StaticSentence(app.getSession(),
                " SELECT A1.TID,A1.TRANSREF,A1.DATE, A2.MESSAGE,A.ID FROM ACCOUNTJOURNALDUP A1,ACCOUNTEDITDETAIL A join ACCOUNTEDITDETAIL A2 ON A2.TID=A1.TID AND A2.MESSAGE IS NOT NULL  WHERE A.TID=A1.TID AND A1.ACTIVE=FALSE AND A.CONFIRMER=A1.ACCOUNTID AND A.FLAG IS NULL AND (A1.ACCOUNTID=? OR A1.ACCOUNTID=?) " +
                "UNION SELECT A1.TID,A1.TRANSREF,A1.DATE,A2.MESSAGE  ,A.ID FROM ACCOUNTJOURNALDUP A1,ACCOUNTEDITDETAIL A join ACCOUNTEDITDETAIL A2 ON A2.TID=A1.TID AND A2.MESSAGE IS NOT NULL  WHERE A.TID=A1.TID AND A1.ACTIVE=FALSE AND A.CONFIRMER=? AND A.FLAG IS NULL "
                , new SerializerWriteBasic(new Datas[]{Datas.STRING,Datas.STRING,Datas.STRING})
