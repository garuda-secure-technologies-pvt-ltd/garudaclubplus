...
public class PaymentsModel {
  ...
  //  private Double m_dSalesTaxes;
    ...
    private java.util.List<CounterTotals> clist;
    private java.util.List<CounterTotals1> clist1;
   ...
   //     p.paymentTotal1=new Double(0.0);
        ...
        p.clist=new ArrayList<CounterTotals>();
        p.clist1=new ArrayList<CounterTotals1>();
    ...
    public static PaymentsModel loadInstance(AppView app,Date date) throws BasicException {
     ...
     //   p.m_dDateStart = app.getAppUserView().getUser().getOpenCashTime();
        p.m_dDateEnd = null;
       // p.m_iSeq=app.getActiveCashSequence();
        Object[] seq=(Object[])new PreparedSentence(app.getSession()
                , "SELECT MAX(HOSTSEQUENCE) FROM CLOSEDCASH WHERE USER_ = ?"// chande: USER -> USER_
                , SerializerWriteString.INSTANCE
                , new SerializerReadBasic(new Datas[]{Datas.INT})).find(app.getAppUserView().getUser().getId());
        if(seq!=null){
            if(seq[0]!=null){
        ...
        List<PaymentsModel.PaymentsLine1> l = new StaticSentence(app.getSession()
            ...
            ,new SerializerWriteBasic(new Datas[]{Datas.STRING,Datas.TIMESTAMP})
            , new SerializerReadClass(PaymentsModel.PaymentsLine1.class)).list(new Object[]{app.getAppUserView().getUser().getName(),date}); //new SerializerReadBasic(new Datas[] {Datas.STRING, Datas.DOUBLE}))
        ...
        List<UCReceiptLine> unclosedreceipt=new StaticSentence(app.getSession(),
                "SELECT DISTINCT R.ID,R.DATENEW , SUM(P.TOTAL) FROM RECEIPTS R,PAYMENTS P WHERE R.CLOSECASHSEQ IS NULL AND R.RUSER=? AND R.DATENEW >? AND P.RECEIPT=R.ID GROUP BY R.ID,R.DATENEW"
                ,new SerializerWriteBasic(new Datas[]{Datas.STRING,Datas.TIMESTAMP})
                ,new SerializerReadClass(PaymentsModel.UCReceiptLine.class)).list(new Object[]{app.getAppUserView().getUser().getName(),date});
         ...
         */
         List<ChequeDetailLine> chdetail = new StaticSentence(app.getSession(),
               "SELECT CNO,CBANK,TOTAL,SEARCHKEY FROM (SELECT C.CHEQUENO AS CNO,C.BANK AS CBANK,C.AMOUNT AS TOTAL,CUSTOMERS.SEARCHKEY AS SEARCHKEY  FROM PAYMENTS,CHEQUE C,CUSTOMERS,PEOPLE,RECEIPTS R  "+
                ...
                "WHERE   PAYMENTS.RECEIPT=C.RNO AND PAYMENTS.PAYMENT ='cheque'  AND PAYMENTS.PUSER=? AND PAYMENTS.PUSER = PEOPLE.NAME AND PTIME<=? AND PAYMENTS.CUSTOMER NOT IN (SELECT ID FROM CUSTOMERS)  AND PAYMENTS.RECEIPT=R.ID AND R.CLOSECASHSEQ IS NULL AND PEOPLE.CLOSECASHTIME IS NULL) ORDER BY 4"

                , new SerializerWriteBasic(new Datas[]{Datas.STRING,Datas.TIMESTAMP,Datas.STRING,Datas.TIMESTAMP})
                , new SerializerReadClass(PaymentsModel.ChequeDetailLine.class))
     ...
     //   p.receiptTotal=0.0;
       // PaymentsLine pl=mew PaymentsLine()
        int i=0;
        Object[] obj5=(Object[]) new StaticSentence(app.getSession()
                ,"SELECT SUM(PAYMENTS.TOTAL) FROM PAYMENTS,PEOPLE,RECEIPTS R WHERE PAYMENTS.PAYMENT='cashin'AND PAYMENTS.PUSER = ? AND PAYMENTS.PUSER=PEOPLE.NAME AND  PTIME <=? AND PAYMENTS.RECEIPT=R.ID AND R.CLOSECASHSEQ IS NULL AND PEOPLE.CLOSECASHTIME IS NULL "
                ,new SerializerWriteBasic(new Datas[]{Datas.STRING,Datas.TIMESTAMP})
                ,new SerializerReadBasic(new Datas[] {Datas.DOUBLE})).find(new Object[]{app.getAppUserView().getUser().getName(),date});
        if(obj5==null)
        {
        ...
        List<CounterTotals> ct= new StaticSentence(app.getSession(),
                "SELECT SUM(BILL.AMOUNT + BILL.TAXTOTAL),ROLES.NAME"+
                " FROM RECEIPTS,BILL,PEOPLE P,PEOPLE P1,ROLES "+
                " WHERE BILL.RECEIPT=RECEIPTS.ID AND BILL.CREATEDBY=P.NAME AND P.ROLE=ROLES.ID  AND RECEIPTS.RUSER = ? AND RECEIPTS.RUSER=P1.NAME AND  RECEIPTS.DATENEW <= ? AND RECEIPTS.CLOSECASHSEQ IS NULL AND P1.CLOSECASHTIME IS NULL  " +
               " GROUP BY ROLES.NAME",
                new SerializerWriteBasic(new Datas[]{Datas.STRING,Datas.TIMESTAMP})
       ...
       if(ct==null)
       {
        ...
        List<CounterTotals1> ct1= new StaticSentence(app.getSession(),
                "SELECT SUM(BILL.AMOUNT + BILL.TAXTOTAL),ROLES.NAME"+
                " FROM RECEIPTS,BILL,PEOPLE P,PEOPLE P1,ROLES,PAYMENTS"+
                " WHERE BILL.RECEIPT=RECEIPTS.ID AND BILL.CREATEDBY=P.NAME AND PAYMENTS.RECEIPT=RECEIPTS.ID AND PAYMENTS.PAYMENT='debt'AND P.ROLE=ROLES.ID  AND RECEIPTS.RUSER = ? AND RECEIPTS.RUSER=P1.NAME AND  RECEIPTS.DATENEW <= ? AND RECEIPTS.CLOSECASHSEQ IS NULL AND P1.CLOSECASHTIME IS NULL  " +
               " GROUP BY ROLES.NAME",
                new SerializerWriteBasic(new Datas[]{Datas.STRING,Datas.TIMESTAMP})
                , new SerializerReadClass(PaymentsModel.CounterTotals1.class)).list(new Object[]{app.getAppUserView().getUser().getName(),date});
       ...
       if(ct1==null)
       {
         ...
         //  p.receiptTotal=new Double[]{0.0};
          ...
          p.receiptTotalsum1 += p.clist1.get(j).getamount1();
       }
        //to refund entries
     ...
     /*   Object[] recsales = (Object []) new StaticSentence(app.getSession(),
       ...
       List<PaymentsLine2> asales = new StaticSentence(app.getSession(),
                ...
                "WHERE PAYMENTS.TOTAL < 0 AND PAYMENTS.PAYMENT != 'debtpaid' AND PAYMENTS.PUSER=? AND PAYMENTS.PUSER = PEOPLE.NAME AND RECEIPTS.ID=PAYMENTS.RECEIPT  AND RECEIPTS.CLOSECASHSEQ IS NULL AND PEOPLE.CLOSECASHTIME IS NULL "
                
                , SerializerWriteString.INSTANCE
                , new SerializerReadClass(PaymentsModel.PaymentsLine2.class))
                .list(LookupUtilityImpl.getInstance(null).getAppView().getAppUserView().getUser().getName());
        if (asales == null) {
            p.m_lpayments = new ArrayList<PaymentsLine2>();
        } else {
...
//debt collected total
        ...
        List<RdetailsLine> rdetail=new StaticSentence(app.getSession(),
                ...
                " FROM RECEIPTS,BILL,PEOPLE P,PEOPLE P1,ROLES "+
                " WHERE BILL.RECEIPT=RECEIPTS.ID AND BILL.CREATEDBY=P.NAME AND P.ROLE=ROLES.ID  AND RECEIPTS.RUSER = ? AND RECEIPTS.RUSER=P1.NAME AND  RECEIPTS.DATENEW <=? AND RECEIPTS.CLOSECASHSEQ IS NULL AND P1.CLOSECASHTIME IS NULL  "+
               " GROUP BY RECEIPTS.ID,RECEIPTS.DATENEW,BILL.CUSTOMER,ROLES.NAME,BILL.ID ORDER BY ROLES.NAME,RECEIPTS.ID"
               ,new SerializerWriteBasic(new Datas[]{Datas.STRING,Datas.TIMESTAMP})
               ,new SerializerReadClass(PaymentsModel.RdetailsLine.class))
                .list(new Object[]{app.getAppUserView().getUser().getName(),date});
        if (rdetail == null) {
            p.m_lrdetails = new ArrayList<RdetailsLine>();
        } else {
          ...
          List<RdetailsLine1> rdetail2=new StaticSentence(app.getSession(),
               " SELECT RECEIPTS.DATENEW,RECEIPTS.ID,BILL.ID,CUSTOMERS.SEARCHKEY,SUM(PAYMENTS.TOTAL),ROLES.NAME,CUSTOMERS.ACCOUNT,CUSTOMERS.ID,BILL.CREATEDDATE "+
                " FROM RECEIPTS,BILL,CUSTOMERS,PEOPLE P,PEOPLE P1,ROLES,PAYMENTS "+
                " WHERE BILL.RECEIPT=RECEIPTS.ID AND CUSTOMERS.ID=BILL.CUSTOMER AND PAYMENTS.RECEIPT=RECEIPTS.ID AND PAYMENTS.PAYMENT='debt' AND BILL.CREATEDBY=P.NAME AND P.ROLE=ROLES.ID  AND RECEIPTS.RUSER = ? AND RECEIPTS.RUSER=P1.NAME AND  RECEIPTS.DATENEW <=? AND RECEIPTS.CLOSECASHSEQ IS NULL AND P1.CLOSECASHTIME IS NULL  "+
               " GROUP BY RECEIPTS.ID,RECEIPTS.DATENEW,BILL.CUSTOMER,ROLES.NAME,CUSTOMERS.ACCOUNT,CUSTOMERS.SEARCHKEY,BILL.CREATEDDATE,BILL.ID,CUSTOMERS.ID ORDER BY ROLES.NAME,RECEIPTS.ID "
               ,new SerializerWriteBasic(new Datas[]{Datas.STRING,Datas.TIMESTAMP})
               ,new SerializerReadClass(PaymentsModel.RdetailsLine1.class))
                .list(new Object[]{app.getAppUserView().getUser().getName(),date});
        if (rdetail2 == null) {
            p.m_lrdetails1 = new ArrayList<RdetailsLine1>();
        } else {
          ...
          List<RdetailsLine2> ddetail=new StaticSentence(app.getSession(),
                " SELECT RECEIPTS.DATENEW,RECEIPTS.ID,BILL.ID,CUSTOMERS.SEARCHKEY,SUM(BILL.AMOUNT + BILL.TAXTOTAL),ROLES.NAME "+
                " FROM RECEIPTS,BILL,PEOPLE P,PEOPLE P1,ROLES,PAYMENTS,CUSTOMERS"+
                " WHERE BILL.RECEIPT=RECEIPTS.ID AND CUSTOMERS.ID=BILL.CUSTOMER AND BILL.CREATEDBY=P.NAME AND PAYMENTS.RECEIPT=RECEIPTS.ID AND PAYMENTS.PAYMENT='debt' AND P.ROLE=ROLES.ID  AND RECEIPTS.RUSER = ? AND RECEIPTS.RUSER=P1.NAME AND  RECEIPTS.DATENEW <=? AND RECEIPTS.CLOSECASHSEQ IS NULL AND P1.CLOSECASHTIME IS NULL  "+
               " GROUP BY RECEIPTS.ID,RECEIPTS.DATENEW,CUSTOMERS.SEARCHKEY,ROLES.NAME,BILL.ID ORDER BY ROLES.NAME,RECEIPTS.ID"
               ,new SerializerWriteBasic(new Datas[]{Datas.STRING,Datas.TIMESTAMP})
               ,new SerializerReadClass(PaymentsModel.RdetailsLine2.class))
                .list(new Object[]{app.getAppUserView().getUser().getName(),date});
        if (ddetail == null) {
            p.m_lrdetails2 = new ArrayList<RdetailsLine2>();
        } else {
          ...
          /*List<OtherIncome> rdetail3=new StaticSentence(app.getSession(),
                " SELECT PAYMENTS.PTIME,PAYMENTS.RECEIPT,CUSTOMERS.NAME,PAYMENTS.TOTAL,RECEIPTS.DESC,(SELECT NAME FROM GAMES WHERE (SELECT PARENT  FROM GAMES,RECEIPTS WHERE GAMES.NAME=RECEIPTS.DESC)=GAMES.ID)AS GAMETYPE,CUSTOMERS.ACCOUNT,CUSTOMERS.SEARCHKEY,RECEIPTS.PAYMENTREF FROM PAYMENTS,CUSTOMERS,RECEIPTS,PEOPLE"+
                " WHERE CUSTOMERS.ID=PAYMENTS.CUSTOMER AND PAYMENTS.TOTAL>0 AND PAYMENTS.PUSER = PEOPLE.NAME AND PEOPLE.NAME=RECEIPTS.RUSER AND PEOPLE.NAME= ?  AND RECEIPTS.ID=PAYMENTS.RECEIPT AND PEOPLE.CLOSECASHTIME IS NULL AND RECEIPTS.PAYMENTREF IS NOT NULL AND RECEIPTS.DATENEW <=? AND RECEIPTS.CLOSECASHSEQ IS NULL  "+
                 " UNION ALL SELECT PAYMENTS.PTIME,PAYMENTS.RECEIPT,PAYMENTS.CUSTOMER,PAYMENTS.TOTAL,RECEIPTS.DESC,(SELECT NAME FROM GAMES WHERE (SELECT PARENT  FROM GAMES,RECEIPTS WHERE GAMES.NAME=RECEIPTS.DESC)=GAMES.ID)AS GAMETYPE,RECEIPTS.PAYMENTREF,PAYMENTS.CUSTOMER,RECEIPTS.PAYMENTREF FROM PAYMENTS,RECEIPTS,PEOPLE"+
          ...
          List<OtherIncome> rdetail3=new StaticSentence(app.getSession(),
           ...
           List<OtherIncomeTotalLine> oitotal= new StaticSentence(app.getSession(),
                " SELECT PAYMENTS.TOTAL,RECEIPTS.PAYMENTREF,RECEIPTS.DESC FROM PAYMENTS,RECEIPTS,PEOPLE "+
                " WHERE  PAYMENTS.TOTAL>0 AND PAYMENTS.PUSER = PEOPLE.NAME AND PEOPLE.NAME=RECEIPTS.RUSER AND PEOPLE.NAME =?  AND RECEIPTS.ID=PAYMENTS.RECEIPT   AND  PEOPLE.CLOSECASHTIME IS NULL AND RECEIPTS.PAYMENTREF IS NOT NULL AND RECEIPTS.PAYMENTREF != 'Cards Room' AND RECEIPTS.PAYMENTREF != 'Guest Entry' AND RECEIPTS.DATENEW <=? AND RECEIPTS.CLOSECASHSEQ IS NULL   "
               , new SerializerWriteBasic(new Datas[]{Datas.STRING,Datas.TIMESTAMP})
                ,new SerializerReadClass(OtherIncomeTotalLine.class)).list(new Object[]{app.getAppUserView().getUser().getName(),date});
         ...
         List<Debtcollected> rdetail1=new StaticSentence(app.getSession(),
                ...
                " WHERE CUSTOMERS.ID=PAYMENTS.CUSTOMER AND PAYMENTS.TOTAL>0 AND PAYMENTS.PUSER = PEOPLE.NAME AND PEOPLE.NAME=RECEIPTS.RUSER AND PEOPLE.NAME= ?  AND RECEIPTS.ID=PAYMENTS.RECEIPT   AND  PEOPLE.CLOSECASHTIME IS NULL AND RECEIPTS.PAYMENTREF IS NULL AND PAYMENTS.RECEIPT NOT IN (SELECT BILL.RECEIPT FROM BILL) AND RECEIPTS.DATENEW <=? AND RECEIPTS.CLOSECASHSEQ IS NULL ORDER BY RECEIPTS.ID  "
               
               ,new SerializerWriteBasic(new Datas[]{Datas.STRING,Datas.TIMESTAMP})
               ,new SerializerReadClass(PaymentsModel.Debtcollected.class))
                .list(new Object[]{LookupUtilityImpl.getInstance(null).getAppView().getAppUserView().getUser().getName(),date});
         if (rdetail1 == null) {
            p.drdetails = new ArrayList<Debtcollected>();
        } else {
           ...
           if(debtc==null || debtc[0]==null)
            ...
            List<Object[]> tokenStatusWise= new StaticSentence(app.getSession(),
                " SELECT TYPE,SUM(TOTALVALUE) FROM TOKENLOG WHERE CLOSECASHREF IS NULL GROUP BY TYPE"
                //" WHERE   RECEIPTS.CLOSECASHSEQ IS NULL AND RECEIPTS.ID=PAYMENTS.RECEIPT AND PAYMENTS.TOTAL>0 AND PAYMENTS.PUSER = PEOPLE.NAME AND PAYMENTS.PAYMENT='cheque'  AND  PEOPLE.NAME=RECEIPTS.RUSER AND PEOPLE.NAME= ?  AND  PEOPLE.CLOSECASHTIME IS NULL AND RECEIPTS.PAYMENTREF IS NULL AND RECEIPTS.ID NOT IN (SELECT BILL.RECEIPT FROM BILL)  AND RECEIPTS.DATENEW <=?    "
               ,null
                , new SerializerReadBasic(new Datas[] { Datas.INT,Datas.DOUBLE})).list();
           ...
           if(debtr==null || debtr[0]==null)
            ...
            List<CreditConfirmList> crconf=new StaticSentence(app.getSession()
                , "SELECT D.DATENEW,C.NAME,C.SEARCHKEY,W.NAME,D.BILLREF,D.ID,D.RUSER,D.amount,c.id FROM CREDITCONFLIST D JOIN CUSTOMERS C ON D.CUSTOMER=C.ID JOIN WAITER W ON D.WAITER=W.ID ORDER BY D.DATENEW,W.NAME",null
                , new SerializerReadClass(CreditConfirmList.class)).list();
            if(crconf==null){
                p.crlist=new ArrayList<CreditConfirmList>();
            }else{
                p.crlist=crconf;
    ...
    public double getReceivedTokenAmount(){
     ...
     {
         return roundTwoDecimals(clist1.get(k).getamount1());
     }
      public Double getReceiptTotal1(int k)
     {
         return clist1.get(k).getamount1();
     }

      public String printReceiptCounter1(int k)
     {
         return clist1.get(k).printName1();
     }
 ...
 //   }
    ...
    public List<CounterTotals> getCounterlines(){
        return clist;
    }
    public List<CounterTotals1> getCounterlines1(){
        return clist1;
    }
 ...
 /*   public String printSalesTotal() {
    ...
    public AbstractTableModel getPaymentsModel2() {
        return new AbstractTableModel() {
            ...
            public Object getValueAt(int row, int column) {
                ...
                switch (column) {
                case 0: return l.getReceipt();
                case 1: return l.getpaymentType1();
                case 2: return l.getpaymentTotal1();
                default: return null;
                }
            }  
        };
      ...
      public AbstractTableModel PendingCrConfTableModel() {
        return new AbstractTableModel() {
            ...
            public Object getValueAt(int row, int column) {
                ...
                case 1: return l.getSearchkey();
                case 2: return l.getBillref();
                case 3: return roundTwoDecimals(l.getAmount());
                default: return null;
                }
            }
        };
     ...
     public AbstractTableModel getCounterModel() {
        return new AbstractTableModel() {
            ...
            public Object getValueAt(int row, int column) {
                CounterTotals l = clist.get(row);
                switch (column) {
                case 0: return l.printName();
      ...
      public AbstractTableModel getCounterModel1() {
        return new AbstractTableModel() {
            ...
            public Object getValueAt(int row, int column) {
                CounterTotals1 l = clist1.get(row);
                switch (column) {
                case 0: return l.printName1();
                case 1: return l.getamount1();
    ...
    public AbstractTableModel getRdetailsModel() {
        return new AbstractTableModel() {
            ...
            public Object getValueAt(int row, int column) {
                ...
                switch (column) {
                case 0: return l.getrDate();
                case 1: return l.getrId();
                    case 2 : return l.getbillid();
                case 3 : return l.getrCustomer();
                case 4 : return l.getrTotal();
                case 5: return l.getCounter();
                default: return null;
                }
            }
        };
    ...
    public AbstractTableModel getRdetails2Model() {
     ...
     /*  public static class PaymentsLine implements SerializableRead {
      ...
      public AbstractTableModel getDebtraisedModel() {
        return new AbstractTableModel() {
            ...
            public Object getValueAt(int row, int column) {
                ...
                case 2 : return l.getrCustomer();
                case 3 : return l.getrTotal();
      ...
      public static class CounterTotals implements SerializableRead {
          private String name;
    ...
    public AbstractTableModel getPaymentsModel1() {
        return new AbstractTableModel() {
            ...
            public Object getValueAt(int row, int column) {
                ...
                switch (column) {
                case 0: return l.getpaymentType1();
                case 1: return l.getpaymentTotal1();
                default: return null;
                }
            }  
        };
     ...
     public AbstractTableModel getChequeDetailModel() {
        return new AbstractTableModel() {
            ...
            public Object getValueAt(int row, int column) {
                ...
                switch (column) {
                case 0: return l.getchid();
                case 1: return l.getBank();
                case 2: return l.getAmount();
                case 3: return l.getcustomer();
                default: return null;
                }
            }
        };
