...
public class ChequeDetail extends javax.swing.JDialog {
    ...
    public ChequeDetail(java.awt.Frame parent, boolean modal) {
     ...
     protected ChequeDetail(Dialog parent, boolean modal) {
        super(parent, modal);
        initComponents();
    }
     public ChequeDetail(Frame parent,PaymentsModel m_Payments){
         super(parent,true);
     ...
     public ChequeDetail(Dialog parent,PaymentsModel m_Payments){
         super(parent,true);
         m_PaymentsToClose=m_Payments;
         initComponents();
      ...
      public static ChequeDetail getDialog(Component parent,PaymentsModel m_Payments) {
        Window window = getWindow(parent);
        ChequeDetail cd;
        if (window instanceof Frame) {
            cd = new ChequeDetail((Frame) window,m_Payments);
        } else {
            cd = new ChequeDetail((Dialog) window,m_Payments);
        }
        return cd;
    }
               ...
               if(obj!=null && obj[0]!=null ){
                   if(obj[0].toString().equals("Guest Entry")){
                       ...
                       Object[] obj1=(Object[])new StaticSentence(m_App.getSession()
                        ,"SELECT R.PAYMENTREF,R.DESC,P.CUSTOMER FROM  RECEIPTS R JOIN PAYMENTS P ON R.ID=P.RECEIPT  WHERE  R.PAYMENTREF IS NOT NULL AND R.ID=?  "
                        , SerializerWriteString.INSTANCE
