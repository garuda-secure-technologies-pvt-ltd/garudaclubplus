   ...
   /*     m_jVisible.setEnabled(false);
    ...
    private void billCustomerForMinUsage(MinUsageCustomer cust, Minusage cminusage, Date date1, Date dnow) {
        ...
        try {
            ...
            for (BillPeriods bp : bperiods) {
                ...
                double custUsage = dlfac.getCustomerUsage(cust.getCid(), bp.getStartDate(), bp.getEndDate());
        ...
        if ((f.getrperiod() != null && f.getramt() > 0) || f.getjamt() > 0) {
            ...
            if (f.getrperiod() != null && f.getramt() > 0) {
                ...
                if (tramt > 0) {
                    ...
                    new PreparedSentence(m_App.getSession(), "INSERT INTO MEMFACILITYUSAGE(ID, ACTIVE, MEMNO, FACILITYTYPE, SDATE, CREATEDBY, CRDATE,LBILLDATE,NBILLDATE,BILLREF,STATUS) VALUES (?,?,?,?,?,?,?,?,?,?,?)", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.BOOLEAN, Datas.STRING, Datas.STRING, Datas.TIMESTAMP, Datas.STRING, Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.STRING, Datas.INT})).exec(value1);
                } else {
                    Object[] value1 = new Object[]{UUID.randomUUID().toString(), true, cid, fid, d, m_App.getAppUserView().getUser().getName(), new Date(), 0};
                    new PreparedSentence(m_App.getSession(), "INSERT INTO MEMFACILITYUSAGE(ID, ACTIVE, MEMNO, FACILITYTYPE, SDATE, CREATEDBY, CRDATE,STATUS) VALUES (?,?,?,?,?,?,?,?)", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.BOOLEAN, Datas.STRING, Datas.STRING, Datas.TIMESTAMP, Datas.STRING, Datas.TIMESTAMP, Datas.INT})).exec(value1);
                }
            }
            if (f.getjamt() > 0) {
    ...
    private void insertMinimumUsageSpecificToMemType(Date date1, String custid, List<MinimumUsage> minusage, Date lbilldate) throws BasicException {
        for (MinimumUsage m : minusage) {
            Object[] value1 = new Object[]{UUID.randomUUID().toString(), true, custid, m.getMid(), date1, m_App.getAppUserView().getUser().getName(), date1, 0, lbilldate};
    ...
    private void insertSingleMinimumUsageSpecificToMemType(Date date1, String custid, MinimumUsage minusage) throws BasicException {
        ...
        Object[] value1 = new Object[]{UUID.randomUUID().toString(), true, custid, minusage.getMid(), date1, m_App.getAppUserView().getUser().getName(), date1, 0, date1};
        new PreparedSentence(m_App.getSession(), "INSERT INTO MEMMINUSAGE(ID, ACTIVE, MEMNO, USAGETYPE, SDATE, CREATEDBY, CRDATE,STATUS,LBILLDATE) VALUES (?,?,?,?,?,?,?,?,?)", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.BOOLEAN, Datas.STRING, Datas.STRING, Datas.TIMESTAMP, Datas.STRING, Datas.TIMESTAMP, Datas.INT, Datas.TIMESTAMP})).exec(value1);
    ...
    private void UpdateSingleMinimumUsageSpecificToMemType(String custid, MinimumUsage minusage, Date enddate, String newMinRef) throws BasicException {
        ...
        Object[] value1 = new Object[]{enddate, newMinRef, custid, minusage.getMid()};
        new PreparedSentence(m_App.getSession(), "UPDATE MEMMINUSAGE SET EDATE=?,NEWMINUSAGEREF=? WHERE MEMNO=? AND USAGETYPE=?", new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP, Datas.STRING, Datas.STRING, Datas.STRING})).exec(value1);
    ...
    public Object createValue() throws BasicException {
        ...
        try {
            ...
            //sanjeev:checking for null account id and join date
            if (customer[21] == null) {
            ...
            if (m_oId == null || mtype == null) {
                if (memtype.getSelectedIndex() != -1) {
                    ...
                    insertMinimumUsageSpecificToMemType(date1, customer[0].toString(), minusage, (Date) customer[31]);
                //   }
                }
                ...
                if (memtype.getSelectedIndex() != -1 && !mtype.equals(m_memtype.getSelectedKey().toString())) {
                    ...
                    if (!m_memtype.getSelectedKey().toString().equals(mtype)) {
                        ...
                        List<MinimumUsage> minusageOld = dlfac.getMinUsageSpecifictToMemType("%" + mtype + "%");//minusages for OLD memtype
                        ...
                        for (MinimumUsage muinfo : minusageOld) {
                            if (!minusageNew.contains(muinfo)) {
                                ...
                                if (date1.compareTo(muinfo.getLbilldate()) <= 0) {
                                    if (JOptionPane.showConfirmDialog(this, muinfo.getMname() + " is already bill till " + Formats.DATE.formatValue(muinfo.getLbilldate()), "Warining...", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.NO_OPTION) {
                                        return customerold;
                                    }
                            ...
                            /*MinUsageCustomer cust = new MinUsageCustomer();
                            String cid = customer[0].toString();
                            cust.setCid(cid);
                            cust.setSearchkey(customer[2].toString());
                            cust.setName(customer[3].toString());
                            cust.setMobile(customer[24].toString());
                            cust.setAccount(customer[21].toString());

                            String muid = muinfo.getMid();
                            Minusage cminusage = dlfac.getMinUsageById(muid);
                            LastBillDate lbillDate = dlfac.getLastBillDateOfMinUsageCustomer(cminusage.getid(), cust.getCid());
                            cust.setLastBillDate(lbillDate.getDate());

                            if(cust.getLastBillDate().compareTo(dnow)<0)
                            {
                            billCustomerForMinUsage(cust, cminusage, date1, dnow);
                            }*/
                            }
                        }
                        for (MinimumUsage muinfo : minusageNew) {
