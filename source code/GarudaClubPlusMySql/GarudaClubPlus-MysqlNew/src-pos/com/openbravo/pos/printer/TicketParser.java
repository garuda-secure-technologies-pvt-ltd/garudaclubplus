...
public class TicketParser extends DefaultHandler {
    ...
    public  void TicketParserBarcode(String Type , String Position , String code ) throws TicketPrinterException {
        bctype = Type;
        bcposition = Position;
        Code=code;
    }
            ...
            if ("image".equals(qName)){
              ...
              //  bcposition = attributes.getValue("position");
               // Code=attributes.getValue("Code");
                ...
                linelength=parseInt(attributes.getValue("width"));
                m_oOutputPrinter.beginLine(parseInt(attributes.getValue("size"), DevicePrinter.SIZE_0),parseInt(attributes.getValue("width"), DevicePrinter.SIZE_0));
        ...
        case OUTPUT_TICKET:
            if ("image".equals(qName)){
                try {
                   ...
                   // Image bg = BarcodeImage.getBarcode128(text.toString());
                    BufferedImage image = m_system.getResourceAsImage(text.toString());
                  //  BufferedImage image = (BufferedImage) bg;
    ...
    public void characters(char[] ch, int start, int length) throws SAXException {
        StringBuffer texttemp;
        String temp=null;
        int i=0;
        if (text != null) {
            text.append(ch, start, length);
            texttemp=new StringBuffer(text.toString());
            if(linelength!=0 && texttemp.length()>linelength){
                //text=new StringBuffer();
               while(i<texttemp.toString().length()){
                   int max=0;
                   if(i+linelength>texttemp.toString().length())
                       max=texttemp.toString().length();
                   else
                       max=i+linelength;
                   if(temp==null)
                       temp=texttemp.substring(i, max);
                   else{
                       temp=temp+"\r\n";
                       temp=temp+texttemp.substring(i, max);
                   }
