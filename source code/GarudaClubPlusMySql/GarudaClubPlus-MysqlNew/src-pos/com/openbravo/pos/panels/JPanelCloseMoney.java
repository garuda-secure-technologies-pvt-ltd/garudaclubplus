...
public class JPanelCloseMoney extends JPanel implements JPanelView, BeanFactoryApp {
    ...
    private void initComponents() {
        ...
        jButton10.setBounds(580, 140, 120, 23);

        jLabel6.setText("Card Details :");
        jPanel2.add(jLabel6);
        jLabel6.setBounds(460, 140, 110, 14);
        ...
        if (res == JOptionPane.YES_OPTION) {
           ...
           // try {
              ...
              //  m_dlSystem.execInsertCash(
                ...
                if( m_PaymentsToClose.getbalance()-m_PaymentsToClose.getdebtcTotal()-m_PaymentsToClose.getOChequeAmount()-m_PaymentsToClose.getMagCardAmount()    >0){
                 Object[] value=new Object[]{UUID.randomUUID().toString(),tid,null,dNow,"D","Close cash",m_PaymentsToClose.getSequence()+"", m_PaymentsToClose.getbalance()-m_PaymentsToClose.getdebtcTotal()-m_PaymentsToClose.getOChequeAmount()-m_PaymentsToClose.getMagCardAmount()   ,dNow,true,m_App.getAppUserView().getUser().getName(),m_App.getProperties().getHost(),narration,m_App.getAppUserView().getUser().getcashaccount(),0.0,dNow,null,true};
                ...
                if(m_PaymentsToClose.getMagCardAmount()>=0){
                    java.util.List<Object> BankCharges_AccountList2 = new ArrayList<Object>();
                    BankCharges_AccountList2 = m_PaymentsToClose.getMagCardReceiptList();
                    
                    java.util.List<MagCardIncomeClass> MagCardClassList = new ArrayList<MagCardIncomeClass>();
                    
                    for(int i=0;i<BankCharges_AccountList2.size();i++){
                        String ReceiptId = BankCharges_AccountList2.get(i).toString();
                        Object[] obj = (Object[]) new StaticSentence(m_App.getSession(), "SELECT bankid,amount , othercharges , transactionid,accountid, PAYMENTFLAG FROM carddetails where receipt=? ", SerializerWriteString.INSTANCE, new SerializerReadBasic(new Datas[]{Datas.STRING , Datas.DOUBLE , Datas.DOUBLE , Datas.STRING , Datas.STRING , Datas.INT})).find(ReceiptId);
                        if(obj!=null){
                            String BankId = null;
                            Double Amount=0.00;
                            ...
                            if(obj[0]!=null){
                                BankId = obj[0].toString();
                            }
                            if(obj[1]!=null){
                                Amount = Double.parseDouble(obj[1].toString());
                            }
                            if(obj[2]!=null){
                                OtherCharges = Double.parseDouble(obj[2].toString());
                            }
                            if(obj[3]!=null){
                                TransactionID = obj[3].toString();
                            }
                            if(obj[4]!=null){
                                Accountid = obj[4].toString();
                            }
                            if(obj[5]!=null){
                                PaymentFlag = Integer.parseInt(obj[5].toString());
                            }
                            Double MagCardAmt = 0.00;
                            
                            if(PaymentFlag==1){
                                MagCardAmt =   OtherCharges;   // for member payments
                            }
                            else{
                                MagCardAmt = Amount+ OtherCharges;
                            }
                            
                            
                            int magCardFlag=0;
                            if(MagCardClassList.size()<1){
                                MagCardIncomeClass mg = new MagCardIncomeClass();
                                mg.setBankAccountID(Accountid);
                                mg.setTotalBankAmount(MagCardAmt);
                                MagCardClassList.add(mg);
                            }
                            else{
                                for(int r=0;r<MagCardClassList.size();r++){
                                    String AccountIDFst = MagCardClassList.get(r).getBankAccountID();
                                    if(AccountIDFst.equals(Accountid)){
                                        Double Amttemp = MagCardClassList.get(r).getTotalBankAmount();
                                        Amttemp=Amttemp+MagCardAmt;
                                        MagCardIncomeClass mg = new MagCardIncomeClass();
                                        mg.setBankAccountID(AccountIDFst);
                                        mg.setTotalBankAmount(Amttemp);
                                        MagCardClassList.set(r, mg);
                                        magCardFlag=1;
                                        break;
                                    }
                                }
                          ...
                          //  Object[] value1=new Object[]{UUID.randomUUID().toString(),tid,null,dNow,"D","Close cash",m_PaymentsToClose.getSequence()+"", MagCardAmt ,dNow,true,m_App.getAppUserView().getUser().getName(),m_App.getProperties().getHost(),narration,Accountid,0.0,dNow,null,true};
                          //  dlfac.insertintoaccjoutnal2(value1);
                    ...
                    for(int f=0;f<MagCardClassList.size();f++){
                        String BankAccountID = MagCardClassList.get(f).getBankAccountID();
                        Double BankAmount = MagCardClassList.get(f).getTotalBankAmount();
                         Object[] value1=new Object[]{UUID.randomUUID().toString(),tid,null,dNow,"D","Close cash",m_PaymentsToClose.getSequence()+"", BankAmount ,dNow,true,m_App.getAppUserView().getUser().getName(),m_App.getProperties().getHost(),narration,BankAccountID,0.0,dNow,null,true};
                         dlfac.insertintoaccjoutnal2(value1);
...
//                 }
               ...
               /* java.util.List<PaymentsModel.RdetailsLine1> debtr=m_PaymentsToClose.getRdetalLines1();
                ...
                Facility barfac=dlfac.getFacilitybyID("1");
                 ...
                 //added by pratima to remove double entry of same receipt due to the partial payment by cheque and partial payment by cash
                 if(OIncome.size()>1){
                 for(int i=0;i<OIncome.size()-1;i++){
                 for(int j=i+1;j<OIncome.size();j++){
                   if(  OIncome.get(i).getReceipt().equals(OIncome.get(j).getReceipt()))
                   {
                   OIncome.remove(j);
                   }
                 }
                ...
                java.util.List<OtherIncomeTotalLineGroupWise> OtherIncomeClassList = new ArrayList<OtherIncomeTotalLineGroupWise>();
                 for(PaymentsModel.OtherIncomeTotalLine o:OIncome){
                    ...
                    // dlfac.insertintoaccjoutnal2(valuer);
                     ...
                     // ### ADDED BY AAAKASH ____________________________________________________________________________________________________
                     String AccountidX = accs[i];
                     Double AmountX = Double.parseDouble(accs[i+1]);
                     
                     if(OtherIncomeClassList.size()<1){
                            OtherIncomeTotalLineGroupWise otherIncomceClass = new OtherIncomeTotalLineGroupWise();
                            otherIncomceClass.setAccountID(accs[i]);
                            otherIncomceClass.setTotal(Double.parseDouble(accs[i+1]));
                             OtherIncomeClassList.add(otherIncomceClass);
                             
                     }
                     else{
                         int flagAccountx=0;
                         for(int y=0;y<OtherIncomeClassList.size();y++){
                             String AccountIdFst = OtherIncomeClassList.get(y).getAccountID();
                            if(AccountIdFst.equals(AccountidX)){
                             Double Gtotal = OtherIncomeClassList.get(y).getTotal();
                                 Gtotal = Gtotal+AmountX;
                                 OtherIncomeTotalLineGroupWise p = new OtherIncomeTotalLineGroupWise();
                                 p.setAccountID(AccountIdFst);
                                 p.setTotal(Gtotal);
                                 OtherIncomeClassList.set(y, p);
                                 flagAccountx=1;
                                 break;
                             }
                          }
               ...
               for(int k=0;k<OtherIncomeClassList.size();k++){
                   String ActID = OtherIncomeClassList.get(k).getAccountID();
                   Double AmountX = OtherIncomeClassList.get(k).getTotal();
                   
                   Object[] valuer=new Object[]{UUID.randomUUID().toString(),tid,null,dNow,"C","Close Cash",m_PaymentsToClose.getSequence()+"", AmountX,dNow,true,m_App.getAppUserView().getUser().getName(),m_App.getProperties().getHost(),narration+" other income",ActID,0.0,dNow,null,true};
                   dlfac.insertintoaccjoutnal2(valuer);
               }  
                ...
                if(m_PaymentsToClose.getGuestFee()>0){
                    ...
                    for(PaymentsModel.GuestReceiptdetailsLine line:guestDetails){
                        ...
                        if(line.getTaxAmount()>0){
                            Object[] valuer=new Object[]{UUID.randomUUID().toString(),tid,null,dNow,"C","Close cash Guest Fees -" + line.getName(),m_PaymentsToClose.getSequence()+"",( line.getAmount()-line.getTaxAmount()) ,dNow,true,m_App.getAppUserView().getUser().getName(),m_App.getProperties().getHost(),narration+" other income",line.getAccount(),0.0,dNow,null,true};
                            dlfac.insertintoaccjoutnal2(valuer);
                            String TaxCatid= line.getTaxCatID();
                            String TaxAccountID = getTaxAccountByTaxCatID(TaxCatid);
                            
                            Object[] valuer2=new Object[]{UUID.randomUUID().toString(),tid,null,dNow,"C","Close cash Guest Fees -" + line.getName(),m_PaymentsToClose.getSequence()+"",line.getTaxAmount() ,dNow,true,m_App.getAppUserView().getUser().getName(),m_App.getProperties().getHost(),narration+" other income",TaxAccountID,0.0,dNow,null,true};
                            dlfac.insertintoaccjoutnal2(valuer2);
              ...
              new PreparedSentence(m_App.getSession()
               ...
               // new PreparedSentence(m_App.getSession()
                new PreparedSentence(m_App.getSession()
                        ...
                        , new SerializerWriteBasic(new Datas[]{Datas.STRING,Datas.TIMESTAMP,Datas.STRING})).exec(new Object[]{m_PaymentsToClose.printHost()+" : "+m_PaymentsToClose.getSequence(),date,m_App.getAppUserView().getUser().getName()});
    ...
    private void jButton10ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton10ActionPerformed
        ...
        ChequeDetail cd=ChequeDetail.getDialog(this, m_PaymentsToClose);
        try{
            boolean flag=  cd.showDialog(m_PaymentsToClose.getCardDetailModel(),true,"Details Of Card Payments");
        }catch(Exception e){
            e.printStackTrace();
        }
    ...
    // GET TAX ACCOUNT DETAILS 
    
    public String getTaxAccountByTaxCatID(String id){
        String accName = null;
        try{
            if(id!=null){
    ...
    public  class OtherIncomeTotalLineGroupWise {
    ...
    public  class MagCardIncomeClass {
        ...
        public void setTotalBankAmount(Double TotalBankAmount){
