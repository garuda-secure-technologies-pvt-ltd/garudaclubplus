 ...
 * @author adrianromero
 */
public class PaymentsModelstd1 {
    ...
    private java.util.List<CreditConfirmListstd> crlist1;
    ...
    private List<BillInfo> cashPend;
     private List<BillInfostd1> cashPendstd;
     private AppView app;

    private PaymentsModelstd1() {
    }

    @SuppressWarnings("empty-statement")
    public static PaymentsModelstd1 emptyInstance() {

        PaymentsModelstd1 p = new PaymentsModelstd1();
        ...
        p.crlist = new ArrayList<CreditConfirmList>();
         p.crlist1 = new ArrayList<CreditConfirmListstd>();
        ...
        p.cashPend = new ArrayList<BillInfo>();
         p.cashPendstd = new ArrayList<BillInfostd1>();
        
        return p;
    }

    public static PaymentsModelstd1 loadInstance(AppView app, Date date) throws BasicException {
        ...
        List<PaymentsModelstd1.PaymentsLine1> l = new StaticSentence(app.getSession(), "SELECT PAYMENTS.PAYMENT, SUM(PAYMENTS.TOTAL) " +
                ...
                "GROUP BY PAYMENTS.PAYMENT", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.TIMESTAMP}), new SerializerReadClass(PaymentsModelstd1.PaymentsLine1.class)).list(new Object[]{app.getAppUserView().getUser().getName(), date}); //new SerializerReadBasic(new Datas[] {Datas.STRING, Datas.DOUBLE}))
        ...
        if (l == null) {
            ...
            p.m_lsales = l;
            for (PaymentsModelstd1.PaymentsLine1 p1 : l) {
        ...
        List<WarehouseTotals> ct = new StaticSentence(app.getSession(),
                ...
                new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.TIMESTAMP}), new SerializerReadClass(PaymentsModelstd1.WarehouseTotals.class)).list(new Object[]{app.getAppUserView().getUser().getName(), date});
...
//debt collected total
        ...
        List<RdetailsLine> rdetail = new StaticSentence(app.getSession(),
                ...
                " GROUP BY RECEIPTS.ID,RECEIPTS.DATENEW,BILL.CUSTOMER,L.NAME,BILL.ID ORDER BY L.NAME,RECEIPTS.ID", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.TIMESTAMP}), new SerializerReadClass(PaymentsModelstd1.RdetailsLine.class)).list(new Object[]{app.getAppUserView().getUser().getName(), date});
        if (rdetail == null) {
            p.m_lrdetails = new ArrayList<RdetailsLine>();
        } else {
        ...
        List<RdetailsLine1> rdetail2 = new StaticSentence(app.getSession(),
                ...
                ".NAME,CUSTOMERS.ACCOUNT,CUSTOMERS.SEARCHKEY,BILL.CREATEDDATE,BILL.ID,CUSTOMERS.ID ORDER BY L.NAME,RECEIPTS.ID ", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.TIMESTAMP}), new SerializerReadClass(PaymentsModelstd1.RdetailsLine1.class)).list(new Object[]{app.getAppUserView().getUser().getName(), date});
        if (rdetail2 == null) {
            p.m_lrdetails1 = new ArrayList<RdetailsLine1>();
        } else {
        ...
        List<RdetailsLine2> ddetail = new StaticSentence(app.getSession(),
                ...
                " GROUP BY RECEIPTS.ID,RECEIPTS.DATENEW,CUSTOMERS.SEARCHKEY,L.NAME,BILL.ID ORDER BY L.NAME,RECEIPTS.ID", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.TIMESTAMP}), new SerializerReadClass(PaymentsModelstd1.RdetailsLine2.class)).list(new Object[]{app.getAppUserView().getUser().getName(), date});
        if (ddetail == null) {
            p.m_lrdetails2 = new ArrayList<RdetailsLine2>();
        } else {
        ...
        List<OtherIncome> rdetail3 = new StaticSentence(app.getSession(),
                ...
                , new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.TIMESTAMP, Datas.STRING, Datas.TIMESTAMP}), new SerializerReadClass(PaymentsModelstd1.OtherIncome.class)).list(new Object[]{app.getAppUserView().getUser().getName(), date, app.getAppUserView().getUser().getName(), date});
        if (rdetail3 == null) {
            p.otherIncomedetails = new ArrayList<OtherIncome>();
        } else {
...
//        }
        ...
        List<Debtcollected> rdetail1 = new StaticSentence(app.getSession(),
                ...
                " WHERE CUSTOMERS.ID=PAYMENTS.CUSTOMER AND PAYMENTS.TOTAL>0 AND PAYMENTS.PUSER = PEOPLE.NAME AND PEOPLE.NAME=RECEIPTS.RUSER AND PEOPLE.NAME= ?  AND RECEIPTS.ID=PAYMENTS.RECEIPT   AND  PEOPLE.CLOSECASHTIME IS NULL AND RECEIPTS.PAYMENTREF IS NULL AND PAYMENTS.RECEIPT NOT IN (SELECT BILL.RECEIPT FROM BILL where bill.receipt is not null) AND RECEIPTS.DATENEW <=? AND RECEIPTS.CLOSECASHSEQ IS NULL ORDER BY RECEIPTS.ID  ", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.TIMESTAMP}), new SerializerReadClass(PaymentsModelstd1.Debtcollected.class)).list(new Object[]{LookupUtilityImpl.getInstance(null).getAppView().getAppUserView().getUser().getName(), date});
        if (rdetail1 == null) {
            p.drdetails = new ArrayList<Debtcollected>();
        } else {
        ...
        if (ticketBag.equals("restaurant")) {
            ...
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
              ...
              List<CreditConfirmListstd> crconf = new StaticSentence(app.getSession(),"SELECT D.DATENEW,C.NAME,C.SEARCHKEY,D.WAITER,D.BILLREF,D.ID,D.RUSER,D.amount,c.id FROM CREDITCONFLIST D JOIN CUSTOMERS C ON D.CUSTOMER=C.ID join bill b on b.id=d.billref and b.warehouse in(" + condition.toString() + ") group by d.id ORDER BY D.DATENEW", new SerializerWriteBasic(data), new SerializerReadClass(CreditConfirmListstd.class)).list(params);
            if (crconf == null) {
                p.crlist1 = new ArrayList<CreditConfirmListstd>();
            } else {
                p.crlist1 = crconf;
    ...
    public List<CreditConfirmListstd> getCreditConfListstd() {
        return crlist1;
    }
    ...
    public List<BillInfostd1> getPendeingCashBillsstd() {
        return cashPendstd;
    }
    ...
    public AbstractTableModel PendingCrConfTableModelstd() {
            ...
            public Object getValueAt(int row, int column) {
                CreditConfirmListstd l = crlist1.get(row);
                switch (column) {
                    case 0:
                        return Formats.DATE.formatValue(l.getDate());
                    case 1:
