...
//        jTextField3.setText(null);
    ...
    private void printSales() throws BasicException {
        ...
        if (sresource == null) {
            ...
            try {
                ...
                //  }
                //      temp=2;
                int CloseSaleSeq = m_salesmodel.getSequence();
    ...
    private void m_jCloseCashActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_m_jCloseCashActionPerformed
       ...
       if(flag!=m_salesmodel.getSequence() && flag!=0){
...
//                                }
                               ...
                               // JOptionPane.showMessageDialog(null, "Press Ok Once the printing is complete", AppLocal.getIntString("message.title"), JOptionPane.INFORMATION_MESSAGE);
                                ...
                                // ***************************************************** EDITED TO ADD NOTIFICATION ************************************
                                
                                 //get product details
                                String prod_details=null;
                                StringBuilder sb=new StringBuilder();
                                if(m_salesmodel!=null){
                                int rows=m_jsalestable.getModel().getRowCount();
                                for(int ir=0;ir<rows;ir++){
                                    String p_name=(String) m_jsalestable.getModel().getValueAt(ir, 0);
                                    int p_qty=(Integer) m_jsalestable.getModel().getValueAt(ir, 1);
                                    double p_rate=(Double) m_jsalestable.getModel().getValueAt(ir, 2);
                                    double p_total=(Double) m_jsalestable.getModel().getValueAt(ir, 3);
                                    sb.append(p_name+"#"+p_qty+"#"+p_rate+"#"+p_total+"@");
                                    
                                    System.out.println("Product details: "+p_name+p_qty+p_rate+p_total);
                                    
                                }
                                }
                                            prod_details=sb.toString();         
                                            String saletotal=jTextField3.getText().toString();
                                            System.out.println("sale total is"+saletotal);


                                            // to get sequence number
                                       String sequence=m_jSequence.getText().toString();
                                      ...
                                      // String  startTime = "YYYY-MM-dd HH:mm:ss";
                                       ...
                                       // Date dt;
                                        //  try {    
                                       //       dt = sdf.parse(time);
                                       //       System.out.println("Time Display: " + sdfs.format(dNow)); // <-- I got result here
                                       //   } catch (ParseException e) {
                                      ...
                                      // to get warehouse
                                                String warehouse_id=m_App.getAppUserView().getUser().getWarehouse();
                                       String warehouse_name=(String) new StaticSentence(m_App.getSession()
                                     ,"SELECT NAME FROM locations WHERE ID=?" ,SerializerWriteString.INSTANCE
                                     ,SerializerReadString.INSTANCE).find(warehouse_id);
                                      System.out.println("warehouse is"+warehouse_name);
                                       ...
                                       String amt= m_jSalesSubtotal.getText().toString();

                                       //to fetch tax

                                         //String tax= m_jSalesTaxes.getText().toString();
                                        String tax  = m_salesmodel.getTaxtotal()+"";   // edited as it was adding currency



                                 String lst=null;
                                 lst=  (String) new StaticSentence(m_App.getSession()
                                     ,"SELECT mobileno FROM contact WHERE description=?" ,SerializerWriteString.INSTANCE
                                     ,SerializerReadString.INSTANCE).find("close_sale");
                                      System.out.println(lst);

                                      
                                 String SMSPermission=null;
                                 SMSPermission=  (String) new StaticSentence(m_App.getSession()
                                     ,"SELECT VALUE FROM generaltable WHERE name= ?" ,SerializerWriteString.INSTANCE
                                     ,SerializerReadString.INSTANCE).find("SMS notification permission");
                                      
                                     if(SMSPermission!=null && SMSPermission.length()>0){    // CHECKING PERMISSION TO  SEND SMS 
                                        if(SMSPermission.equals("1")){ 
                                            String[]  mobile_no= lst.split("#");   
                                            ...
                                            for(int i=0;i<mobile_no.length;i++)
                                             { 
                                                   // AppConfig config = new AppConfig(args);
                                                     //String rs=config.getProperty(format.currency); 
                                                   new StaticSentence(m_App.getSession()
                                                    , "insert into activemsgtable(id,message,sendto,flag) values(?,?,?,?)"
                                                    , new SerializerWriteBasic(new Datas[] {Datas.STRING, Datas.STRING,Datas.STRING,Datas.INT}))
                                                    .exec(new Object[] {UUID.randomUUID().toString(),m_App.getAppUserView().getUser().getName()+", has done close-sale for "+warehouse_name+" from "+ Formats.TIMESTAMP.formatValue(m_salesmodel.getDateStart())+" to "+Formats.TIMESTAMP.formatValue(dNow)+" Total amount is  "+saletotal, mobile_no[i] , 0});
                                             }
                                        }
                                     }
                                        

                                       // to update notification table  

                                            String[]  mobile_no1= lst.split("#");   
                                        for(int i=0;i<mobile_no1.length;i++)
                                                {
                                                    System.out.println(mobile_no1[i]);
                                                }
                                        
                                        String clubname= (String) new StaticSentence(m_App.getSession()
                                                            ,"SELECT name FROM garudaconame.company WHERE descr=?" ,SerializerWriteString.INSTANCE
                                                            ,SerializerReadString.INSTANCE).find(m_App.getSession().getCompanyName());

                                         for(int i=0;i<mobile_no1.length;i++)
                                          { 
                                              String number=mobile_no1[i];
                                              String regid= (String) new StaticSentence(m_App.getSession()
                                                            ,"SELECT reg_id FROM reg_id WHERE mobile_no=?" ,SerializerWriteString.INSTANCE
                                                            ,SerializerReadString.INSTANCE).find(number);


                                              
                                              
                                           new StaticSentence(m_App.getSession()
                                            , "insert into notification(id,message,mobileno,flag,product_details) values(?,?,?,?,?)"
                                            , new SerializerWriteBasic(new Datas[] {Datas.STRING, Datas.STRING,Datas.STRING,Datas.INT,Datas.STRING}))
                                            .exec(new Object[] {UUID.randomUUID().toString(),clubname+" : "+m_App.getAppUserView().getUser().getName()+"  has done close-sale for "+warehouse_name+" from "+ Formats.TIMESTAMP.formatValue(m_salesmodel.getDateStart())+" to "+Formats.TIMESTAMP.formatValue(dNow)+" Total amount is  "+saletotal+"&"+tax, mobile_no1[i] , 0 ,prod_details});
                                          }
            ...
            JOptionPane.showMessageDialog(this, "Sequence number already present.", "Warning", JOptionPane.WARNING_MESSAGE);
       }
    ...
    // End of variables declaration//GEN-END:variables
      ...
      public int  getCurrMaxSequence() throws ParseException , BasicException{
      
       int opass = 0;
       Object o = null;
      
       o =(Object) new PreparedSentence(m_App.getSession(), "SELECT MAX(SEQUENCE) FROM CLOSEDSALE WHERE ROLE = ? ", SerializerWriteString.INSTANCE ,SerializerReadString.INSTANCE).find(m_App.getAppUserView().getUser().getRole());
       if(o!=null){
