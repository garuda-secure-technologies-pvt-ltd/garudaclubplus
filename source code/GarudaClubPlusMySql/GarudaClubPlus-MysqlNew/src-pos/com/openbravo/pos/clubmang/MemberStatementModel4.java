 ...
 * @author swathi
 */
public class MemberStatementModel4 {
    ...
    private MemberStatementModel4() {
    }

    public static MemberStatementModel4 emptyinstance() {
        MemberStatementModel4 d = new MemberStatementModel4();
    ...
    public static MemberStatementModel4 loadInstance(AppView app, String ID, String accid, DataLogicFacilities dlfac, Date fromdate, Date todate,String searchkey) throws BasicException {
        MemberStatementModel4 d = new MemberStatementModel4();
        d.dlfac = dlfac;
        Date sdate = new Date();
        try {
        ...
        //this is for a month details
         dlist = new StaticSentence(app.getSession()//Added to replace above commented qry
                ...
                " FROM CUSTOMERS C,ACCOUNTJOURNAL A JOIN FACILITY F ON  F.ID=A.TRANSREF WHERE  A.ACCOUNTID=? AND A.ACTIVE=TRUE " +
                "  AND A.TRANSTYPE='D' AND A.DATE>=? AND A.DATE<=? AND C.ACCOUNT=A.ACCOUNTID GROUP BY A.ID" +
                " UNION ALL " +
                ...
                "  AND AJ.TRANSTYPE='D' AND AJ.DATE>=? AND AJ.DATE<=? AND AJ.ACTIVE=TRUE AND C.ACCOUNT=AJ.ACCOUNTID GROUP BY AJ.ID " +
                " UNION ALL " +
                " SELECT AJ.TRANSREF AS NAME,AJ.DUEDATE AS DUEDATE,AJ.BALANCEAMOUNT AS AMOUNT,AJ.ID AS AID,AJ.TRANSNO AS TRANSNO," +
                " AJ.ACCOUNTID AS ACCOUNT,AJ.NARRATION AS NARRATION,AJ.DATE AS DATE,AJ.TRANSREF AS FID,AJ.AMOUNT AS AMT,C.NAME AS MEMNAME,C.SEARCHKEY AS MEMNO,AJ.TRANSTYPE AS TTYPE   " +
                " FROM CUSTOMERS C,AJUNADJUSTED AJ WHERE  AJ.ACCOUNTID=?    " +
                " AND AJ.TRANSTYPE='D' AND AJ.DATE>=? AND AJ.DATE<=? AND AJ.ACTIVE=TRUE AND AJ.TRANSREF NOT IN (SELECT ID FROM FACILITY) AND C.ACCOUNT=AJ.ACCOUNTID GROUP BY AJ.ID " + " ) AS COLLTRANS ORDER BY 8,12 ",
                ...
                new SerializerReadClass(MemberStatementModel2.Facilityline.class)).list(new Object[]{accid, fromdate, todate,accid, fromdate, todate, accid, fromdate, todate, accid, fromdate, todate, accid, fromdate, todate});
        if (dlist == null) {
            d.fac = new ArrayList<Facilityline>();
        } else {
            ...
            if(dlist.size()==0){
               ...
               dlist = new StaticSentence(app.getSession()//Added to replace above commented qry
                ...
                new SerializerReadClass(MemberStatementModel2.Facilityline.class)).list(new Object[]{accid});
        ...
        double crtemp = 0, drtemp = 0, temp = 0;
          ...
          if(dlist.size()!=0){
           ...
           List<amountline> previousDues = new StaticSentence(app.getSession(),
                "SELECT SUM(AMT1),SUM(AMT2),SUM(AMT3),SKEY FROM (SELECT SUM(A.DEBIT-A.CREDIT) AS AMT1,0.0 AS AMT2,0.0 AS AMT3,C.SEARCHKEY AS SKEY  FROM AJPERIODTOTALS A,CUSTOMERS C WHERE A.EDATE < ? AND A.ACCOUNTID=?  and c.account=a.accountid   GROUP BY C.SEARCHKEY " +
                " UNION ALL " +
                " SELECT 0.0 AS AMT1,SUM(AMOUNT) AS AMT2,0.0 AS AMT3,C.SEARCHKEY AS SKEY  FROM ACCOUNTJOURNAL A,CUSTOMERS C WHERE  A.DATE>=? AND A.DATE < ?  AND A.TRANSTYPE='C' AND A.ACCOUNTID=?  and c.account=a.accountid  AND A.ACTIVE=TRUE GROUP BY C.SEARCHKEY" +
                " UNION ALL " +
                " SELECT 0.0 AS AMT1,0.0 AS AMT2,SUM(AMOUNT) AS AMT3,C.SEARCHKEY AS SKEY  FROM ACCOUNTJOURNAL A,CUSTOMERS C WHERE  A.DATE>=? AND A.DATE < ?  AND A.TRANSTYPE='D' AND A.ACCOUNTID=?  and c.account=a.accountid AND A.ACTIVE=TRUE  GROUP BY C.SEARCHKEY)" +
                " AS FSDA GROUP BY SKEY having SUM(AMT1)!='0.0' and SUM(AMT2)!='0.0' and SUM(AMT3)!='0.0' ",
                new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP,Datas.STRING,Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.STRING, Datas.TIMESTAMP, Datas.TIMESTAMP,Datas.STRING}),
                new SerializerReadClass(MemberStatementModel2.amountline.class)).list(new Object[]{ fsdate,accid, fsdate, sdate, accid, fsdate, sdate, accid});
        ...
        List<addressline> adr = new StaticSentence(app.getSession(),
                ...
                new SerializerReadClass(MemberStatementModel2.addressline.class)).list(accid);
        String addr = null;
      ...
      // FOR MEMBER WHO DON'T HAVE ANY TRASACTION ----- AAKASH    
             ...
             dlist = new StaticSentence(app.getSession()//Added to replace above commented qry
                ...
                new SerializerReadClass(MemberStatementModel2.Facilityline.class)).list(new Object[]{Temp_todate , Temp_todate , accid});
            ...
            d.fac = dlist;
                    ...
                    List<amountline> previousDues = new StaticSentence(app.getSession(),  "SELECT SUM(A.DEBIT) AS AMT1 , SUM(A.CREDIT) AS AMT2, 0.0 AS AMT3  , C.searchkey FROM AJPERIODTOTALS A , customers c WHERE A.EDATE < ? AND A.ACCOUNTID=? and a.accountid = c.account group by a.accountid ",
                            new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP,Datas.STRING }),
                            new SerializerReadClass(MemberStatementModel2.amountline.class)).list(new Object[]{ todate,accid });
    ...
    public static MemberStatementModel4 loadInstance(AppView app, DataLogicFacilities dlfac, Date fromdate, Date todate, String memtype) throws BasicException {
        MemberStatementModel4 d = new MemberStatementModel4();
        d.dlfac = dlfac;
        Date sdate = new Date();
        try {
        ...
        List dlist = new StaticSentence(app.getSession()//Added to replace above commented qry
                ...
                " AND A.TRANSTYPE='D' AND A.DATE>=? AND A.DATE<=? AND C.ACCOUNT=A.ACCOUNTID AND A.ACTIVE=TRUE  GROUP BY A.ID" +
                " UNION ALL " +
                " SELECT AJ.TRANSREF AS NAME,AJ.DUEDATE AS DUEDATE,AJ.BALANCEAMOUNT AS AMOUNT,AJ.ID AS AID,AJ.TRANSNO AS TRANSNO," +
                " AJ.ACCOUNTID AS ACCOUNT,AJ.NARRATION AS NARRATION,AJ.DATE AS DATE,AJ.TRANSREF AS FID,AJ.AMOUNT AS AMT,C.NAME AS MEMNAME,C.SEARCHKEY AS MEMNO,AJ.TRANSTYPE AS TTYPE  " +
                " FROM CUSTOMERS C,ACCOUNTJOURNAL AJ WHERE  C.MEMTYPE=?  " +
                " AND AJ.TRANSTYPE='C' AND AJ.DATE>=? AND AJ.DATE<=?  AND C.ACCOUNT=AJ.ACCOUNTID AND AJ.ACTIVE=TRUE  GROUP BY AJ.ID " +
                " UNION ALL" +
                " SELECT A.TRANSREF AS NAME,A.DUEDATE AS DUEDATE,A.BALANCEAMOUNT AS AMOUNT,A.ID AS AID,A.TRANSNO AS TRANSNO," +
                " A.ACCOUNTID AS ACCOUNT,A.NARRATION AS NARRATION,A.DATE AS DATE,A.TRANSREF AS FID,A.AMOUNT AS AMT,C.NAME AS MEMNAME,C.SEARCHKEY AS MEMNO,A.TRANSTYPE AS TTYPE  " +
                " FROM CUSTOMERS C,ACCOUNTJOURNAL A  WHERE  C.MEMTYPE= ?   " +
                " AND A.TRANSTYPE='D' AND A.DATE>=? AND A.DATE<=?  AND A.TRANSREF NOT IN (SELECT ID FROM FACILITY) AND C.ACCOUNT=A.ACCOUNTID AND A.ACTIVE=TRUE GROUP BY A.ID " +
                " UNION ALL " +
                " SELECT F.NAME AS NAME,AJ.DUEDATE AS DUEDATE,AJ.BALANCEAMOUNT AS AMOUNT,AJ.ID AS AID,AJ.TRANSNO AS TRANSNO," +
                " AJ.ACCOUNTID AS ACCOUNT,AJ.NARRATION AS NARRATION,AJ.DATE AS DATE,F.ID AS FID,AJ.AMOUNT AS AMT,C.NAME AS MEMNAME,C.SEARCHKEY AS MEMNO,AJ.TRANSTYPE AS TTYPE  " +
                ...
                " SELECT AJ.TRANSREF AS NAME,AJ.DUEDATE AS DUEDATE,AJ.BALANCEAMOUNT AS AMOUNT,AJ.ID AS AID,AJ.TRANSNO AS TRANSNO," +
                " AJ.ACCOUNTID AS ACCOUNT,AJ.NARRATION AS NARRATION,AJ.DATE AS DATE,AJ.TRANSREF AS FID,AJ.AMOUNT AS AMT,C.NAME AS MEMNAME,C.SEARCHKEY AS MEMNO,AJ.TRANSTYPE AS TTYPE  " +
                " FROM CUSTOMERS C,AJUNADJUSTED AJ WHERE  C.MEMTYPE=?    " +
                " AND AJ.TRANSTYPE='D' AND AJ.DATE>=? AND AJ.DATE<=? AND AJ.ACTIVE=TRUE AND AJ.TRANSREF NOT IN (SELECT ID FROM FACILITY) AND C.ACCOUNT=AJ.ACCOUNTID  GROUP BY AJ.ID " + " ) AS COLLTRANS ORDER BY 12,8", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.STRING, Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.STRING, Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.STRING, Datas.TIMESTAMP, Datas.TIMESTAMP}), new SerializerReadClass(MemberStatementModel2.Facilityline.class)).list(new Object[]{memtype, fromdate, todate, memtype, fromdate, todate, memtype, fromdate, todate, memtype, fromdate, todate});
        if (dlist == null) {
            d.fac = new ArrayList<Facilityline>();
        } else {
            d.fac = dlist;
        ...
        List<amountline> previousDues = new StaticSentence(app.getSession(),
               "SELECT SUM(AMT1),SUM(AMT2),SUM(AMT3),SKEY FROM (SELECT SUM(A.DEBIT-A.CREDIT) AS AMT1,0.0 AS AMT2,0.0 AS AMT3,C.SEARCHKEY AS SKEY  FROM AJPERIODTOTALS A,CUSTOMERS C WHERE A.EDATE < ? AND A.ACCOUNTID=?  and c.account=a.accountid   GROUP BY C.SEARCHKEY " +
                ...
                " AS FSDA GROUP BY SKEY having SUM(AMT1)!='0.0' and SUM(AMT2)!='0.0' and SUM(AMT3)!='0.0' ",
                new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP,Datas.STRING, Datas.TIMESTAMP, Datas.TIMESTAMP,Datas.STRING, Datas.TIMESTAMP, Datas.TIMESTAMP,Datas.STRING}),
                new SerializerReadClass(MemberStatementModel2.amountline.class)).list(new Object[]{ fsdate,memtype, fsdate, sdate,memtype, fsdate, sdate,memtype});
        if (previousDues != null) {
            for (amountline a : previousDues) {
                for (Facilityline f : d.getfacilityline()) {
                    if (a.getSearchKey().equals(f.getMemNo())) {
        ...
        List<addressline> adr = new StaticSentence(app.getSession(),
                ...
                SerializerWriteString.INSTANCE,
                new SerializerReadClass(MemberStatementModel2.addressline.class)).list(memtype);
        String addr = null;
        Map<String, String> adrmap = new HashMap<String, String>();
        if (adr != null) {
            for (addressline a : adr) {
    ...
    public static MemberStatementModel4 loadInstanceforAllMembers(AppView app, DataLogicFacilities dlfac, Date fromdate, Date todate) throws BasicException {
        MemberStatementModel4 d = new MemberStatementModel4();
        d.dlfac = dlfac;
        Date sdate = new Date();
        try {
        ...
        List dlist = new StaticSentence(app.getSession()//Added to replace above commented qry
                ...
                " SELECT F.NAME AS NAME,A.DUEDATE AS DUEDATE,A.BALANCEAMOUNT AS AMOUNT,A.ID AS AID,A.TRANSNO AS TRANSNO," +
                " A.ACCOUNTID AS ACCOUNT,A.NARRATION AS NARRATION,A.DATE AS DATE,F.ID AS FID,A.AMOUNT AS AMT,C.NAME AS MEMNAME,C.SEARCHKEY AS MEMNO,A.TRANSTYPE AS TTYPE " +
                " FROM CUSTOMERS C,ACCOUNTJOURNAL A JOIN FACILITY F ON  F.ID=A.TRANSREF WHERE" +
                "  A.TRANSTYPE='D' AND A.DATE>=? AND A.DATE<=? AND C.ACCOUNT=A.ACCOUNTID  AND A.ACTIVE=TRUE  GROUP BY A.ID" +
                " UNION ALL " +
                " SELECT AJ.TRANSREF AS NAME,AJ.DUEDATE AS DUEDATE,AJ.BALANCEAMOUNT AS AMOUNT,AJ.ID AS AID,AJ.TRANSNO AS TRANSNO," +
                " AJ.ACCOUNTID AS ACCOUNT,AJ.NARRATION AS NARRATION,AJ.DATE AS DATE,AJ.TRANSREF AS FID,AJ.AMOUNT AS AMT,C.NAME AS MEMNAME,C.SEARCHKEY AS MEMNO,AJ.TRANSTYPE AS TTYPE  " +
                " FROM CUSTOMERS C,ACCOUNTJOURNAL AJ WHERE " +
                " AJ.TRANSTYPE='C' AND AJ.DATE>=? AND AJ.DATE<=?  AND C.ACCOUNT=AJ.ACCOUNTID AND AJ.ACTIVE=TRUE GROUP BY AJ.ID " +
                " UNION ALL" +
                " SELECT A.TRANSREF AS NAME,A.DUEDATE AS DUEDATE,A.BALANCEAMOUNT AS AMOUNT,A.ID AS AID,A.TRANSNO AS TRANSNO," +
                " A.ACCOUNTID AS ACCOUNT,A.NARRATION AS NARRATION,A.DATE AS DATE,A.TRANSREF AS FID,A.AMOUNT AS AMT,C.NAME AS MEMNAME,C.SEARCHKEY AS MEMNO,A.TRANSTYPE AS TTYPE  " +
                " FROM CUSTOMERS C,ACCOUNTJOURNAL A  WHERE   " +
                " A.TRANSTYPE='D' AND A.DATE>=? AND A.DATE<=?  AND A.TRANSREF NOT IN (SELECT ID FROM FACILITY) AND C.ACCOUNT=A.ACCOUNTID AND A.ACTIVE=TRUE GROUP BY A.ID " +
                " UNION ALL " +
                " SELECT F.NAME AS NAME,AJ.DUEDATE AS DUEDATE,AJ.BALANCEAMOUNT AS AMOUNT,AJ.ID AS AID,AJ.TRANSNO AS TRANSNO," +
                " AJ.ACCOUNTID AS ACCOUNT,AJ.NARRATION AS NARRATION,AJ.DATE AS DATE,F.ID AS FID,AJ.AMOUNT AS AMT,C.NAME AS MEMNAME,C.SEARCHKEY AS MEMNO,AJ.TRANSTYPE AS TTYPE  " +
                " FROM CUSTOMERS C,AJUNADJUSTED AJ JOIN FACILITY F ON  F.ID=AJ.TRANSREF WHERE " +
                " AJ.TRANSTYPE='D' AND AJ.DATE>=? AND AJ.DATE<=?  AND AJ.ACTIVE=TRUE AND C.ACCOUNT=AJ.ACCOUNTID GROUP BY AJ.ID " +
                " UNION ALL " +
                " SELECT AJ.TRANSREF AS NAME,AJ.DUEDATE AS DUEDATE,AJ.BALANCEAMOUNT AS AMOUNT,AJ.ID AS AID,AJ.TRANSNO AS TRANSNO," +
                " AJ.ACCOUNTID AS ACCOUNT,AJ.NARRATION AS NARRATION,AJ.DATE AS DATE,AJ.TRANSREF AS FID,AJ.AMOUNT AS AMT,C.NAME AS MEMNAME,C.SEARCHKEY AS MEMNO,AJ.TRANSTYPE AS TTYPE  " +
                " FROM CUSTOMERS C,AJUNADJUSTED AJ WHERE  " +
                " AJ.TRANSTYPE='D' AND AJ.DATE>=? AND AJ.DATE<=? AND AJ.ACTIVE=TRUE  AND AJ.TRANSREF NOT IN (SELECT ID FROM FACILITY) AND C.ACCOUNT=AJ.ACCOUNTID GROUP BY AJ.ID " + " ) AS COLLTRANS ORDER BY 12,8", new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.TIMESTAMP}), new SerializerReadClass(MemberStatementModel2.Facilityline.class)).list(new Object[]{fromdate, todate, fromdate, todate, fromdate, todate, fromdate, todate});
        if (dlist == null) {
            d.fac = new ArrayList<Facilityline>();
        } else {
            d.fac = dlist;
        ...
        List<amountline> previousDues = new StaticSentence(app.getSession(),
                "SELECT SUM(AMT1),SUM(AMT2),SUM(AMT3),SKEY FROM (SELECT SUM(A.DEBIT-A.CREDIT) AS AMT1,0.0 AS AMT2,0.0 AS AMT3,C.SEARCHKEY AS SKEY  FROM AJPERIODTOTALS A,CUSTOMERS C WHERE A.EDATE < ? AND A.ACCOUNTID=?  and c.account=a.accountid   GROUP BY C.SEARCHKEY " +
                " UNION ALL " +
                " SELECT 0.0 AS AMT1,SUM(AMOUNT) AS AMT2,0.0 AS AMT3,C.SEARCHKEY AS SKEY  FROM ACCOUNTJOURNAL A,CUSTOMERS C WHERE  A.DATE>=? AND A.DATE < ?  AND A.TRANSTYPE='C' AND A.ACCOUNTID=?  and c.account=a.accountid   GROUP BY C.SEARCHKEY" +
                " UNION ALL " +
                " SELECT 0.0 AS AMT1,0.0 AS AMT2,SUM(AMOUNT) AS AMT3,C.SEARCHKEY AS SKEY  FROM ACCOUNTJOURNAL A,CUSTOMERS C WHERE  A.DATE>=? AND A.DATE < ?  AND A.TRANSTYPE='D' AND A.ACCOUNTID=?  and c.account=a.accountid GROUP BY C.SEARCHKEY)" +
                " AS FSDA GROUP BY SKEY having SUM(AMT1)!='0.0' and SUM(AMT2)!='0.0' and SUM(AMT3)!='0.0' ",
                new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP,  Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.TIMESTAMP}),
                new SerializerReadClass(MemberStatementModel2.amountline.class)).list(new Object[]{ fsdate, fsdate, sdate, fsdate, sdate});
        if (previousDues != null) {
            for (amountline a : previousDues) {
                for (Facilityline f : d.getfacilityline()) {
                    if (a.getSearchKey().equals(f.getMemNo())) {
        ...
        List<addressline> adr = new StaticSentence(app.getSession(),
                ...
                null,
                new SerializerReadClass(MemberStatementModel2.addressline.class)).list();
        String addr = null;
        Map<String, String> adrmap = new HashMap<String, String>();
        if (adr != null) {
            for (addressline a : adr) {
