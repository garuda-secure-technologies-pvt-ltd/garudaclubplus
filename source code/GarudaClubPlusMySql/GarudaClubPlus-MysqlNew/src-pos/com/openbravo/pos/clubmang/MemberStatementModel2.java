 ...
 * @author swathi
 */
public class MemberStatementModel2 {
    ...
    private MemberStatementModel2() {
    }

    public static MemberStatementModel2 emptyinstance() {
        MemberStatementModel2 d = new MemberStatementModel2();
    ...
    public static MemberStatementModel2 loadInstance(AppView app, String ID, String accid, DataLogicFacilities dlfac, Date fromdate, Date todate,String searchkey ,int Flag) throws BasicException {
        MemberStatementModel2 d = new MemberStatementModel2();
        d.dlfac = dlfac;
        Date sdate = new Date();
        try {
        ...
        //this is for a month details
         dlist = new StaticSentence(app.getSession()//Added to replace above commented qry
                ...
                " FROM CUSTOMERS C,ACCOUNTJOURNAL AJ WHERE  AJ.ACCOUNTID=? AND A.ACTIVE=TRUE  " +
        ...
        double crtemp = 0, drtemp = 0, temp = 0;
         ...
         List<amountline> previousDues = new StaticSentence(app.getSession(),
                "SELECT SUM(AMT1),SUM(AMT2),SUM(AMT3),SKEY FROM (SELECT SUM(A.DEBIT-A.CREDIT) AS AMT1,0.0 AS AMT2,0.0 AS AMT3,C.SEARCHKEY AS SKEY  FROM AJPERIODTOTALS A,CUSTOMERS C WHERE A.EDATE < ? AND A.ACCOUNTID=C.ACCOUNT  GROUP BY C.SEARCHKEY " +
                " UNION ALL " +
                " SELECT 0.0 AS AMT1,SUM(AMOUNT) AS AMT2,0.0 AS AMT3,C.SEARCHKEY AS SKEY  FROM ACCOUNTJOURNAL A,CUSTOMERS C WHERE  A.DATE>=? AND A.DATE < ?  AND A.TRANSTYPE='C' AND A.ACCOUNTID=C.ACCOUNT AND A.ACTIVE=TRUE GROUP  BY C.SEARCHKEY" +
                " UNION ALL " +
                " SELECT 0.0 AS AMT1,0.0 AS AMT2,SUM(AMOUNT) AS AMT3,C.SEARCHKEY AS SKEY  FROM ACCOUNTJOURNAL A,CUSTOMERS C WHERE  A.DATE>=? AND A.DATE < ?  AND A.TRANSTYPE='D' AND A.ACCOUNTID=C.ACCOUNT AND A.ACTIVE=TRUE GROUP BY C.SEARCHKEY)" +
      ...
      // FOR MEMBER WHO DON'T HAVE ANY TRASACTION ----- AAKASH    
            ...
            if(Flag==0)
              ...
              d.fac = dlist;
                ...
                previousDues = new StaticSentence(app.getSession(),  "SELECT SUM(A.DEBIT) AS AMT1 , SUM(A.CREDIT) AS AMT2, 0.0 AS AMT3  , C.searchkey FROM AJPERIODTOTALS A , customers c WHERE A.EDATE < ? AND A.ACCOUNTID=? and a.accountid = c.account group by a.accountid having amt1!='0.00'",
                new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP,Datas.STRING }),
                new SerializerReadClass(MemberStatementModel2.amountline.class)).list(new Object[]{ todate,accid });
    ...
    public static MemberStatementModel2 loadInstance(AppView app, DataLogicFacilities dlfac, Date fromdate, Date todate, String memtype,int Flag ) throws BasicException {
        MemberStatementModel2 d = new MemberStatementModel2();
        d.dlfac = dlfac;
        Date sdate = new Date();
        try {
    ...
    public static MemberStatementModel2 loadInstanceforAllMembers(AppView app, DataLogicFacilities dlfac, Date fromdate, Date todate ,int Flag) throws BasicException {
        MemberStatementModel2 d = new MemberStatementModel2();
        d.dlfac = dlfac;
        Date sdate = new Date();
        try {
