 ...
 * @author garuda
 */
public class Lib_Vendordetails extends javax.swing.JPanel implements EditorRecord {
    ...
    public Lib_Vendordetails(AppView app, DirtyManager dirty) {
        m_App=app;
        initComponents();
    ...
    public Object createValue() throws BasicException {
        ...
        if(status==0){
            ...
            vendor[15]=null;
             ...
             if(pstatus){
                  ...
                  vendor[14]=new Date();
                  vendor[15]=m_App.getProperties().getHost();
             }
        }else if(status==2){
             if(pstatus){
                  vendor[13]=null;
        ...
        vendor[16]=Formats.STRING.formatValue(m_App.getProperties().getHost());
        if(panno.getText().length()>0 && tinno.getText().length()>0 && cstno.getText().length()>0){
            return vendor;
        }else{
            String missingnos=" ";
