...
public class PaymentsModel2 {
    ...
    private java.util.List<PaymentsModel2.OtherIncomeTotalLine> m_lotherincometotals;
    ...
    private java.util.List<PaymentsModel2.PaymentsLine1> m_lsales;
    private java.util.List<PaymentsModel2.PaymentsLine2> m_lpayments;
    private java.util.List<PaymentsModel2.UCReceiptLine> ucreceipts;
    private double ucreceipttotal;
    private Double dtotal;
    private Double cashinsum;
    private Double balance;
    private final static String[] PAYMENTHEADERS2 = {"Date", "RECEIPT NO.", "MODE OF PAYMENT", "TOTAL"};
    ...
    private java.util.List<PaymentsModel2.WarehouseTotals> clist;
    private java.util.List<PaymentsModel2.WarehouseTotals> clist1;
    //warehouse changes - end
    private java.util.List<PaymentsModel2.RdetailsLine> m_lrdetails;
    private java.util.List<PaymentsModel2.RdetailsLine2> m_lrdetails2;
    private java.util.List<PaymentsModel2.RdetailsLine1> m_lrdetails1;
    private java.util.List<PaymentsModel2.Debtcollected> drdetails;
    private java.util.List<PaymentsModel2.OtherIncome> otherIncomedetails;
    private java.util.List<PaymentsModel2.ChequeDetailLine> cdetail;
    private java.util.List<DebtBillList.CreditConfirmList> crlist;
    private java.util.List<PaymentsModel2.GuestReceiptdetailsLine> guestreceiptDetails;
    private java.util.List<PaymentsModel2.GuestReceiptdetailsLine1> guestreceiptDetails1;
    private java.util.List<PaymentsModel2.GuestReceiptdetailsLine1> guestreceiptDetails2;
    ...
    public static PaymentsModel2 emptyInstance() {
        ...
        p.crlist = new ArrayList<DebtBillList.CreditConfirmList>();
    ...
    public static PaymentsModel2 loadInstance(AppView app, Date date) throws BasicException {
        ...
        List<PaymentsModel2.PaymentsLine1> l = new StaticSentence(app.getSession(), "SELECT PAYMENTS.PAYMENT, SUM(PAYMENTS.TOTAL) " +
                ...
                "GROUP BY PAYMENTS.PAYMENT", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.TIMESTAMP}), new SerializerReadClass(PaymentsModel2.PaymentsLine1.class)).list(new Object[]{app.getAppUserView().getUser().getName(), date}); //new SerializerReadBasic(new Datas[] {Datas.STRING, Datas.DOUBLE}))
        ...
        if (l == null) {
            ...
            p.m_lsales = l;
            for (PaymentsModel2.PaymentsLine1 p1 : l) {
        ...
        List<WarehouseTotals> ct = new StaticSentence(app.getSession(),
                ...
                new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.TIMESTAMP}), new SerializerReadClass(PaymentsModel2.WarehouseTotals.class)).list(new Object[]{app.getAppUserView().getUser().getName(), date});
        ...
        List<PaymentsLine2> asales = new StaticSentence(app.getSession(),
                " SELECT PAYMENTS.RECEIPT,PAYMENTS.PAYMENT AS PYMT,(PAYMENTS.TOTAL * -1) AS PAYTOTAL ,PAYMENTS.PTIME  FROM PAYMENTS,PEOPLE,RECEIPTS " +
                "WHERE PAYMENTS.TOTAL < 0 AND PAYMENTS.PAYMENT != 'debtpaid' AND PAYMENTS.PUSER=? AND PAYMENTS.PUSER = PEOPLE.NAME AND RECEIPTS.ID=PAYMENTS.RECEIPT  AND RECEIPTS.CLOSECASHSEQ IS NULL AND PEOPLE.CLOSECASHTIME IS NULL ", SerializerWriteString.INSTANCE, new SerializerReadClass(PaymentsModel2.PaymentsLine2.class)).list(LookupUtilityImpl.getInstance(null).getAppView().getAppUserView().getUser().getName());
        if (asales == null) {
            p.m_lpayments = new ArrayList<PaymentsLine2>();
        } else {
...
//debt collected total
        ...
        List<RdetailsLine> rdetail = new StaticSentence(app.getSession(),
                ...
                " GROUP BY RECEIPTS.ID,RECEIPTS.DATENEW,BILL.CUSTOMER,L.NAME,BILL.ID ORDER BY L.NAME,RECEIPTS.ID", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.TIMESTAMP}), new SerializerReadClass(PaymentsModel2.RdetailsLine.class)).list(new Object[]{app.getAppUserView().getUser().getName(), date});
        if (rdetail == null) {
            p.m_lrdetails = new ArrayList<RdetailsLine>();
        } else {
        ...
        List<RdetailsLine1> rdetail2 = new StaticSentence(app.getSession(),
                ...
                ".NAME,CUSTOMERS.ACCOUNT,CUSTOMERS.SEARCHKEY,BILL.CREATEDDATE,BILL.ID,CUSTOMERS.ID ORDER BY L.NAME,RECEIPTS.ID ", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.TIMESTAMP}), new SerializerReadClass(PaymentsModel2.RdetailsLine1.class)).list(new Object[]{app.getAppUserView().getUser().getName(), date});
        if (rdetail2 == null) {
            p.m_lrdetails1 = new ArrayList<RdetailsLine1>();
        } else {
        ...
        List<RdetailsLine2> ddetail = new StaticSentence(app.getSession(),
                ...
                " GROUP BY RECEIPTS.ID,RECEIPTS.DATENEW,CUSTOMERS.SEARCHKEY,L.NAME,BILL.ID ORDER BY L.NAME,RECEIPTS.ID", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.TIMESTAMP}), new SerializerReadClass(PaymentsModel2.RdetailsLine2.class)).list(new Object[]{app.getAppUserView().getUser().getName(), date});
        if (ddetail == null) {
            p.m_lrdetails2 = new ArrayList<RdetailsLine2>();
        } else {
        ...
        List<OtherIncome> rdetail3 = new StaticSentence(app.getSession(),
                ...
                , new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.TIMESTAMP, Datas.STRING, Datas.TIMESTAMP}), new SerializerReadClass(PaymentsModel2.OtherIncome.class)).list(new Object[]{app.getAppUserView().getUser().getName(), date, app.getAppUserView().getUser().getName(), date});
        if (rdetail3 == null) {
            p.otherIncomedetails = new ArrayList<OtherIncome>();
        } else {
...
//        }
        ...
        List<Debtcollected> rdetail1 = new StaticSentence(app.getSession(),
                ...
                " WHERE CUSTOMERS.ID=PAYMENTS.CUSTOMER AND PAYMENTS.TOTAL>0 AND PAYMENTS.PUSER = PEOPLE.NAME AND PEOPLE.NAME=RECEIPTS.RUSER AND PEOPLE.NAME= ?  AND RECEIPTS.ID=PAYMENTS.RECEIPT   AND  PEOPLE.CLOSECASHTIME IS NULL AND RECEIPTS.PAYMENTREF IS NULL AND PAYMENTS.RECEIPT NOT IN (SELECT BILL.RECEIPT FROM BILL where bill.receipt is not null) AND RECEIPTS.DATENEW <=? AND RECEIPTS.CLOSECASHSEQ IS NULL ORDER BY RECEIPTS.ID  ", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.TIMESTAMP}), new SerializerReadClass(PaymentsModel2.Debtcollected.class)).list(new Object[]{LookupUtilityImpl.getInstance(null).getAppView().getAppUserView().getUser().getName(), date});
        if (rdetail1 == null) {
            p.drdetails = new ArrayList<Debtcollected>();
        } else {
        ...
        if (warehouses != null) {
            ...
            List<DebtBillList.CreditConfirmList> crconf = new StaticSentence(app.getSession(), "SELECT D.DATENEW,C.NAME,C.SEARCHKEY,W.NAME,D.BILLREF,D.ID,D.RUSER,D.amount,c.id FROM CREDITCONFLIST D JOIN CUSTOMERS C ON D.CUSTOMER=C.ID JOIN WAITER W ON D.WAITER=W.ID join bill b on b.id=d.billref and b.warehouse in  (" + condition.toString() + ") group by d.id ORDER BY D.DATENEW,W.NAME", new SerializerWriteBasic(data), new SerializerReadClass(DebtBillList.CreditConfirmList.class)).list(params);
            if (crconf == null) {
                p.crlist = new ArrayList<DebtBillList.CreditConfirmList>();
            } else {
                p.crlist = crconf;
            ...
            p.creditconftotal = 0;
            for (DebtBillList.CreditConfirmList c : crconf) {
        ...
        Object[] PaymntOut = (Object[]) new StaticSentence(app.getSession(),
                " SELECT COALESCE(SUM(REFUND_AMT),0) FROM room_hall_refund_voucher WHERE CLOSECASH IS NULL AND REFUND_BY=?  AND REFUND_DATE<=?",
                new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.TIMESTAMP}), new SerializerReadBasic(new Datas[]{Datas.DOUBLE})).find(new Object[]{app.getAppUserView().getUser().getName(), date});

        if (PaymntOut == null) {
            p.payTotal = new Double(0.0);
        } else {
            p.payTotal = (Double) PaymntOut[0];
        }
        
        List<PaymentsLine2> RefundDetials = new StaticSentence(app.getSession(),
                " SELECT  RV_NO , 'CASH' ,  REFUND_AMT , REFUND_DATE  FROM room_hall_refund_voucher WHERE CLOSECASH IS NULL AND REFUND_BY=? ORDER BY REFUND_DATE DESC  ", SerializerWriteString.INSTANCE, new SerializerReadClass(PaymentsModel2.PaymentsLine2.class)).list(LookupUtilityImpl.getInstance(null).getAppView().getAppUserView().getUser().getName());
        if (asales == null) {
            p.m_lpayments = new ArrayList<PaymentsLine2>();
        } else {
            p.m_lpayments = RefundDetials;
        }
       ...
       // p.balance = p.receiptTotalsum + p.cashinsum + p.debtcollection - p.debtraised + p.otherIncomeTotal;
        ...
        p.balance = p.cashinsum + p.debtcollection - p.debtraised + p.otherIncomeTotal -  p.payTotal ;         // NEWLY EDITED FOR BOOKING CLOSE CASH ......................................................................................# AAKASH
        return p;
    }
    ...
    public List<DebtBillList.CreditConfirmList> getCreditConfList() {
        return crlist;
    }
    ...
    public AbstractTableModel PendingCrConfTableModel() {
        return new AbstractTableModel() {
            ...
            public Object getValueAt(int row, int column) {
                DebtBillList.CreditConfirmList l = crlist.get(row);
                switch (column) {
                    case 0:
    ...
    public static class PaymentsLine2 implements SerializableRead {
        ...
        public String getDate(){
            String d = Formats.TIMESTAMP.formatValue(Refund_Date);
            return d;
        }
        public void setDate(Date Refund_Date){
