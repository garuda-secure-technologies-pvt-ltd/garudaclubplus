...
public class MemDebtBillingTableModel {
  ...
  public static MemDebtBillingTableModel loadInstance(AppView app,String ID,String accid,DataLogicFacilities dlfac) throws BasicException{
      ...
      List dlist = new StaticSentence(app.getSession()//Added to replace above commented qry
                  ...
                  +" UNION ALL SELECT A.TRANSREF AS NAME,A.DUEDATE AS DUEDATE,A.BALANCEAMOUNT AS AMOUNT,A.ID AS AID,A.TRANSNO AS TRANSNO,A.ACCOUNTID AS ACCOUNT,A.NARRATION AS NARRATION,A.DATE AS DATE,A.TRANSREF AS FID,A.AMOUNT AS AMT "
                  +" FROM ACCOUNTJOURNAL A  WHERE  A.ACCOUNTID= ? AND  A.CLEARDATE IS NULL AND A.ACTIVE=TRUE  AND A.ADJUSTED=FALSE AND A.TRANSTYPE='D'   AND A.TRANSREF NOT IN (SELECT ID FROM FACILITY) "
                  +" UNION ALL SELECT F.NAME AS NAME,AJ.DUEDATE AS DUEDATE,AJ.BALANCEAMOUNT AS AMOUNT,AJ.ID AS AID,AJ.TRANSNO AS TRANSNO,AJ.ACCOUNTID AS ACCOUNT,AJ.NARRATION AS NARRATION,AJ.DATE AS DATE,F.ID AS FID,AJ.AMOUNT AS AMT "
                  +" FROM AJUNADJUSTED AJ JOIN FACILITY F ON  F.ID=AJ.TRANSREF WHERE  AJ.ACCOUNTID=? AND AJ.CLEARDATE IS NULL AND AJ.ACTIVE=TRUE AND  AJ.ADJUSTED=FALSE AND AJ.TRANSTYPE='D' "
                  +" UNION ALL SELECT AJ.TRANSREF AS NAME,AJ.DUEDATE AS DUEDATE,AJ.BALANCEAMOUNT AS AMOUNT,AJ.ID AS AID,AJ.TRANSNO AS TRANSNO,AJ.ACCOUNTID AS ACCOUNT,AJ.NARRATION AS NARRATION,AJ.DATE AS DATE,AJ.TRANSREF AS FID,AJ.AMOUNT AS AMT "
                  +" FROM AJUNADJUSTED AJ WHERE  AJ.ACCOUNTID=? AND  AJ.CLEARDATE IS NULL AND AJ.ACTIVE=TRUE  AND AJ.ADJUSTED=FALSE AND AJ.TRANSTYPE='D'   AND AJ.TRANSREF NOT IN (SELECT ID FROM FACILITY) "
                  +" ) AS COLLTRANS ORDER BY 8 "  
                  ,new SerializerWriteBasic(new Datas[]{Datas.STRING,Datas.STRING,Datas.STRING,Datas.STRING})
                  ,new SerializerReadClass( MemDebtBillingTableModel.Facilityline.class )).list(new Object[]{accid,accid,accid,accid});
         if(dlist==null)
         {
             d.fac=new ArrayList<Facilityline>();
         }
        ...
        {   
         ...
         List<Creditline> obj= new StaticSentence(app.getSession()
                 ,"SELECT ID,BALANCEAMOUNT,TRANSNO,NARRATION,DATE,TRANSREF,AMOUNT FROM ACCOUNTJOURNAL WHERE ADJUSTED=FALSE AND BALANCEAMOUNT>0 AND TRANSTYPE='C' AND ACCOUNTID=? AND ACTIVE=TRUE ORDER BY DATE"
                 ,SerializerWriteString.INSTANCE
                 ,new SerializerReadClass(MemDebtBillingTableModel.Creditline.class)
                 ).list(accid);
         if(obj.size()>0 ){
        ...
        return new MyAbstractTableModel() {
           ...
           public void setValueAt(Object aValue, int row, int column) {
            ...
            public Object getValueAt(int row, int column) {
                ...
                case 1:return Formats.DATE.formatValue(l.getBilledDate());
                case 2: return Formats.DATE.formatValue(l.getduedate());
                case 3: return dlfac.roundTwoDecimals(l.getamount());
                case 4: return l.getNarration();
                case 5:return l.getamountb();
                case 6:return l.isSelected();
                case 7:return l.getamt();
                case 8: return l.getaccid();
                case 9:return l.gettransno();
                case 10: return l.getcustomeraccount();
                case 11: return l.getFacilityId();
        ...
        return new MyAbstractTableModel() {
           ...
           public void setValueAt(Object aValue, int row, int column) {
            ...
            public Object getValueAt(int row, int column) {
                ...
                case 0: return l.getTransref();
                case 1:return l.getTransno();
                case 2: return Formats.DATE.formatValue(l.getDate());
                case 3: return l.getNarration();
                case 4: return l.getamount();
                case 5:return l.isSelected();
                case 6:return l.getAmt();
                default: return null;
                }
            }
        };
