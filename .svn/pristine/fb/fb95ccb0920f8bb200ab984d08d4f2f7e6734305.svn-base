...
public class VendorDetailTableModel {
  ...
  public static VendorDetailTableModel loadInstance(AppView app) throws BasicException{
      ...
      List dlist = new StaticSentence(app.getSession()
                , "SELECT NAME,ADDRESS,CONTACTPERSON,CONTACTNUM,PANNO,TINNO,CST,ACCOUNTMASTER.NAME,INSTRUCTION,CREATEDBY,DEACTBY,ID FROM VENDOR,ACCOUNTMASTER WHERE ACTIVE =TRUE AND ACCOUNTMASTER.ID=ACCOUNT ORDER BY NAME"
                , null
