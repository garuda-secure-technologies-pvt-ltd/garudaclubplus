...
public class TrialBalanceReport extends javax.swing.JPanel implements JPanelView,BeanFactoryApp {
   ...
   private Date getSecondDate(String date) throws BasicException{
    ...
    private List<TrailBalanceData> getStatement(Date edate) throws BasicException{
        ...
        String statement="SELECT AM.NAME,AM.ID,AM.SEARCHKEY,AM.PARENT,AM.LEVEL,COALESCE(SUM(AJ.DEBIT+AJ.CURDEBIT),0.0) AS DAMOUNT,COALESCE(SUM(AJ.CREDIT+AJ.CURCREDIT),0.0) AS CAMOUNT,AM.SUMMARY AS SUMMARY FROM ACCOUNTMASTER AM LEFT OUTER JOIN AJPERIODTOTALS AJ ON AM.ID=AJ.ACCOUNTID AND AJ.EDATE<=? "+
                          "GROUP BY AM.NAME,AM.ID,AM.SEARCHKEY,AM.PARENT,AM.LEVEL,AM.SUMMARY ORDER BY AM.SEARCHKEY";
 ...
 private List<TrailBalanceData> createTrailBalanceReport2(String statement,Date fedate) throws BasicException{//before there was sent
       //  gfhf
          ...
          List<TrailBalanceData> acclistele= new StaticSentence(m_app.getSession()
                           ...
                           ,new SerializerReadClass(TrailBalanceData.class)).list(new Object[]{fedate});
              ...
              if(t.getParent()!=null){
                if(t.getcredit()>0 || t.getdebit()>0){
                 ...
                 do{
                    ...
                    if(t.getSearchkey().startsWith("1.2")){
                       System.out.println("Name : "+t.getName()+"::Searchkey :"+t.getSearchkey()+" :: Debit : "+t.getdebit()+" :: Credit : "+t.getcredit());
                       System.out.println("Name : "+t1.getName()+"::Searchkey :"+t1.getSearchkey()+" :: Debit : "+t1.getdebit()+" :: Credit : "+t1.getcredit());
                    }
