...
public class DataLogicSalesMySQL extends DataLogicSales {
    ...
    public final SentenceList getProductCatQBF() {
        return new StaticSentence(s
            , new QBFBuilder(
                "SELECT P.ID, P.REFERENCE, P.CODE, P.NAME, P.ISCOM, P.ISSCALE, P.PRICEBUY, P.PRICESELL, P.CATEGORY, P.TAXCAT, P.IMAGE, P.STOCKCOST, P.STOCKVOLUME, NOT (C.PRODUCT IS NULL), C.CATORDER, P.ATTRIBUTES, P.PRCATEGORY,P.UNITTYPE,P.PACCOUNT,P.SACCOUNT,null " +
    ...
    public Integer getNextTransNumberIndex() throws BasicException {
        int seqNumber =  (Integer) new StaticSentence(s, "SELECT MAXVALUE FROM BILLSEQUENCE WHERE NAME=?", SerializerWriteString.INSTANCE, SerializerReadInteger.INSTANCE).find("collectiontransfer_seq");
         new StaticSentence(s, "UPDATE BILLSEQUENCE SET MAXVALUE=MAXVALUE+1 WHERE NAME=?", SerializerWriteString.INSTANCE, null).exec("collectiontransfer_seq");
        return seqNumber;
    }
}