...
public class CollectionTransfer extends javax.swing.JPanel implements JPanelView,BeanFactoryApp{
  ...
  //  }
    ...
    private void initComponents() {
        ...
        nos.setFont(new java.awt.Font("Tahoma", 1, 12));
         ...
         if(chequecash>0.0){
          ...
          if(jComboBox1.getSelectedIndex()!=-1 && (cashtotal!=0.0 || chequecash!=0.0)){
           ...
           new PreparedSentence(m_App.getSession()
                  , "INSERT INTO COLLECTIONTRANSFER(ID,DATE,CASHDETAIL,CASHTOTAL,CHEQUEDETAIL,CHEQUETOTAL,SENDER,RECEIVER,RECEIVED,NO) VALUES (?,?,?,?,?,?,?,?,?,?)"
                  , new SerializerWriteBasic(new Datas[]{Datas.STRING,Datas.TIMESTAMP,Datas.STRING,Datas.DOUBLE,Datas.STRING,Datas.DOUBLE,Datas.STRING,Datas.STRING,Datas.BOOLEAN,Datas.INT})
                  ).exec(value);
          }else{
        ...
        try{
         ...
         if(ctotal>0){
             if(JOptionPane.showConfirmDialog(this, "Do you want to transfer " +Formats.CURRENCY.formatValue(ctotal)+" to " + account.getName(), null, JOptionPane.YES_NO_OPTION)==JOptionPane.YES_OPTION){
                 ...
                 Object[] chvalue=new Object[]{UUID.randomUUID().toString(),tid,null,d,"C","Contra",transno,ctotal,d,true,receiver,m_App.getProperties().getHost(),"Cash transfered to "+account.getName(),m_App.getAppUserView().getUser().getchequeaccount(),0.0,d,"Cash handover",true};
               ...
               //  if(JOptionPane.showConfirmDialog(this, "Cash was sucessfully transfered.Do u want a print out", null, JOptionPane.YES_NO_CANCEL_OPTION)==JOptionPane.YES_OPTION){
