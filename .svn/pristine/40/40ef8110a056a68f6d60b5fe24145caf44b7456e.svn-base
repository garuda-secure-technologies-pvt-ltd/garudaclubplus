...
public class JRootApp extends JPanel implements AppView {
    ...
    public String initApp(AppProperties props) {
        jButton1.setText("Create Company");
        flag=false;
        m_props = props;
    ...
    private java.util.List getActiveCompanies() throws BasicException{
        java.util.List<CompanyInfo> list=new StaticSentence(session, "SELECT ID,NAME,DESC,DATABASEPATH,CRDATE,URL,ADDRESS FROM COMPANY WHERE ACTIVE='true'"
                                                ,null, new SerializerReadClass(CompanyInfo.class)).list();
        return list;
    }
    ...
    private class AppUserAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent evt) {
            ...
            try{
             Object[] obj=(Object[])new StaticSentence(session, "SELECT ACTIVE FROM APPLICATIONS WHERE ID=?"
                    , SerializerWriteString.INSTANCE,new SerializerReadBasic(new Datas[]{Datas.BOOLEAN})).find(AppLocal.APP_ID);
    ...
    private void openAppView(AppUser user) {
      if(user.getName().trim().toUpperCase().equals("MEMBERS")){
       ...
       try {
        ...
        Object[] log =(Object[])   new StaticSentence(LookupUtilityImpl.getInstance(null).getAppView().getSession()
                        , "SELECT LOGINTIME,OPENCASHTIME,OPENSALE FROM PEOPLE WHERE  ID= ?"
                        ,  SerializerWriteString.INSTANCE
                        , new SerializerReadBasic(new Datas[] {Datas.TIMESTAMP,Datas.TIMESTAMP,Datas.TIMESTAMP}))
                        .find(user.getId());
                  if(log!=null ){
                      if(log[0]!=null)
                      newdate=(Date)(log[0]);
        ...
        if (ocashtime == null) {
            if(user.hasPermission("payment")){
              String  sActiveCashIndex=UUID.randomUUID().toString();
               Object[] temp =(Object[])   new StaticSentence(LookupUtilityImpl.getInstance(null).getAppView().getSession()
                        , "SELECT COUNT(*) FROM CLOSEDCASH WHERE  HOSTSEQUENCE='-1' AND USER_=?"// change:USER -> USER_
                        ,  SerializerWriteString.INSTANCE
           ...
           if(cal1.before(cal2)){
            if( JOptionPane.showConfirmDialog(this, "Login Time is less than Open Cash Time.Previous Open Cash Time is "+ocashtime+" .Do you want to override the open cash time ?", "System Time is Modified", JOptionPane.OK_OPTION)==JOptionPane.NO_OPTION){
                ...
                user.setOpenCashTime(d);
                  new PreparedSentence(LookupUtilityImpl.getInstance(null).getAppView().getSession()
                    , "UPDATE CLOSEDCASH SET DATESTART=? WHERE USER_=? AND HOSTSEQUENCE<0" // change:USER -> USER_
        ...
        if(user.getOpenSaleTime() == null) {
            ...
            new PreparedSentence(LookupUtilityImpl.getInstance(null).getAppView().getSession()
                    , "UPDATE PEOPLE SET OPENSALE=?,CLOSESALE=? WHERE ROLE=?"
                    ,new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP,Datas.STRING,Datas.STRING})).exec(new Object[]{d,null,user.getRole()});
            }
       ...
       if(flag1==true){
           ...
           if(user.hasPermission("sales")){
              int count1=1;
               Object[] con =(Object[])   new StaticSentence(LookupUtilityImpl.getInstance(null).getAppView().getSession()
               ...
               if(count1!=0 )
               {
                    java.util.List<Object[]> con1 =new StaticSentence(LookupUtilityImpl.getInstance(null).getAppView().getSession()
                        , "SELECT OPENSALE,LOGINTIME FROM PEOPLE WHERE LOGINTIME IS NOT NULL AND ROLE = ? ORDER BY LOGINTIME"
                        ,  SerializerWriteString.INSTANCE
                        , new SerializerReadBasic(new Datas[] {Datas.TIMESTAMP,Datas.TIMESTAMP}))
                        .list(user.getRole());
            ...
            int port=0;
            sSocketActive=true;
            ListenerSocket l=new ListenerSocket(user.getName());
    ...
    private void initComponents() {
        ...
        m_jLblTitle.setText("Garuda Club Plus is a Club Management System with integrated POS designed for touch screens.");
