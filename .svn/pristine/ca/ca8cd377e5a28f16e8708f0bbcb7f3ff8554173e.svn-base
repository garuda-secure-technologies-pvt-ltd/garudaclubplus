  ...
  /*  public String getname()
     ...
     //   m_LocationsModel = new ComboBoxValModel(m_sentlocations);
       ...
       m_jLocation.setModel(m_LocationsModel);*/
        ...
        Object[] obj=(Object[]) new StaticSentence(m_App.getSession(),
                ...
                ,new SerializerReadBasic(new Datas[] { Datas.STRING})).find(LookupUtilityImpl.getInstance(null).getAppView().getAppUserView().getUser().getId());
        String[] prcat;
                ...
                , SerializerWriteString.INSTANCE
                ,new SerializerReadClass(CategoryInfo.class)).find(prcat[i]);
                     if(obj1!=null)
                     {
                        temp=obj1;
    ...
    private void saveData() {
        try {
            Double num;
            Object[] obj=(Object[]) new StaticSentence(m_App.getSession(),
                "SELECT MAX(NUM) FROM STOCKDIARY  "
                , SerializerWriteString.INSTANCE
     ...
     private void saveData1(InventoryRecord rec1,InventoryRecord rec,Double num) throws BasicException {
      ...
      private void saveData2(InventoryRecord rec,Double num) throws BasicException {
        ...
        for (int i = 0; i < m_invlines.getCount(); i++) {
          ...
          "SELECT C.NAME,C.ID,C.PARENTID FROM PRODUCTS P,CATEGORIES C WHERE C.ID=P.CATEGORY AND P.ID=?"
           , SerializerWriteString.INSTANCE
           ,new SerializerReadBasic(new Datas[] { Datas.STRING,Datas.STRING,Datas.STRING})).find(il.getProductID());
    ...
    private void initComponents() {
        ...
        m_jDelete.setBounds(260, 280, 50, 42);
        ...
        jPanel3.add(m_jDown);
        m_jDown.setBounds(260, 240, 50, 42);
        ...
        if(m_jLocationDes.getSelectedItem()!=null && m_jLocation.getSelectedItem()!=null)
        {
            ...
            try{
              ...
              Object[] obj1=(Object[]) new StaticSentence(m_App.getSession(),
                ...
                , SerializerWriteString.INSTANCE
                ,new SerializerReadBasic(new Datas[] {Datas.DOUBLE, Datas.STRING,Datas.DOUBLE})).find(pid);
              if(obj1==null || obj1[0]==null)
              {
                   Object[] obj2=(Object[]) new StaticSentence(m_App.getSession(),
                "SELECT NOFST,PRODUCTFST,NOSEC FROM CONVERTER WHERE PRODUCTSCN = ? AND ACTIVE=TRUE "
                , SerializerWriteString.INSTANCE
                ,new SerializerReadBasic(new Datas[] {Datas.DOUBLE, Datas.STRING,Datas.DOUBLE})).find(pid);
                   ...
                   if(obj2!=null && obj2[0]!= null)
                   {
                       ...
                       " FROM PRODUCTS P  LEFT OUTER JOIN TAXCATEGORIES TC ON P.TAXCAT = TC.ID WHERE P.ID = ?"
                        , SerializerWriteString.INSTANCE
                       ,new SerializerReadClass(ProductInfoExt.class)).find(obj2[1].toString());
                        ...
                        " FROM PRODUCTS P  LEFT OUTER JOIN TAXCATEGORIES TC ON P.TAXCAT = TC.ID WHERE P.ID = ?"
                       , SerializerWriteString.INSTANCE
                        ,new SerializerReadClass(ProductInfoExt.class)).find(obj1[1].toString());
                        System.out.println(obj1[0]);
                        System.out.println(obj1[2]);
