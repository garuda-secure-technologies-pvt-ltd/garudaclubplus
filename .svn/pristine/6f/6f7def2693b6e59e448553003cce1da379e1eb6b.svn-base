...
public abstract class DataLogicIntegration extends BeanFactoryDataSingle {
        ...
        Transaction t = new Transaction(s) {
            public Object transact() throws BasicException {
                ...
                if (new PreparedSentence(s, 
                    ...
                    // If not updated, try to insert
                    new PreparedSentence(s, 
                            "INSERT INTO PRODUCTS (ID, REFERENCE, CODE, NAME, ISCOM, ISSCALE, PRICEBUY, PRICESELL, CATEGORY, TAXCAT, IMAGE, STOCKCOST, STOCKVOLUME, PRCATEGORY,UNITTYPE,PACCOUNT,SACCOUNT) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?)",
                            SerializerWriteParams.INSTANCE
                            ).exec(new DataParams() { public void writeValues() throws BasicException {
    ...
    public List getTickets() throws BasicException {
        ...
        return new PreparedSentence(s
                , "SELECT T.ID, T.TICKETID, R.DATENEW, R.ID, P.ID, P.NAME, C.ID, C.TAXID, C.SEARCHKEY, C.NAME FROM RECEIPTS R JOIN TICKETS T ON R.ID = T.ID LEFT OUTER JOIN PEOPLE P ON T.PERSON = P.ID LEFT OUTER JOIN CUSTOMERS C ON T.CUSTOMER = C.ID WHERE T.STATUS_ = 0"
                , null
    ...
    public void execTicketUpdate() throws BasicException {
        new StaticSentence(s, "UPDATE TICKETS SET STATUS_ = 1 WHERE STATUS_ = 0").exec();
    }
}
