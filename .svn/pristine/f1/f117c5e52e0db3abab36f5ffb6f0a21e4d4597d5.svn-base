...
public class DueListTableModel {
   ...
   public static DueListTableModel loadData(AppView app,List<Facility> flist,String memtypeid,Date date,Date overduedate,DataLogicFacilities dlfac) throws BasicException{
       ...
       d.flist=flist;
        if(memtypeid==null)
            query="SELECT C.ID as cid,C.NAME as cname,C.SEARCHKEY as ckey,F.NAME as fname,F.ID as fid,M.CURRENTDEBT as currdebt, MF.LBILLDATE as lbilldate,MF.SDATE as sdate,MF.ACTIVE as active,MF.STATUS as status,0.0 as overdue,0.0 as due,c.account as acc FROM  CUSTOMERS C join  MEMDEBTTABLE M on c.id=m.memid join facility F on M.FACILITY=f.id JOIN MEMFACILITYUSAGE MF ON M.MEMID=MF.MEMNO AND F.id=MF.FACILITYTYPE   ";
       else{
       ...
       if(barfacflag==0){
           if(queryOthers==null)
             obj=new PreparedSentence(app.getSession(),"select cid,cname,ckey,fname,fid,currdebt,lbilldate,sdate,active,status,overdue,due,acc,sum(a.balanceamount) from( "+query+" ) left join accountjournal a on a.accountid=acc and a.transtype='C' where currdebt >0  group by cid,cname,ckey,fname,fid,currdebt,lbilldate,sdate,active,status,overdue,due,acc order by 3,4"
          ...
          if(queryOthers==null)
              obj=new PreparedSentence(app.getSession(),"select cid,cname,ckey,fname,fid,currdebt,lbilldate,sdate,active,status,overdue,due,acc,sum(a.balanceamount) from( "+query+" ) left join accountjournal a on a.accountid=acc and a.transtype='C' group by cid,cname,ckey,fname,fid,currdebt,lbilldate,sdate,active,status,overdue,due,acc order by 3,4"
