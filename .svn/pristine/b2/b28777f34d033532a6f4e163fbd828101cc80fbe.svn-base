    ...
    public static CloseDayModel loadInstance(AppView app,Date d) throws BasicException {
       ...
       // p.m_iSeq = app.getActiveCashSequence();
        ...
        List salebillwise = new StaticSentence(app.getSession(),
                "SELECT G.PAYMENTREF,C.NAME,R.DESC,G1.GAMEAMOUNT,G.CLUBCOLLECTION FROM GAMELOG G,CUSTOMERS C,RECEIPTS R,GAMES G1 WHERE G.JACKID=C.ID AND G.PAYMENTREF=R.ID AND R.DESC=G1.NAME AND R.CLOSECASHSEQ IS NULL AND G.COMPLETEDON<=?"
                /*" SELECT SUM(TOTAL) AS TOTAL,GAMETYPE FROM (SELECT PAYMENTS.PTIME,PAYMENTS.RECEIPT,CUSTOMERS.NAME,PAYMENTS.TOTAL AS TOTAL,RECEIPTS.DESC,(SELECT NAME FROM GAMES WHERE (SELECT PARENT  FROM GAMES,RECEIPTS WHERE GAMES.NAME=RECEIPTS.DESC)=GAMES.ID)AS GAMETYPE,CUSTOMERS.ACCOUNT,CUSTOMERS.SEARCHKEY,RECEIPTS.PAYMENTREF FROM PAYMENTS,CUSTOMERS,RECEIPTS,PEOPLE"+
