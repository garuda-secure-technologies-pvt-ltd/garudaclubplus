...
//    }
   ...
   private String getNextBillID() throws BasicException{
     ...
     Object[] obj=(Object[])new  StaticSentence(app.getSession()
            , "SELECT SEQUENCEDETAIL.BSERIES,SEQUENCEDETAIL.BMAX FROM SEQUENCEDETAIL,PEOPLE,ROLES WHERE USERNAME=ROLES.NAME AND SEQUENCEDETAIL.CATEGORY=ROLES.NAME AND ROLES.ID=PEOPLE.ROLE AND PEOPLE.NAME=? AND ACTIVE=TRUE"
            ,new SerializerWriteBasic(new Datas[]{Datas.STRING})
            ,new SerializerReadBasic(new Datas[]{Datas.STRING,Datas.DOUBLE})).find(new Object[]{uname});
     if(obj!=null){
         ...
         billnum=obj[0].toString()+ max.intValue();
          new StaticSentence(app.getSession()
                        ...
                        .exec(new Object[] {max,uname,uname});
          return billnum;
     }else{
  ...
  /*  private String getNextBillID() throws BasicException {
         String prefix = LookupUtilityImpl.getInstance(null).getAppView().getProperties().getProperty("sequence.billnumber");
...
//         return sb.append(dlsales.getNextQTicketIndex().toString());
         ...
         return s.concat(dlsales.getNextQTicketIndex().toString());
    ...
    private void btnPayActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPayActionPerformed
        ...
        if (paymentdialog.showDialog(/*customerext.getCurdebt()*/100, null,app.getAppUserView().getUser().getName(),true)) {
           ...
           String var=null;
            try {
                ...
                if(!bno.equals(""))
               var=dlsales.payaccount(ticket,app.getInventoryLocation(),false);
