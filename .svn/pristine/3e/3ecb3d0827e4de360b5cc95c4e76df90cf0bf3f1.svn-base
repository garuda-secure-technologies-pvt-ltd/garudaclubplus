    ...
    public void displayBill() {
        ...
        try{
            ...
            Object obj[]=(Object[]) new StaticSentence(app.getSession()
                    ,"SELECT COUNT(QTITEMID) FROM DISCOUNTLIST WHERE QTITEMID = ? AND AUTHORISED is null GROUP BY QTITEMID "
                    ,SerializerWriteString.INSTANCE
                    ,new SerializerReadBasic(new Datas[]{Datas.INT})).find(qtInfo.getId());
            //temp
            if(obj!=null )
            {
                temp=obj[0].toString();
        ...
        {
        try {
            bla = new BillLogicApply(dlSales, qticket, dlBill, pendingQTList);
            BillInfo b = bla.getBillInfo();
            Billpage billpage = Billpage.getDialog(this, dlSales, bla, cust);
            billpage.init(b);
        ...
        try {
            AppView m_App = LookupUtilityImpl.getInstance(null).getAppView();
                 Object[] obj=(Object[])   new StaticSentence(m_App.getSession()
                       ...
                       ,SerializerWriteString.INSTANCE
                       ,new SerializerReadBasic(new Datas[] { Datas.STRING})).find(qTicket.getWaiter());
        ...
        if(obj== null)
           ...
           waitername=obj[0].toString();
                  ...
                  Object[] obj1=(Object[])   new StaticSentence(m_App.getSession()
                       ...
                       ,SerializerWriteString.INSTANCE
                       ,new SerializerReadBasic(new Datas[] { Datas.STRING})).find(qTicket.getPlace());
