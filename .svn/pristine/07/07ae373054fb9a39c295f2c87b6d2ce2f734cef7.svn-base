...
public class AccountReports extends javax.swing.JPanel implements JPanelView,BeanFactoryApp{
    ...
    public AccountReports(AppView app) {
     ...
     public void init(AppView app) throws BeanFactoryException {
        ...
        GroupingType.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Daily", "Monthly", "Quaterly","Yearly" }));
        ...
        //  m_app.getSession().getConnection().getMetaData().getCatalogs().get;
          ...
          if(!rscal.after(fscal)){
              if(rscal1.get(Calendar.DATE)!=rscal1.getActualMaximum(Calendar.DATE) ){
                 Object[] obj2 =(Object[])new StaticSentence(m_app.getSession(), "SELECT SUM(A.DEBIT-A.CREDIT) FROM AJPERIODTOTALS A WHERE A.ACCOUNTID=? AND A.EDATE < ? "
                                 ,new SerializerWriteBasic(new Datas[]{Datas.STRING,Datas.TIMESTAMP}),new SerializerReadBasic(new Datas[]{Datas.DOUBLE})).find(new Object[]{accm.getid(),sdate});
               ...
               Object[] obj2 =(Object[])new StaticSentence(m_app.getSession(), "SELECT (SELECT SUM(A.DEBIT-A.CREDIT) FROM AJPERIODTOTALS A WHERE A.ACCOUNTID=? AND A.EDATE < ? ),(SELECT SUM(AMOUNT) FROM ACCOUNTJOURNAL WHERE ACCOUNTID=? AND DATE>=? AND DATE < ?  AND TRANSTYPE='C' AND ACTIVE=TRUE)"
                                  +" ,(SELECT SUM(AMOUNT) FROM ACCOUNTJOURNAL WHERE ACCOUNTID=? AND DATE>=? AND DATE < ?  AND TRANSTYPE='D' AND ACTIVE=TRUE) FROM APPLICATIONS WHERE ID=?   "
         ...
         }else if(reportType.getSelectedItem()=="Trail Balance"){
              acclist=new ArrayList<TrailBalanceData>();
                Date d=new Date();
     ...
     private List<AccountsList> createLedgerReport1(Date sDate,Date eDate,String grouptype) throws BasicException{
        ...
        List<AccountsList> ledgerlist=new StaticSentence(m_app.getSession()
                , " SELECT  A.ID,A.NAME ,(SELECT SUM(A1.AMOUNT) FROM ACCOUNTJOURNAL A1 WHERE A1.TRANSTYPE='D' AND A1.ACCOUNTID=A.ID AND A1.DATE < ? GROUP BY A1.ACCOUNTID)AS DOPENBALANCE, "+
                  ...
                  " FROM ACCOUNTMASTER A ORDER BY A.NAME "
                  , new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP,Datas.TIMESTAMP,Datas.TIMESTAMP,Datas.TIMESTAMP})
                  , new SerializerReadClass(AccountsList.class)).list(new Object[]{sDate,sDate,eDate,eDate});
      return ledgerlist;
    }
  ...
  /*    private List<AccountsList> createLedgerReportforAccount1(Date sDate,Date eDate,String grouptype,String accid) throws BasicException{
        ...
        List<AccountsList> ledgerlist=new StaticSentence(m_app.getSession()
                ,  "select  a.tid, "+
                  " a.date,a.amount,a.transtype,a.transno,a.transref,a.narration,a1.transtype,a.id "+
                   "from accountjournal a join accountjournal a1 on  a.tid=a1.tid and a1.active=true and a.accountid != a1.accountid  and a.transtype!=a1.transtype join accountmaster am on a1.accountid=am.id  where a.accountid=? and a.date>=? and a.date<=? and a.active=true  order by a.date,a.tid"
                  , new SerializerWriteBasic(new Datas[]{Datas.STRING,Datas.TIMESTAMP,Datas.TIMESTAMP})
                  , new SerializerReadClass(AccountsList.class)).list(new Object[]{accid,sDate,eDate});
        int i=0,j=0;
   ...
   /*  private List<ReportData> createLedgerReportforAccount(Date sDate,Date eDate,String grouptype,String accid) throws BasicException{
    ...
    private void getsubaccounts(TrailBalanceData main) throws BasicException{
         List<TrailBalanceData> acclistsubacc= new StaticSentence(m_app.getSession()
                           , " SELECT  DISTINCT AM2.ID,AM2.NAME, AM2.SEARCHKEY,(SELECT SUM(A1.AMOUNT)  FROM ACCOUNTJOURNAL A1 WHERE A1.ACCOUNTID=AM2.ID AND A1.TRANSTYPE='C'AND A1.ACTIVE=TRUE  GROUP BY A1.ACCOUNTID)  ,(SELECT SUM(A1.AMOUNT)  FROM ACCOUNTJOURNAL A1 WHERE A1.ACCOUNTID=AM2.ID AND A1.TRANSTYPE='D' AND A1.ACTIVE=TRUE   GROUP BY A1.ACCOUNTID) ,REPLACE(AM2.LEVEL,5),AM2.LEVEL,AM2.SUMMARY"+
                                " FROM ACCOUNTMASTER AM2 "+
                                " WHERE  AM2.LEVEL='S' AND AM2.PARENT=? "+
                              //  " GROUP BY AM2.ID,AM2.NAME, AM2.SEARCHKEY,AM2.LEVEL,AM2.SUMMARY,AM2.OPENINGBALANCE"+
                                " ORDER BY AM2.NAME "
                           , new SerializerWriteBasic(new Datas[]{Datas.STRING})
                           , new SerializerReadClass(TrailBalanceData.class)).list(new Object[]{main.getSearchkey()});
    ...
    private void breakdown(TrailBalanceData main) throws BasicException{
          List<TrailBalanceData> acclistbreak= new StaticSentence(m_app.getSession()
                           ,  "  SELECT  AM2.ID,AM2.NAME,AM2.SEARCHKEY, AM2.OPENINGBALANCE,AM2.OPENINGBALANCE,REPLACE(AM2.LEVEL,3),AM2.LEVEL,AM2.SUMMARY "+
                                " FROM ACCOUNTMASTER AM2 "+
                                " WHERE  AM2.LEVEL='D'  AND AM2.PARENT=?"+

                                " ORDER BY AM2.NAME  "
                           , SerializerWriteString.INSTANCE
                           , new SerializerReadClass(TrailBalanceData.class)).list(main.getSearchkey());
                           // breakdown();
        for(TrailBalanceData breakd:acclistbreak){
                                acclist.add(breakd);
                                 getsubaccounts(breakd);
                                 List<TrailBalanceData> acclistbreak1= new StaticSentence(m_app.getSession()
                                 , "  SELECT  AM2.ID,AM2.NAME,AM2.SEARCHKEY, AM2.OPENINGBALANCE,AM2.OPENINGBALANCE,REPLACE(AM2.LEVEL,3),AM2.LEVEL,AM2.SUMMARY "+
                                " FROM ACCOUNTMASTER AM2 "+
                                " WHERE  AM2.LEVEL='D' AND AM2.PARENT=?"+
                                " ORDER BY AM2.NAME  "
                              , SerializerWriteString.INSTANCE
                              , new SerializerReadClass(TrailBalanceData.class)).list(breakd.getSearchkey());
    ...
    private void elements(TrailBalanceData ele) throws BasicException{
         List<TrailBalanceData> acclistele1= new StaticSentence(m_app.getSession()
                           , "  SELECT  AM2.ID,AM2.NAME,AM2.SEARCHKEY, AM2.OPENINGBALANCE,AM2.OPENINGBALANCE,CASE WHEN AM2.PARENT IS NULL THEN REPLACE(AM2.LEVEL,1) ELSE REPLACE(AM2.LEVEL,2) END,AM2.LEVEL,AM2.SUMMARY "+
                                " FROM ACCOUNTMASTER AM2 "+
                                " WHERE  AM2.LEVEL='E' AND AM2.PARENT=?"+

                                " ORDER BY AM2.NAME  "
                           , SerializerWriteString.INSTANCE
                           , new SerializerReadClass(TrailBalanceData.class)).list(ele.getSearchkey());
                 ...
                 List<TrailBalanceData> acclistmain= new StaticSentence(m_app.getSession()
                           , " SELECT  AM2.ID,AM2.NAME, AM2.SEARCHKEY,(SELECT SUM(A1.AMOUNT)  FROM ACCOUNTJOURNAL A1 WHERE A1.ACCOUNTID=AM2.ID AND A1.TRANSTYPE='C' AND A1.ACTIVE=TRUE   GROUP BY A1.ACCOUNTID ) ,(SELECT SUM(A1.AMOUNT)  FROM ACCOUNTJOURNAL A1 WHERE A1.ACCOUNTID=AM2.ID AND A1.TRANSTYPE='D' AND A1.ACTIVE=TRUE   GROUP BY A1.ACCOUNTID) ,REPLACE(AM2.LEVEL,5),AM2.LEVEL,AM2.SUMMARY"+
                                " FROM ACCOUNTMASTER AM2 "+
                                " WHERE  AM2.LEVEL='C' AND AM2.PARENT=? "+
                                 " GROUP BY AM2.ID,AM2.NAME, AM2.SEARCHKEY,AM2.LEVEL,AM2.SUMMARY,AM2.OPENINGBALANCE"+
                                " ORDER BY AM2.NAME  "
                           , SerializerWriteString.INSTANCE
                           , new SerializerReadClass(TrailBalanceData.class)).list(ele.getSearchkey());
     ...
     private void createTrailBalanceReport() throws BasicException{
          List<TrailBalanceData> acclistele= new StaticSentence(m_app.getSession()
                           , " SELECT  AM2.ID,AM2.NAME, AM2.SEARCHKEY, AM2.OPENINGBALANCE,AM2.OPENINGBALANCE,CASE WHEN AM2.PARENT IS NULL THEN REPLACE(AM2.LEVEL,1) ELSE REPLACE(AM2.LEVEL,2) END ,AM2.LEVEL,AM2.SUMMARY "+
                                " FROM ACCOUNTMASTER AM2 "+
                                " WHERE  AM2.LEVEL='E'AND AM2.PARENT IS NULL "+
                                " ORDER BY AM2.NAME "
                           , null
      ...
      private List<TrailBalanceData> createTrailBalanceReport1(String sent) throws BasicException{
       //  gfhf
          ...
          List<TrailBalanceData> acclistele= new StaticSentence(m_app.getSession()
                           , " SELECT  NAME,SEARCHKEY, CAMOUNT,DAMOUNT,PARENT,LEVEL,SUMMARY "+
                                " FROM ( "+sent +
                                ") ORDER BY 5,1 "
                           , null
                           , new SerializerReadClass(TrailBalanceData.class)).list();
      ...
      private List<TrailBalanceData> getPresentStatement() throws BasicException{
        String statement="SELECT AM.NAME AS NAME,AM.SEARCHKEY AS SEARCHKEY,T.OBCREDIT+T.CURCREDIT AS CAMOUNT,T.OBDEBIT+T.CURDEBIT AS DAMOUNT,AM.PARENT,AM.LEVEL,AM.SUMMARY "
                             + " FROM ACCOUNTMASTER AM LEFT OUTER JOIN TRAILBALANCE T ON AM.ID=T.ACCOUNTID ORDER BY AM.SEARCHKEY";
        return createTrailBalanceReport1(statement);
      }
      private List<TrailBalanceData> getTrailBalanceofPrevFYear() throws BasicException{
         String statement="SELECT AM.NAME AS NAME,AM.SEARCHKEY AS SEARCHKEY,T.OBCREDIT AS CAMOUNT,T.OBDEBIT AS DAMOUNT,AM.PARENT,AM.LEVEL,AM.SUMMARY "
                             + " FROM ACCOUNTMASTER AM LEFT OUTER JOIN TRAILBALANCE T ON AM.ID=T.ACCOUNTID ORDER BY AM.SEARCHKEY";
        return createTrailBalanceReport1(statement);
      }
...
//   private List<AccountReports.ReportData> temp;
  ...
  /*  public List<AccountReports.ReportData> getlistdata(){
    ...
    private List<TrailBalanceData> getStatement(Calendar fscal,Calendar fecal,Calendar recal,Calendar recal1) throws BasicException{
       ...
       /*" SELECT  AM2.ID,AM2.NAME, AM2.SEARCHKEY, (SELECT SUM(AMOUNT) FROM ACCOUNTJOURNAL WHERE ACCOUNTID=AM2.ID AND TRANSTYPE='C' AND ACTIVE=TRUE),(SELECT SUM(AMOUNT) FROM ACCOUNTJOURNAL WHERE ACCOUNTID=AM2.ID AND TRANSTYPE='D' AND ACTIVE=TRUE),AM2.PARENT ,AM2.LEVEL,AM2.SUMMARY "+
                                " FROM ACCOUNTMASTER AM2 "+
       ...
       if(!recal.after(fscal)){
           ...
           sent="SELECT AM.NAME AS NAME,AM.SEARCHKEY AS SEARCHKEY,A.CREDIT AS CAMOUNT,A.DEBIT AS DAMOUNT,AM.PARENT AS PARENT ,AM.LEVEL AS LEVEL,AM.SUMMARY AS SUMMARY FROM ACCOUNTMASTER AM LEFT OUTER JOIN AJPERIODTOTALS A ON AM.ID=A.ACCOUNTID AND A.EDATE< ?";
           if(statement==null)
               statement=sent;
           else
          ...
          if(recal1.get(Calendar.DATE)!=recal1.getActualMaximum(Calendar.DATE)){
           ...
           Date d=new Date(fscal.getTimeInMillis());
           sent="SELECT AM.NAME AS NAME,AM.SEARCHKEY AS SEARCHKEY,A.OBCREDIT AS CAMOUNT,A.OBDEBIT AS DAMOUNT,AM.PARENT AS PERANT,AM.LEVEL AS LEVEL,AM.SUMMARY AS SUMMARY FROM ACCOUNTMASTER AM LEFT OUTER JOIN TRAILBALANCE A ON AM.ID=A.ACCOUNTID ";
           if(statement==null)
               statement=sent;
           else
               statement=statement+" union all "+sent;
          // list.add(d);
           ...
           //Date d=new Date(fscal.getTimeInMillis());
           sent="SELECT AM.NAME AS NAME,AM.SEARCHKEY AS SEARCHKEY,A.OBCREDIT+A.CURCREDIT AS CAMOUNT,A.OBDEBIT+A.CURDEBIT AS DAMOUNT,AM.PARENT AS PARENT,AM.LEVEL AS LEVEL,AM.SUMMARY AS SUMMARY FROM ACCOUNTMASTER AM LEFT OUTER JOIN AJPERIODTOTALS A ON AM.ID=A.ACCOUNTID ";
           if(statement==null)
               statement=sent;
           else
               statement=statement+" union all "+sent;
         //  list.add(d);
          /* String sent1=" SELECT  AM2.NAME AS NAME,AM2.SEARCHKEY AS SEARCHKEY, (SELECT SUM(AMOUNT) FROM ACCOUNTJOURNAL WHERE ACCOUNTID=AM2.ID AND DATE<? AND TRANSTYPE='C' AND ACTIVE=TRUE) AS CAMOUNT,(SELECT SUM(AMOUNT) FROM ACCOUNTJOURNAL WHERE ACCOUNTID=AM2.ID AND DATE<? AND TRANSTYPE='D' AND ACTIVE=TRUE) AS DAMOUNT,AM2.PARENT AS PARENT,AM2.LEVEL AS LEVEL,AM2.SUMMARY AS SUMMARY "+
           ...
           list.add(d3);*/
           //while(){
           sent="SELECT AM.NAME AS NAME,AM.SEARCHKEY AS SEARCHKEY,A.CREDIT AS CAMOUNT,A.DEBIT AS DAMOUNT,AM.PARENT AS PARENT ,AM.LEVEL AS LEVEL,AM.SUMMARY AS SUMMARY FROM ACCOUNTMASTER AM LEFT OUTER JOIN AJPERIODTOTALS A ON AM.ID=A.ACCOUNTID AND EDATE > ? AND A.EDATE< ?";
           if(statement==null)
               statement=sent;
           else
               statement=statement+" union all "+sent;
    ...
    private void initComponents() {
        ...
        reportType.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Ledger", "Day Book", "Cash Book", "Bank Book", "Journal", "Trail Balance" }));
    ...
    private void reportTypeItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_reportTypeItemStateChanged
        // TODO add your handling code here:
        if(reportType.getSelectedIndex()==6){
