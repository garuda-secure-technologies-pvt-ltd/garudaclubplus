...
public class JTicketsBagShared extends JTicketsBag {
    ...
    private void saveCurrentTicket() {
        ...
        if (m_sCurrentTicket != null) {
            try {
                ...
                dlReceipts.insertSharedTicket(m_sCurrentTicket, m_panelticket.getActiveTicket(),m_App.getAppUserView().getUser().getRole());
    ...
    private void setActiveTicket(String id) throws BasicException{
        ...
        // BEGIN TRANSACTION
        TicketInfo ticket = dlReceipts.getSharedTicket(id,m_App.getAppUserView().getUser().getRole());
        if (ticket == null)  {
            // Does not exists ???
    ...
    private boolean selectValidTicket() {
        ...
        try {
            List<SharedTicketInfo> l = dlReceipts.getSharedTicketList(m_App.getAppUserView().getUser().getRole());
            if (l.size() == 0) {
                newTicket();
            } else {
