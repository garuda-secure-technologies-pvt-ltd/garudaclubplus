...
public abstract class DataLogicFacilities extends BeanFactoryDataSingle {
    ...
    public final Object[] getMamberbySkey(String mno) throws BasicException {
        return (Object[]) new StaticSentence(s, "SELECT ID,NAME,MEMTYPE,MOBILE FROM CUSTOMERS WHERE SEARCHKEY = ?", SerializerWriteString.INSTANCE, new SerializerReadBasic(new Datas[]{Datas.STRING, Datas.STRING, Datas.STRING, Datas.STRING})).find(mno);
    }
...
//praveen":
    ...
    //praveen:added to get other standard facility
    ...
    public final List<MemCat> getMemberCategory() throws BasicException {
        return new StaticSentence(s, "SELECT NAME,ID FROM MEMTYPE ORDER BY NAME", null, new SerializerReadClass(MemCat.class)).list();
    }
    ...
    public List<MinimumUsage> getMinUsageSpecifictToMemType(String membertype) throws BasicException {
        List<MinimumUsage> mulist=new StaticSentence(s, "SELECT ID,NAME FROM MINUSAGE WHERE ACTIVE =TRUE  AND  ACHEAD!='NULL' AND  (MEMBERS LIKE ? OR MEMBERS='ALL') ORDER BY NAME", SerializerWriteString.INSTANCE, new SerializerReadClass(MinimumUsage.class)).list(membertype);
        return mulist;
    }
    ...
    public  List<FacilityBillingTableModel.Facilityline> getMembersToBeBilled1(Date dnow, String facility, String type) throws BasicException {
        //DATEDIFF(M.LBILLDATE,?)
         
 return new StaticSentence(s, "SELECT M.ID,C.SEARCHKEY,C.NAME,F.RENEWALAMT,M.SDATE,M.LBILLDATE, "+type+" ,F.DUEPERIOD,C.ACCOUNT,M.MEMNO,CASE WHEN M.USERID IS NULL THEN NULL ELSE (SELECT DNAME FROM MEMDEPENDENT WHERE ID=M.USERID) END,M.USERID,C.MOBILE " + "FROM MEMFACILITYUSAGE M,FACILITY F ,CUSTOMERS C WHERE M.FACILITYTYPE=F.ID AND M.ACTIVE=TRUE AND C.ID=M.MEMNO AND C.VISIBLE=TRUE AND M.FACILITYTYPE=? AND M.LBILLDATE<? ORDER BY C.SEARCHKEY", new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP,  Datas.STRING, Datas.TIMESTAMP}), new SerializerReadClass(FacilityBillingTableModel.Facilityline.class)).list(new Object[]{ dnow, facility, dnow});
    }

    public final List<AccountMasterExt> getaccountElements() throws BasicException {
        return new StaticSentence(s, "SELECT A.ID,A.NAME,A.SEARCHKEY,A.TYPE_,A.SIGN,A.MAXIMUM AS MAXVALUE,A.PARENT,A.EDITABLE FROM ACCOUNTMASTER A WHERE  A.LEVEL_='E' AND A.OPERANDS IS NULL AND A.PARENT IS NOT NULL ORDER BY A.NAME", null, new SerializerReadClass(AccountMasterExt.class)).list();
    }
    ...
    public final List<AccountMasterExt> getaccountMainHeads(String elementid) throws BasicException {
        return new StaticSentence(s, "SELECT A.ID,A.NAME,A.SEARCHKEY,A.TYPE_,A.SIGN,A.MAXIMUM AS MAXVALUE,A.PARENT,A.EDITABLE FROM ACCOUNTMASTER A WHERE A.PARENT=? AND A.LEVEL_='C' AND A.OPERANDS IS NULL ORDER BY A.NAME", SerializerWriteString.INSTANCE, new SerializerReadClass(AccountMasterExt.class)).list(elementid);
    }

    public final List<AccountMasterExt> getaccountMainHeads1(String elementid) throws BasicException {
        return new StaticSentence(s, "SELECT A.ID,A.NAME,A.SEARCHKEY,A.TYPE_,A.SIGN, A.MAXIMUM  AS MAXVALUE,A.PARENT,A.EDITABLE FROM ACCOUNTMASTER A WHERE A.PARENT=? AND A.LEVEL_='C' AND A.OPERANDS IS NULL AND A.SUMMARY=TRUE ORDER BY A.NAME", SerializerWriteString.INSTANCE, new SerializerReadClass(AccountMasterExt.class)).list(elementid);
    }

    public final AccountMasterExt getaccountbysearchkey(String elementid) throws BasicException {
        return (AccountMasterExt) new StaticSentence(s, "SELECT A.ID,A.NAME,A.SEARCHKEY,A.TYPE_,A.SIGN, A.MAXIMUM  AS MAXVALUE,A.PARENT,A.EDITABLE FROM ACCOUNTMASTER A WHERE A.SEARCHKEY=? AND A.OPERANDS IS NULL ORDER BY A.NAME", SerializerWriteString.INSTANCE, new SerializerReadClass(AccountMasterExt.class)).find(elementid);
    }

    public final AccountMasterExt getaccountbyid(String elementid) throws BasicException {
        return (AccountMasterExt) new StaticSentence(s, "SELECT A.ID,A.NAME,A.SEARCHKEY,A.TYPE_,A.SIGN,A.SIGN AS MAXVALUE,A.PARENT,A.EDITABLE FROM ACCOUNTMASTER A WHERE A.ID=? AND A.OPERANDS IS NULL ORDER BY A.NAME", SerializerWriteString.INSTANCE, new SerializerReadClass(AccountMasterExt.class)).find(elementid);
    }
    ...
    public final List<AccountMasterExt> getSubaccountswithparentid(String elementid) throws BasicException {
        return new StaticSentence(s, "SELECT A.ID,A.NAME,A.SEARCHKEY,A.TYPE_,A.SIGN,MAXIMUM AS MAXVALUE,A.PARENT,A.EDITABLE FROM ACCOUNTMASTER A WHERE A.PARENT=? AND A.LEVEL_='S' ORDER BY A.SEARCHKEY", SerializerWriteString.INSTANCE, new SerializerReadClass(AccountMasterExt.class)).list(elementid);
    }

    public final List<AccountMasterExt> getBreakpointswithparentid(String elementid) throws BasicException {
        return new StaticSentence(s, "SELECT A.ID,A.NAME,A.SEARCHKEY,A.TYPE_,A.SIGN,MAXIMUM AS MAXVALUE,A.PARENT,A.EDITABLE FROM ACCOUNTMASTER A WHERE A.PARENT=? AND A.LEVEL_='D' ORDER BY A.SEARCHKEY", SerializerWriteString.INSTANCE, new SerializerReadClass(AccountMasterExt.class)).list(elementid);
    }

    public final List<AccountMasterExt> getAccountswithparentid(String elementid) throws BasicException {
        return new StaticSentence(s, "SELECT A.ID,A.NAME,A.SEARCHKEY,A.TYPE_,A.SIGN,MAXIMUM AS MAXVALUE,A.PARENT,A.EDITABLE FROM ACCOUNTMASTER A WHERE A.PARENT=? AND A.LEVEL_='C' ORDER BY A.SEARCHKEY", SerializerWriteString.INSTANCE, new SerializerReadClass(AccountMasterExt.class)).list(elementid);
    }

    public final AccountMasterExt getBreakdownsparentAcc(String elementid) throws BasicException {
        AccountMasterExt accmaster;
        do {
            accmaster = (AccountMasterExt) new StaticSentence(s, "SELECT A.ID,A.NAME,A.SEARCHKEY,A.TYPE_,A.SIGN,(SELECT A1.MAXIMUM FROM ACCOUNTMASTER A1 WHERE A1.SEARCHKEY=A.PARENT) AS MAXVALUE,A.PARENT,A.EDITABLE FROM ACCOUNTMASTER A WHERE A.SEARCHKEY=? AND A.OPERANDS IS NULL ORDER BY A.NAME", SerializerWriteString.INSTANCE, new SerializerReadClass(AccountMasterExt.class)).find(elementid);
        //if(acc=null)
    ...
    public final List<AccountMasterExt> getaccountBreakpoints(String elementid) throws BasicException {
        return new StaticSentence(s, "SELECT A.ID,A.NAME,A.SEARCHKEY,A.TYPE_,A.SIGN, A.MAXIMUM  AS MAXVALUE,A.PARENT,A.EDITABLE FROM ACCOUNTMASTER A WHERE A.PARENT=? AND A.LEVEL_='D' ", SerializerWriteString.INSTANCE, new SerializerReadClass(AccountMasterExt.class)).list(elementid);
    }

    public final List<AccountMasterExt> getaccounts() throws BasicException {
        return new StaticSentence(s, "SELECT A.ID,A.NAME,A.SEARCHKEY,A.TYPE_,A.SIGN,A.SUMMARY,A.PARENT,A.EDITABLE FROM ACCOUNTMASTER A WHERE  LEVEL_='S' ORDER BY A.NAME", SerializerWriteString.INSTANCE, new SerializerReadClass(AccountMasterExt.class)).list();
    }
    ...
    public final String getNextBillno(String name) throws BasicException {
        String bill = "";
        Object[] obj = (Object[]) new StaticSentence(s, "SELECT SERIES,MAXVALUE FROM BILLSEQUENCE WHERE NAME = ?", SerializerWriteString.INSTANCE, new SerializerReadBasic(new Datas[]{Datas.STRING, Datas.STRING})).find(name);
        if (obj != null) {
            bill = obj[0].toString() + (Integer.parseInt(obj[1].toString()) + 1);
        }
        return bill;
    ...
    public final void updateBillNum(String name) throws BasicException {
        new PreparedSentence(s, "UPDATE BILLSEQUENCE SET MAXVALUE=(MAXVALUE + 1) WHERE NAME=?", new SerializerWriteBasic(new Datas[]{Datas.STRING})).exec(new Object[]{name});
    }
    ...
    public final List<GuestCategory> getGuestCategory() throws BasicException {
        return new StaticSentence(s, "SELECT ID,NAME,DAYS,MAXGUEST,RATE,ENTRANCECONTROL,RECEIPTSEQ,MRECEIPTSEQ,CREATEDBY,CRDATE,DEACTBY,DEACTDATE FROM GUESTCAT WHERE ACTIVE=TRUE ORDER BY NAME", null, new SerializerReadClass(GuestCategory.class)).list();
    }

    public final GuestCategory getGuestCategoryByID(String id) throws BasicException {
        return (GuestCategory) new StaticSentence(s, "SELECT ID,NAME,DAYS,MAXGUEST,RATE,ENTRANCECONTROL,RECEIPTSEQ,MRECEIPTSEQ,CREATEDBY,CRDATE,DEACTBY,DEACTDATE FROM GUESTCAT WHERE ID=? ", SerializerWriteString.INSTANCE, new SerializerReadClass(GuestCategory.class)).find(id);
    }

    public final List<GuestCategory> getGuestCategoryALL() throws BasicException {
        return new StaticSentence(s, "SELECT ID,NAME,DAYS,MAXGUEST,RATE,ENTRANCECONTROL,RECEIPTSEQ,MRECEIPTSEQ,CREATEDBY,CRDATE,DEACTBY,DEACTDATE FROM GUESTCAT  ORDER BY ACTIVE,NAME", null, new SerializerReadClass(GuestCategory.class)).list();
    }
    ...
    public final boolean updateaccountjournal(String id, Date d, String rno, Double bamt, Boolean adjusted) throws BasicException {
        ...
        ,"UPDATE ACCOUNTJOURNAL SET BALANCEAMOUNT = ?,ADJUSTED=?,CLEARDATE=?,PAYMENTREF=(PAYMENTREF+' : '+?) WHERE ID = ? AND ADJUSTED=FALSE"
        ,new SerializerWriteBasic(new Datas[] {Datas.DOUBLE,Datas.BOOLEAN,Datas.TIMESTAMP,Datas.STRING,Datas.STRING})
        ...
        if (new PreparedSentence(s, "UPDATE ACCOUNTJOURNAL SET BALANCEAMOUNT = ?,ADJUSTED=?,CLEARDATE=?,PAYMENTREF=?  WHERE ID = ? AND PAYMENTREF IS NULL AND ADJUSTED=FALSE", new SerializerWriteBasic(new Datas[]{Datas.DOUBLE, Datas.BOOLEAN, Datas.TIMESTAMP, Datas.STRING, Datas.STRING})).exec(new Object[]{bamt, adjusted, d, rno, id}) <= 0) {
            cnt = new PreparedSentence(s, "UPDATE ACCOUNTJOURNAL SET BALANCEAMOUNT = ?,ADJUSTED=?,CLEARDATE=?,PAYMENTREF=(PAYMENTREF+' : '+?) WHERE ID = ? AND ADJUSTED=FALSE", new SerializerWriteBasic(new Datas[]{Datas.DOUBLE, Datas.BOOLEAN, Datas.TIMESTAMP, Datas.STRING, Datas.STRING})).exec(new Object[]{bamt, adjusted, d, rno, id});
            if (cnt == 0) {
                flag = false;
            }
        }
        ...
        if (flag == false) {
            ...
            if (new PreparedSentence(s, "UPDATE AJUNADJUSTED SET BALANCEAMOUNT = ?,ADJUSTED=?,CLEARDATE=?,PAYMENTREF=?  WHERE ID = ? AND PAYMENTREF IS NULL AND ADJUSTED=FALSE", new SerializerWriteBasic(new Datas[]{Datas.DOUBLE, Datas.BOOLEAN, Datas.TIMESTAMP, Datas.STRING, Datas.STRING})).exec(new Object[]{bamt, adjusted, d, rno, id}) <= 0) {
                cnt1 = new PreparedSentence(s, "UPDATE AJUNADJUSTED SET BALANCEAMOUNT = ?,ADJUSTED=?,CLEARDATE=?,PAYMENTREF=(PAYMENTREF+' : '+?) WHERE ID = ? AND ADJUSTED=FALSE", new SerializerWriteBasic(new Datas[]{Datas.DOUBLE, Datas.BOOLEAN, Datas.TIMESTAMP, Datas.STRING, Datas.STRING})).exec(new Object[]{bamt, adjusted, d, rno, id});
                if (cnt1 == 0) {
                    flag = false;
                }
            }
            ...
            if (flag == true) {
                ...
                if (new PreparedSentence(s, "UPDATE " + name1 + " SET BALANCEAMOUNT = ?,ADJUSTED=?,CLEARDATE=?,PAYMENTREF=?  WHERE ID = ? AND PAYMENTREF IS NULL AND ADJUSTED=FALSE", new SerializerWriteBasic(new Datas[]{Datas.DOUBLE, Datas.BOOLEAN, Datas.TIMESTAMP, Datas.STRING, Datas.STRING})).exec(new Object[]{bamt, adjusted, d, rno, id}) <= 0) {
                    cnt2 = new PreparedSentence(s, "UPDATE " + name1 + " SET BALANCEAMOUNT = ?,ADJUSTED=?,CLEARDATE=?,PAYMENTREF=(PAYMENTREF+' : '+?) WHERE ID = ? AND ADJUSTED=FALSE", new SerializerWriteBasic(new Datas[]{Datas.DOUBLE, Datas.BOOLEAN, Datas.TIMESTAMP, Datas.STRING, Datas.STRING})).exec(new Object[]{bamt, adjusted, d, rno, id});
                    if (cnt2 == 0) {
                        flag = false;
                    }
                }
    ...
    public final void updateaccountjournal1(String id, Double bamt, String rnum, Double payableamt) throws BasicException {
        ...
        if (new PreparedSentence(s, " UPDATE ACCOUNTJOURNAL SET BALANCEAMOUNT = ?,PAYMENTREF=? WHERE ID = ?  AND PAYMENTREF IS NULL AND BALANCEAMOUNT=? ", new SerializerWriteBasic(new Datas[]{Datas.DOUBLE, Datas.STRING, Datas.STRING, Datas.DOUBLE})).exec(new Object[]{bamt, rnum, id, payableamt}) <= 0) {
            cnt = new PreparedSentence(s, " UPDATE ACCOUNTJOURNAL SET BALANCEAMOUNT = ?,PAYMENTREF=(PAYMENTREF+' : '+?) WHERE ID = ? AND BALANCEAMOUNT=? ", new SerializerWriteBasic(new Datas[]{Datas.DOUBLE, Datas.STRING, Datas.STRING, Datas.DOUBLE})).exec(new Object[]{bamt, rnum, id, payableamt});
            if (cnt <= 0) {
                flag = false;
            }
        }
        ...
        if (flag == false) {
            ...
            if (new PreparedSentence(s, " UPDATE AJUNADJUSTED SET BALANCEAMOUNT = ?,PAYMENTREF=? WHERE ID = ?  AND PAYMENTREF IS NULL AND BALANCEAMOUNT=? ", new SerializerWriteBasic(new Datas[]{Datas.DOUBLE, Datas.STRING, Datas.STRING, Datas.DOUBLE})).exec(new Object[]{bamt, rnum, id, payableamt}) <= 0) {
                cnt1 = new PreparedSentence(s, " UPDATE AJUNADJUSTED SET BALANCEAMOUNT = ?,PAYMENTREF=(PAYMENTREF+' : '+?) WHERE ID = ? AND BALANCEAMOUNT=? ", new SerializerWriteBasic(new Datas[]{Datas.DOUBLE, Datas.STRING, Datas.STRING, Datas.DOUBLE})).exec(new Object[]{bamt, rnum, id, payableamt});
                if (cnt1 <= 0) {
                    flag = false;
                }
            }
            ...
            if (flag == true) {
                ...
                if (new PreparedSentence(s, " UPDATE " + name1 + " SET BALANCEAMOUNT = ?,PAYMENTREF=? WHERE ID = ?  AND PAYMENTREF IS NULL AND BALANCEAMOUNT=? ", new SerializerWriteBasic(new Datas[]{Datas.DOUBLE, Datas.STRING, Datas.STRING, Datas.DOUBLE})).exec(new Object[]{bamt, rnum, id, payableamt}) <= 0) {
                    cnt1 = new PreparedSentence(s, " UPDATE " + name1 + " SET BALANCEAMOUNT = ?,PAYMENTREF=(PAYMENTREF+' : '+?) WHERE ID = ? AND BALANCEAMOUNT=? ", new SerializerWriteBasic(new Datas[]{Datas.DOUBLE, Datas.STRING, Datas.STRING, Datas.DOUBLE})).exec(new Object[]{bamt, rnum, id, payableamt});
                    if (cnt1 <= 0) {
                        flag = false;
                    }
                }
    ...
    public final void updatecredit(Double amt, Boolean flag, String id, String ref) throws BasicException {
        if ((new PreparedSentence(s, "UPDATE ACCOUNTJOURNAL SET BALANCEAMOUNT=?,ADJUSTED=?,PAYMENTREF=? WHERE ID=? AND ADJUSTED=FALSE AND PAYMENTREF IS  NULL", new SerializerWriteBasic(new Datas[]{Datas.DOUBLE, Datas.BOOLEAN, Datas.STRING, Datas.STRING})).exec(new Object[]{amt, flag, ref, id})) <= 0) {
            new PreparedSentence(s, "UPDATE ACCOUNTJOURNAL SET BALANCEAMOUNT=?,ADJUSTED=?,PAYMENTREF=(PAYMENTREF+' : '+?) WHERE ID=? AND ADJUSTED=FALSE AND PAYMENTREF IS NOT NULL", new SerializerWriteBasic(new Datas[]{Datas.DOUBLE, Datas.BOOLEAN, Datas.STRING, Datas.STRING})).exec(new Object[]{amt, flag, ref, id});
        }
    ...
    public final void updateDebit(String ref, Double amt, String id) throws BasicException {
        ...
        if ((new PreparedSentence(s, "UPDATE ACCOUNTJOURNAL SET PAYMENTREF=?,BALANCEAMOUNT=? WHERE ID=? AND PAYMENTREF IS NULL", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.DOUBLE, Datas.STRING})).exec(new Object[]{ref, amt, id})) <= 0) {
            new PreparedSentence(s, "UPDATE ACCOUNTJOURNAL SET PAYMENTREF=(PAYMENTREF+' : '+?),BALANCEAMOUNT=? WHERE ID=?", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.DOUBLE, Datas.STRING})).exec(new Object[]{ref, amt, id});
        }
    //  }
    }
    ...
    public final AppUser getnewuserdetail(DataLogicSystem dlSystem, String id) throws BasicException {
        Object[] obj = (Object[]) new StaticSentence(s, "SELECT ID,NAME,APPPASSWORD,CARD,ROLE,IMAGE,LOGINTIME,CLOSECASHTIME,OPENCASHTIME,CLOSESALE,OPENSALE,CASHACCOUNT,CHEQUEACCOUNT FROM PEOPLE WHERE NAME=?", SerializerWriteString.INSTANCE, new SerializerReadBasic(new Datas[]{Datas.STRING, Datas.STRING, Datas.STRING, Datas.STRING, Datas.STRING, Datas.IMAGE, Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.STRING, Datas.STRING})).find(id);

        AppUser appuser = new AppUser(obj[0].toString(), obj[1].toString(), obj[4].toString(), obj[11].toString(), obj[12].toString());
        appuser.fillPermissions(dlSystem);
        return appuser;
    }

    public final List<AccountMasterExt> getsubAccounts(String name) throws BasicException {
        return new StaticSentence(s, "SELECT A.ID,A.NAME,A.SEARCHKEY,A.TYPE_,A.SIGN,A.SUMMARY,A.PARENT,A.EDITABLE FROM ACCOUNTMASTER A WHERE  A.LEVEL_='S' AND UCASE(A.NAME) LIKE ? ORDER BY A.NAME", SerializerWriteString.INSTANCE, new SerializerReadClass(AccountMasterExt.class)).list(name + "%");
    }

    public final List<AccountMasterExt> getsubAccounts1(String name) throws BasicException {
        return new StaticSentence(s, "SELECT A.ID,A.NAME,A.SEARCHKEY,A.TYPE_,A.SIGN,A.SUMMARY,A.PARENT,A.EDITABLE FROM ACCOUNTMASTER A WHERE  A.SUMMARY=FALSE AND UCASE(A.NAME) LIKE ? ORDER BY A.NAME", SerializerWriteString.INSTANCE, new SerializerReadClass(AccountMasterExt.class)).list(name + "%");
    }

    public final List<AccountMaster> getsubAccountsExceptOB(String name) throws BasicException {
        return new StaticSentence(s, "SELECT A.ID,A.NAME,A.SEARCHKEY,A.PARENT FROM ACCOUNTMASTER A WHERE  A.LEVEL_='S' AND A.SEARCHKEY !='1.1.7' AND UCASE(A.NAME) LIKE ? ORDER BY A.NAME", SerializerWriteString.INSTANCE, new SerializerReadClass(AccountMaster.class)).list(name + "%");
    }

    public final List<AccountMasterExt> getsubAccountsExceptMemAccandCashAcc(String name) throws BasicException {
        return new StaticSentence(s, "SELECT A.ID,A.NAME,A.SEARCHKEY,A.TYPE_,A.SIGN,A.SUMMARY,A.PARENT,A.EDITABLE FROM ACCOUNTMASTER A WHERE  A.LEVEL_='S' AND A.PARENT!='1.1.5' AND A.PARENT!='1.1.1' AND UCASE(A.NAME) LIKE ? ORDER BY A.NAME", SerializerWriteString.INSTANCE, new SerializerReadClass(AccountMasterExt.class)).list(name + "%");
    }
    ...
    public final boolean checkForGameLog(String gamelogid) {
        try {
            ...
            int cnt = (Integer) new StaticSentence(s, "SELECT COUNT(ID) FROM TOKENLOG T,GAMELOG G WHERE G.ID=T.GAMEREF AND MEMID=? AND G.COMPLETEDON IS NULL", SerializerWriteString.INSTANCE, SerializerReadInteger.INSTANCE).find(gamelogid);
            if (cnt >= 1) {
                return true;
            } else {
                return false;
    ...
    public final boolean checkForMembersAlreadyAdded(String id) {
        try {
            int cnt = (Integer) new StaticSentence(s, "SELECT COUNT(ID) FROM TOKENLOG T,GAMELOG G WHERE G.ID=T.GAMEREF AND MEMID=? AND G.COMPLETEDON IS NULL", SerializerWriteString.INSTANCE, SerializerReadInteger.INSTANCE).find(id);
            if (cnt >= 1) {
                return true;
            } else {
                return false;
    ...
    public final void insertNewGameInstance(Object[] obj) throws BasicException {
        ...
        new PreparedSentence(s, "INSERT INTO GAMELOG ( ID,CRDATE,CRBY,NOP,JACKID,GAMEID,CLUBCOLLECTION,TABLE,GAMESEQ) VALUES (?,?,?,?,?,?,?,?,?)", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.TIMESTAMP, Datas.STRING, Datas.INT, Datas.STRING, Datas.INT, Datas.DOUBLE, Datas.STRING, Datas.STRING})).exec(obj);
    }
    ...
    public final List<CancelRequestInfo> LoadPendingGameCancellationRequest() throws BasicException {
        return new PreparedSentence(s, " SELECT ID,GAMEID,INITIATEDBY,INITIATEDDATE,PROCESSEDBY,PROCESSEDDATE,STATUS_ FROM GAMECANCELREQUEST WHERE STATUS_='0'", null, new SerializerReadClass(CancelRequestInfo.class)).list();
    }
