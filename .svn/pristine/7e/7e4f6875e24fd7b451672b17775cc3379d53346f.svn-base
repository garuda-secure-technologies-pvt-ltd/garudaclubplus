 ...
 * @author swathi
 */
public class MemTypeTableModel {
  ...
  private MemTypeTableModel()
  {
  }

  public static MemTypeTableModel emptyinstance()
  {
      MemTypeTableModel d=new MemTypeTableModel();
      d.fac=new ArrayList<MemTypeline>();
      return d;
  }
  public static MemTypeTableModel loadInstance(AppView app,int flag) throws BasicException{
      MemTypeTableModel d = new MemTypeTableModel();
         d.flag=flag;
         ...
         if(flag==1){
           dlist = new StaticSentence(app.getSession()
                ...
                +"FROM MEMTYPE M,PERIODICITY P1 WHERE M.ACTIVE = TRUE  AND P1.ID=M.PERIOD "
              ,SerializerWriteString.INSTANCE
              ,new SerializerReadClass( MemTypeTableModel.MemTypeline.class )).list();
         }else
         {
            dlist = new StaticSentence(app.getSession()
                ,"SELECT M.ID,M.NAME,P1.NAME,M.DATE,M.CREATEDBY,M.DEACTIVATEDDATE,M.DEACTIVATEDBY,M.DEBTMAX "
