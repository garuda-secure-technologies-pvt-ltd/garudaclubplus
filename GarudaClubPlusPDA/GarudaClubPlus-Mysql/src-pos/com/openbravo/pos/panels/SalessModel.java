...
public class SalessModel  {
    ...
    public static SalessModel loadInstance(AppView app,Date d) throws BasicException {
       ...
       Object[] valtickets = (Object []) new StaticSentence(app.getSession()
                ...
                "WHERE  PEOPLE.NAME=BILL.CREATEDBY AND ROLES.ID=PEOPLE.ROLE AND  BILL.CLOSESALESEQ IS NULL AND BILL.CREATEDDATE <= ? AND  ROLES.ID = ?  "
                ,new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP,Datas.STRING})
                ,new SerializerReadBasic(new Datas[] { Datas.DOUBLE, Datas.DOUBLE})).find(new Object[]{d,app.getAppUserView().getUser().getRole()});
        if(valtickets[0] == null && valtickets[1]!= null ) {
         ...
         List disqt = new StaticSentence(app.getSession()
                ...
                , new SerializerReadClass( SalessModel.discountlist.class )).list(new Object[]{d,d1,app.getAppUserView().getUser().getRole()});
        ...
        if (disqt == null) {
          ...
          List disbill = new StaticSentence(app.getSession()
                ,"SELECT R.BILLID,P.NAME,(R.QTY * -1),(R.RATE * -1),R.CREATEDBY,R.REASON "
                ...
                ,new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP,Datas.STRING})
                , new SerializerReadClass( SalessModel.discountlist.class )).list(new Object[]{d,app.getAppUserView().getUser().getRole()});
        ...
        if (disbill == null) {
           ...
           List disbillstk = new StaticSentence(app.getSession()
                ,"SELECT R.BILLID,P.ID,(R.QTY * -1),(R.RATE * -1),R.CREATEDBY,R.REASON "
                +"FROM REVERSEDBILL R,PRODUCTS P ,PEOPLE,ROLES,BILL "
                +"WHERE   R.BILLID=BILL.ID AND BILL.CREATEDDATE <PEOPLE.OPENSALE AND  PEOPLE.NAME=R.CREATEDBY AND ROLES.ID=PEOPLE.ROLE AND P.ID=R.PRODUCT AND R.AUTHORISED=TRUE AND R.CRDATE <= ? AND ROLES.ID = ? AND BILL.CLOSESALESEQ IS NULL  "
                ,new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP,Datas.STRING})
                , new SerializerReadClass( SalessModel.discountlist.class )).list(new Object[]{d,app.getAppUserView().getUser().getRole()});
     ...
     /*    List disqtstk = new StaticSentence(app.getSession()
         ...
         * */
         ...
         List<ProductsCategoryLine> categorywise = new StaticSentence(app.getSession()
                ...
                +"FROM BILL, PEOPLE ,ROLES,PRODUCTS,BILLITEM,CATEGORIES C "
                +"WHERE   BILL.CLOSESALESEQ IS NULL AND  PEOPLE.NAME=BILL.CREATEDBY AND BILL.ID=BILLITEM.PARENTID AND BILLITEM.PRODUCT=PRODUCTS.ID AND ROLES.ID=PEOPLE.ROLE AND BILL.CREATEDDATE <=?   AND ROLES.ID = ? AND PRODUCTS.CATEGORY=C.ID  GROUP BY PRODUCTS.CATEGORY,C.NAME,C.PARENTID,C.ID ORDER BY C.PARENTID"
                ,new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP,Datas.STRING})
                , new SerializerReadClass( SalessModel.ProductsCategoryLine.class )).list(new Object[]{d,app.getAppUserView().getUser().getRole()});
         ...
         List productaccountwise = new StaticSentence(app.getSession()
                ,"SELECT PRODUCTS.SACCOUNT,SUM(BILLITEM.RATE * BILLITEM.DMULTIPLY) "
                +"FROM BILL, PEOPLE ,ROLES,PRODUCTS,BILLITEM "
                +"WHERE   BILL.CLOSESALESEQ IS NULL  AND PEOPLE.NAME=BILL.CREATEDBY AND BILL.ID=BILLITEM.PARENTID AND BILLITEM.PRODUCT=PRODUCTS.ID AND ROLES.ID=PEOPLE.ROLE AND BILL.CREATEDDATE <=?   AND ROLES.ID = ?  GROUP BY PRODUCTS.SACCOUNT"
                ,new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP,Datas.STRING})
                , new SerializerReadClass( SalessModel.ProductsAccountLine.class )).list(new Object[]{d,app.getAppUserView().getUser().getRole()});
        ...
        List pendingqt = new StaticSentence(app.getSession()
                ," SELECT QTICKET.ID, QTICKET.CUSTOMER, WAITER.NAME,SUM(QTITEMS.DMULTIPLY * QTITEMS.RATE) AS AMOUNT, CRDATE,QTICKET.REASON FROM QTICKET,WAITER,QTITEMS,ROLES,PEOPLE "+
                     " WHERE QTITEMS.PARENTID=QTICKET.ID AND BILLED = FALSE AND WAITER.ID=QTICKET.WAITER AND ROLES.ID=PEOPLE.ROLE AND PEOPLE.NAME=QTICKET.CREATEDBY  AND ROLES.ID = ?  "+
                     " GROUP BY QTICKET.ID,QTICKET.CUSTOMER, WAITER.NAME,CRDATE,QTICKET.REASON "+
                    " ORDER BY QTICKET.ID "
                ,new SerializerWriteBasic(new Datas[]{Datas.STRING})
                ,new SerializerReadClass(SalessModel.QTLine.class)).list(new Object[]{app.getAppUserView().getUser().getRole()});
        ...
        Object[] obj= (Object[])new StaticSentence(app.getSession()
                ," SELECT SUM(QTITEMS.DMULTIPLY * QTITEMS.RATE) AS AMOUNT FROM QTICKET,QTITEMS,PEOPLE,ROLES "+
                     " WHERE QTITEMS.PARENTID=QTICKET.ID AND BILLED = FALSE  AND ROLES.ID=PEOPLE.ROLE AND PEOPLE.NAME=QTICKET.CREATEDBY  AND ROLES.ID = ?   "
                    ,new SerializerWriteBasic(new Datas[]{Datas.STRING})
                ,new SerializerReadBasic(new Datas [] { Datas.DOUBLE})).find(new Object[]{app.getAppUserView().getUser().getRole()});
...
*/      Object[] x = (Object[]) new StaticSentence(app.getSession()
                ...
                "WHERE  BILL.CLOSESALESEQ IS NULL   AND PRODUCTS.ID = BILLITEM.PRODUCT AND PEOPLE.NAME=BILL.CREATEDBY AND BILL.ID = BILLITEM.PARENTID AND BILL.CREATEDDATE <=?  AND ROLES.ID = ? AND ROLES.ID=PEOPLE.ROLE"
                ,new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP,Datas.STRING})
                ,new SerializerReadBasic(new Datas [] { Datas.DOUBLE})).find(new Object[]{d,app.getAppUserView().getUser().getRole()});
          ...
          Object[] y = (Object[]) new StaticSentence(app.getSession()
                ...
                "FROM BILL, PEOPLE ,ROLES " +
                "WHERE   BILL.CLOSESALESEQ IS NULL AND PEOPLE.NAME=BILL.CREATEDBY AND BILL.CREATEDDATE <=?   AND ROLES.ID = ? AND ROLES.ID=PEOPLE.ROLE "
                 ,new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP,Datas.STRING})
                ,new SerializerReadBasic(new Datas [] { Datas.DOUBLE})).find(new Object[]{d,app.getAppUserView().getUser().getRole()});
          if(y[0] == null ) {
                    p.taxtotal = 0.0;
        ...
        List pdtwise = new StaticSentence(app.getSession()
                , "SELECT PRODUCTS.NAME,SUM(DMULTIPLY),BILLITEM.RATE ,SUM(BILLITEM.TOTAL),PRODUCTS.ID,PRODUCTS.CATEGORY "+
                "FROM BILLITEM,PRODUCTS,BILL,PEOPLE ,ROLES "+
                "WHERE BILLITEM.PARENTID = BILL.ID AND PRODUCTS.ID = BILLITEM.PRODUCT AND "+
                "  BILL.CLOSESALESEQ IS NULL AND BILL.CREATEDDATE <=?  AND PEOPLE.NAME=BILL.CREATEDBY AND ROLES.ID=PEOPLE.ROLE AND ROLES.ID = ?  "+
                "GROUP BY PRODUCTS.ID ,PRODUCTS.NAME , BILLITEM.RATE,PRODUCTS.CATEGORY,PRODUCTS.ID " +
                "ORDER BY PRODUCTS.CATEGORY"
                ,new SerializerWriteBasic(new Datas[]{Datas.TIMESTAMP,Datas.STRING})
                ,new SerializerReadClass(SalessModel.ProductsLine.class)).list(new Object[]{d,app.getAppUserView().getUser().getRole()});
