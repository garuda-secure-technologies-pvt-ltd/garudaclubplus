...
public class StockMovementDetails extends javax.swing.JPanel implements JPanelView, BeanFactoryApp {
    ...
    public StockMovementDetails() {
     ...
     public void init(AppView app) throws BeanFactoryException {
        ...
        stockdetail.setDefaultRenderer(Object.class, new TableRendererBasic(
                new Formats[] {Formats.DOUBLE, Formats.TIMESTAMP,Formats.STRING,Formats.DOUBLE,Formats.STRING,Formats.STRING,Formats.DOUBLE,Formats.STRING,Formats.STRING,Formats.STRING}));
   ...
   private void loadData() throws BasicException {
        ...
        m_stockmodel = StockTransferTable.loadInstance(m_App);
    ...
    private void receivedActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_receivedActionPerformed
      ...
      String pdtname1=m_stockmodel.getdiscountTableModel().getValueAt(row[j], 2).toString();
   //   Double qty1=(Double) m_stockmodel.getdiscountTableModel().getValueAt(row, 3);
       Double qty2=(Double) m_stockmodel.getdiscountTableModel().getValueAt(row[j], 6);
       String pdtname2=m_stockmodel.getdiscountTableModel().getValueAt(row[j], 5).toString();
     //   String location=m_stockmodel.getdiscountTableModel().getValueAt(row, 10).toString();
         String location1=m_stockmodel.getdiscountTableModel().getValueAt(row[j], 11).toString();
          String id=m_stockmodel.getdiscountTableModel().getValueAt(row[j], 12).toString();
      String user=LookupUtilityImpl.getInstance(null).getAppView().getAppUserView().getUser().getName();
      try{
       int temp=  new StaticSentence(m_App.getSession()
                        , "UPDATE STOCKDIARY SET RECEIVEDBY = ? WHERE NUM=? AND CREATEDBY != ? AND ID=? AND RECEIVEDBY IS NULL"
                        , new SerializerWriteBasic(new Datas[] {Datas.STRING,Datas.DOUBLE,Datas.STRING,Datas.STRING}))
                        .exec(new Object[] {user,num,user,id});
      if(temp>0)
      {
       Object[] pdtid=(Object []) new StaticSentence(m_App.getSession()
                        , "SELECT P.ID,P1.ID FROM PRODUCTS P, PRODUCTS P1 WHERE P.NAME= ? AND P1.NAME=?"
                        , new SerializerWriteBasic(new Datas[] {Datas.STRING,Datas.STRING})
                        ,new SerializerReadBasic(new Datas[]{Datas.STRING,Datas.STRING}))
                        .find(new Object[] {pdtname1,pdtname2});
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
      ...
      for(int j=0;j<rowcount;j++){
        Double num=(Double)  m_stockmodel.getdiscountTableModel().getValueAt(row[j], 0);
        String pdtname1=m_stockmodel.getdiscountTableModel().getValueAt(row[j], 2).toString();
        Double qty1=(Double) m_stockmodel.getdiscountTableModel().getValueAt(row[j], 3);
        String location=m_stockmodel.getdiscountTableModel().getValueAt(row[j], 10).toString();
       //  String location1=m_stockmodel.getdiscountTableModel().getValueAt(row, 11).toString();
        String id=m_stockmodel.getdiscountTableModel().getValueAt(row[j], 12).toString();
        String user=LookupUtilityImpl.getInstance(null).getAppView().getAppUserView().getUser().getName();
        try{
          int temp=  new StaticSentence(m_App.getSession()
                        , "DELETE FROM STOCKDIARY WHERE CREATEDBY = ? AND ID=? AND RECEIVEDBY IS NULL"
                        , new SerializerWriteBasic(new Datas[] {Datas.STRING,Datas.STRING}))
                        .exec(new Object[] {user,id});
          if(temp==1){
          Object[] pdtid=(Object []) new StaticSentence(m_App.getSession()
