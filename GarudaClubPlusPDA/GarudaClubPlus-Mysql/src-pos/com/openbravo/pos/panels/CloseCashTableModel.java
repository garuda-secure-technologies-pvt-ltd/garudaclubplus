...
public class CloseCashTableModel {
    ...
    public static CloseCashTableModel loadInstanceOfCash(AppView app, Date fdate, Date todate, String seq) throws BasicException {
        CloseCashTableModel c1 = new CloseCashTableModel();
        c1.receiptDetails = new ArrayList<CashList>();
        List<CashList> saleDetailList = new StaticSentence(app.getSession(), "SELECT RID,RDATE,RUSER,descr,CSEQ,AMOUNT FROM (SELECT R.ID AS RID,R.DATENEW AS RDATE,R.RUSER AS RUSER,R.DESC_ AS DESCR,R.CLOSECASHSEQ AS CSEQ,P.TOTAL AS AMOUNT FROM RECEIPTS R,PAYMENTS P WHERE R.CLOSECASHSEQ=? AND P.RECEIPT=R.ID AND R.DATENEW BETWEEN ? AND ?" +
                "UNION ALL" +
                " SELECT R.ID AS RID,R.DATENEW AS RDATE,R.RUSER AS RUSER,R.DESC_ AS DESCR,R.CLOSECASHSEQ AS CSEQ,P.TOTAL AS AMOUNT FROM RECEIPTS_ARV R,PAYMENTS_ARV P WHERE R.CLOSECASHSEQ=? AND P.RECEIPT=R.ID AND R.DATENEW BETWEEN ? AND ? )AS CLOSECASH ORDER BY RID", new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.TIMESTAMP, Datas.TIMESTAMP, Datas.STRING, Datas.TIMESTAMP, Datas.TIMESTAMP}), new SerializerReadClass(CloseCashTableModel.CashList.class)).list(new Object[]{seq, fdate, todate, seq, fdate, todate});
