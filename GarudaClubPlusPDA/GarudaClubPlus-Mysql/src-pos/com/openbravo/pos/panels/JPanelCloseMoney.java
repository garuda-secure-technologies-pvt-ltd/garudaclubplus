           ...
           // try {
               ...
               // {
                    new StaticSentence(m_App.getSession()
                        , "UPDATE CLOSEDCASH SET DATEEND = ?,REMARK=?,HOST = ?,HOSTSEQUENCE=? WHERE   USER = ? AND HOSTSEQUENCE<0"
             ...
             for(int k=0;k<m_PaymentsToClose.getcountercount();k++){
                if(m_PaymentsToClose.printReceiptCounter(k).equals("Counter role")){
                     Object[] value2=new Object[]{UUID.randomUUID().toString(),tid,null,dNow,"C","Close cash",m_PaymentsToClose.getSequence()+"", m_PaymentsToClose.getReceiptTotal(k),dNow,true,m_App.getAppUserView().getUser().getName(),m_App.getProperties().getHost(),narration+" restaurant","4",0.0,dNow,null,true};
                     dlfac.insertintoaccjoutnal2(value2);
                }else{
                     Object[] value1=new Object[]{UUID.randomUUID().toString(),tid,null,dNow,"C","Close cash",m_PaymentsToClose.getSequence()+"", m_PaymentsToClose.getReceiptTotal(k),dNow,true,m_App.getAppUserView().getUser().getName(),m_App.getProperties().getHost(),narration +" bar ","3",0.0,dNow,null,true};
                     dlfac.insertintoaccjoutnal2(value1);
                }
               }
                }
              ...
              new PreparedSentence(m_App.getSession()
               ...
               // new PreparedSentence(m_App.getSession()
                ...
                new PreparedSentence(m_App.getSession()
                       , "INSERT INTO RECEIPTS_ARV (ID,DATENEW,ATTRIBUTES,RUSER,PAYMENTREF,DESC_,CLOSECASHSEQ) SELECT R.ID,R.DATENEW,R.ATTRIBUTES,R.RUSER,R.PAYMENTREF,R.DESC_,R.CLOSECASHSEQ FROM RECEIPTS R,BILL B WHERE  R.CLOSECASHSEQ IS NOT NULL AND B.CLOSESALESEQ IS NOT NULL AND B.RECEIPT=R.ID"
                       , null).exec();
                 new PreparedSentence(m_App.getSession()
                       , "INSERT INTO RECEIPTS_ARV (ID,DATENEW,ATTRIBUTES,RUSER,PAYMENTREF,DESC_,CLOSECASHSEQ) SELECT R.ID,R.DATENEW,R.ATTRIBUTES,R.RUSER,R.PAYMENTREF,R.DESC_,R.CLOSECASHSEQ FROM RECEIPTS R  WHERE  R.CLOSECASHSEQ IS NOT NULL AND  R.ID NOT IN (SELECT RECEIPT FROM BILL) "
                       , null).exec();
                   new PreparedSentence(m_App.getSession()
