   ...
   public boolean deactivate() {
        ...
        if(jTable1.getSelectedRowCount()==1)
        {
          ...
          try{
           new StaticSentence(m_App.getSession()
                        , "UPDATE REVERSEDBILL SET AUTHORISED = FALSE WHERE ID=? AND AUTHORISED IS NULL "
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
       ...
       int del=jTable1.getSelectedRowCount();
         if(jTable1.getSelectedRowCount()==1)
        {
          ...
          try{
           new StaticSentence(m_App.getSession()
                        , "DELETE FROM REVERSEDBILL WHERE BILLID = ? AND PRODUCT =? AND CREATEDBY=? AND AUTHORISED IS NULL "
                        , new SerializerWriteBasic(new Datas[] { Datas.STRING,Datas.STRING,Datas.STRING }))
                        .exec(new Object[] {id,product,m_App.getAppUserView().getUser().getName()});
           loadData();
             }
          catch(Exception e)
           {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if(jTable1.getSelectedRowCount()==1)
        {
          ...
          try{
           int count=new StaticSentence(m_App.getSession()
                        , "UPDATE REVERSEDBILL SET AUTHORISED = TRUE,CRDATE=? WHERE BILLID = ? AND PRODUCT =? AND AUTHORISED IS NULL "
                        , new SerializerWriteBasic(new Datas[] {Datas.TIMESTAMP, Datas.STRING,Datas.STRING }))
                        .exec(new Object[] {dnow,id,product});
           if(count>0){
        ...
        // Datas type[]=new Datas{Datas.STRING,};
           ...
           taxrate=(rate1 ) * taxrate;
           Double amt=rate1*(qty1 * -1);
            new StaticSentence(m_App.getSession()
                        , "UPDATE BILL SET AMOUNT = (AMOUNT - ?),TAXTOTAL=(TAXTOTAL-?) WHERE ID = ? "
                        , new SerializerWriteBasic(new Datas[] {Datas.DOUBLE, Datas.DOUBLE,Datas.STRING }))
                        .exec(new Object[] {amt,taxrate,id});
            int qline=0;
            String qt="";
            Object[] obj=(Object[])new StaticSentence(m_App.getSession()
                        , "SELECT QT.ID,MAX(Q.LINE) FROM QTICKET QT,QTITEMS Q WHERE Q.PARENTID = QT.ID AND QT.BILLREF = ? GROUP BY Q.PARENTID,QT.ID"
                        , SerializerWriteString.INSTANCE
                        ,new SerializerReadBasic(new Datas[]{Datas.STRING,Datas.INT})).find(id);
            if(obj!=null)
            {
                qt=obj[0].toString();
            ...
            Object value1=new Object[]{UUID.randomUUID().toString(),qline,qt,product,qty1,rate1};
              new PreparedSentence(m_App.getSession()
                    ...
                    , new SerializerWriteBasic(new Datas[]{Datas.STRING,Datas.INT,Datas.STRING,Datas.STRING,Datas.DOUBLE,Datas.DOUBLE})).exec(value1);
